...
ngx_int_t ngx_child_request_start(
	...
	// optional, if it is not supplied, the module will finalize the request when the upstream
    // request completes.
	ngx_child_request_callback_t callback,
	void* callback_context,
	ngx_str_t* internal_location,
	ngx_child_request_params_t* params,
	// optional, if it is not supplied, the upstream response gets written to the parent request.
    // when a response buffer is supplied, the response is written to it, the buffer should be large
    // enough to contain both the response body and the response headers
