...
#define ngx_perf_counter_context(ctx) ngx_perf_counter_context_t ctx

#define ngx_perf_counter_start(ctx) ngx_get_tick_count(&ctx.start);

// NOTE: the calculation of 'max' has a race condition, the value can decrease since the condition
// and the assignment are not performed atomically. however, the value of max expected to converge
// quickly so that its updates will be performed less and less frequently, so it should be accurate
// enough.
