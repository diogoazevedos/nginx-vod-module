...
#define ngx_http_vod_ends_with(start_pos, end_pos, postfix) \
	((end_pos) - (start_pos) >= (int)(postfix)->len         \
	 && ngx_memcmp((end_pos) - (postfix)->len, (postfix)->data, (postfix)->len) == 0)

#define ngx_http_vod_ends_with_static(start_pos, end_pos, postfix) \
	((end_pos) - (start_pos) >= (int)sizeof(postfix) - 1           \
...
ngx_int_t ngx_http_vod_init_uri_params_hash(ngx_conf_t* cf, struct ngx_http_vod_loc_conf_s* conf);
