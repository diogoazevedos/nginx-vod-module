...
ngx_file_reader_init_open_file_info(
	ngx_open_file_info_t* of, ngx_http_request_t* r, ngx_http_core_loc_conf_t* clcf, ngx_str_t* path
) {
	ngx_int_t rc;
	...
	if (rc != NGX_OK) {
		ngx_log_debug1(
			...
			r->connection->log,
			0,
			"ngx_file_reader_init_open_file_info: ngx_http_set_disable_symlinks failed %i",
			rc
		);
...
ngx_file_reader_update_state_file_info(ngx_file_reader_state_t* state, ngx_open_file_info_t* of, ngx_int_t rc) {
	...
	if (rc != NGX_OK) {
		...
		if (rc != NGX_HTTP_NOT_FOUND || state->log_not_found) {
			ngx_log_error(
				...
				state->log,
				of->err,
				"ngx_file_reader_update_state_file_info: %s \"%s\" failed",
				of->failed,
	...
	if (!of->is_file) {
		ngx_log_error(
			...
			state->log,
			0,
			"ngx_file_reader_update_state_file_info: \"%s\" is not a file",
			state->file.name.data
		);
		if (of->fd != NGX_INVALID_FILE) {
			if (ngx_close_file(of->fd) == NGX_FILE_ERROR) {
				ngx_log_error(
					...
					state->log,
					ngx_errno,
					"ngx_file_reader_update_state_file_info: " ngx_close_file_n " \"%s\" failed",
					state->file.name.data
				);
			}
		}
...
ngx_file_reader_init(
	...
	rc = ngx_open_cached_file(
		(flags & OPEN_FILE_NO_CACHE) != 0 ? NULL : clcf->open_file_cache, path, &of, r->pool
	);
...
ngx_file_reader_init_async(
	...
	if (open_context == NULL) {
		...
		if (open_context == NULL) {
			ngx_log_debug0(
				NGX_LOG_DEBUG_HTTP, state->log, 0, "ngx_file_reader_init_async: ngx_palloc failed"
			);
			return NGX_HTTP_INTERNAL_SERVER_ERROR;
		}
...
ngx_file_reader_dump_file_part(void* context, off_t start, off_t end) {
	...
	if (b == NULL) {
		ngx_log_debug0(
			NGX_LOG_DEBUG_HTTP, state->log, 0, "ngx_file_reader_dump_file_part: ngx_pcalloc failed (1)"
		);
		return NGX_HTTP_INTERNAL_SERVER_ERROR;
	}
	...
	if (b->file == NULL) {
		ngx_log_debug0(
			NGX_LOG_DEBUG_HTTP, state->log, 0, "ngx_file_reader_dump_file_part: ngx_pcalloc failed (2)"
		);
		return NGX_HTTP_INTERNAL_SERVER_ERROR;
	}
	...
	if (end != 0) {
		if (end > state->file_size) {
			ngx_log_error(
				...
				state->log,
				ngx_errno,
				"ngx_file_reader_dump_file_part: end offset %O exceeds file size %O, probably a truncated file",
				end,
				state->file_size
			);
	...
	if (rc != NGX_OK && rc != NGX_AGAIN) {
		ngx_log_debug1(
			...
			state->log,
			0,
			"ngx_file_reader_dump_file_part: ngx_http_output_filter failed %i",
			rc
		);
		return rc;
	}
	...
	if (state->directio <= state->file_size) {
		if (ngx_directio_on(state->file.fd) == NGX_FILE_ERROR) {
			ngx_log_error(
				...
				state->log,
				ngx_errno,
				"ngx_file_reader_enable_directio: " ngx_directio_on_n " \"%s\" failed",
				state->file.name.data
			);
...
ngx_async_read_completed_callback(ngx_event_t* ev) {
	...
	if (rc < 0) {
		ngx_log_debug1(
			...
			state->log,
			0,
			"ngx_async_read_completed_callback: ngx_file_aio_read failed rc=%z",
			rc
		);
		bytes_read = 0;
	} else {
		ngx_log_debug1(
			...
			state->log,
			0,
			"ngx_async_read_completed_callback: ngx_file_aio_read returned %z",
			rc
		);
		state->buf->last += rc;
		bytes_read = rc;
...
ngx_async_file_read(ngx_file_reader_state_t* state, ngx_buf_t* buf, size_t size, off_t offset) {
	...
	ngx_log_debug2(
		NGX_LOG_DEBUG_HTTP, state->log, 0, "ngx_async_file_read: reading offset %O size %uz", offset, size
	);
	...
	if (rc < 0) {
		ngx_log_error(
			NGX_LOG_ERR, state->log, 0, "ngx_async_file_read: ngx_file_aio_read failed rc=%z", rc
		);
		return rc;
	}

	ngx_log_debug1(
		NGX_LOG_DEBUG_HTTP, state->log, 0, "ngx_async_file_read: ngx_file_aio_read returned %z", rc
	);
	buf->last += rc;
...
ngx_async_file_read(ngx_file_reader_state_t* state, ngx_buf_t* buf, size_t size, off_t offset) {
	...
	ngx_log_debug1(
		NGX_LOG_DEBUG_HTTP, state->log, 0, "ngx_async_file_read: ngx_read_file returned %z", rc
	);
	buf->last += rc;
