...
ngx_http_vod_parse_string(
	const ngx_http_vod_match_definition_t* match_def, u_char* start_pos, u_char* end_pos, void* output
) {
	uint64_t value;
	u_char* delim_pos;
...
ngx_http_vod_extract_track_tokens(
	ngx_http_request_t* r, u_char* start_pos, u_char* end_pos, track_mask_t* result
) {
...
ngx_http_vod_parse_uri_file_name(
	ngx_http_request_t* r, u_char* start_pos, u_char* end_pos, uint32_t flags, request_params_t* result
) {
	...
	if ((flags & PARSE_FILE_NAME_EXPECT_SEGMENT_INDEX) != 0) {
		...
		if (result->segment_index <= 0) {
			ngx_log_error(
				NGX_LOG_ERR, r->connection->log, 0, "ngx_http_vod_parse_uri_file_name: failed to parse segment index"
			);
		...
		if (*start_pos == 'i') {
			...
			if (segment_index_shift <= 0) {
				ngx_log_error(
					NGX_LOG_ERR, r->connection->log, 0, "ngx_http_vod_parse_uri_file_name: failed to parse segment index shift"
				);
				return ngx_http_vod_status_to_ngx_error(r, VOD_BAD_REQUEST);
			}
	...
	if (*start_pos == 'c' && (flags & PARSE_FILE_NAME_ALLOW_CLIP_INDEX) != 0) {
		...
		if (clip_index <= 0) {
			ngx_log_error(
				NGX_LOG_ERR, r->connection->log, 0, "ngx_http_vod_parse_uri_file_name: failed to parse clip index"
			);
			return ngx_http_vod_status_to_ngx_error(r, VOD_BAD_REQUEST);
		}
	...
	if (*start_pos == 'f' || *start_pos == 's') {
		...
		for (;;) {
			if (*start_pos == 'f') {
				...
				if (start_pos < end_pos && *start_pos >= '0' && *start_pos <= '9') {
					...
					if (sequence_index > MAX_SEQUENCES) {
						ngx_log_error(
							NGX_LOG_ERR, r->connection->log, 0, "ngx_http_vod_parse_uri_file_name: sequence index too big"
						);
						return ngx_http_vod_status_to_ngx_error(r, VOD_BAD_REQUEST);
					}
					start_pos++; // skip the digit
			...
			if (*start_pos == 'v' || *start_pos == 'a') {
				...
				start_pos = ngx_http_vod_extract_track_tokens(r, start_pos, end_pos, tracks_mask);
				if (start_pos == NULL) {
					return NGX_OK;
				}
			}
			...
			sequence_tracks_mask =
				ngx_palloc(r->pool, sizeof(sequence_tracks_mask[0]) * MAX_SEQUENCE_TRACKS_MASKS);
			if (sequence_tracks_mask == NULL) {
				ngx_log_debug0(
					NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "ngx_http_vod_parse_uri_file_name: ngx_palloc failed"
				);
			...
			if (tracks_mask_updated) {
				...
				sequence_tracks_mask->index = sequence_index;
				ngx_memcpy(
					sequence_tracks_mask->tracks_mask,
					result->tracks_mask,
					sizeof(sequence_tracks_mask->tracks_mask)
				);
				sequence_tracks_mask++;
				result->sequence_tracks_mask_end = sequence_tracks_mask;
	...
	if (*start_pos == 'p') {
		...
		if (pts_delay <= 0) {
			ngx_log_error(
				NGX_LOG_ERR, r->connection->log, 0, "ngx_http_vod_parse_uri_file_name: failed to parse pts delay"
			);
			return ngx_http_vod_status_to_ngx_error(r, VOD_BAD_REQUEST);
		}
		...
		for (;;) {
			...
			if (lang_id == 0) {
				ngx_log_error(
					...
					r->connection->log,
					0,
					"ngx_http_vod_parse_uri_file_name: failed to parse language specifier %*s",
					(size_t)3,
					start_pos
				);
				return ngx_http_vod_status_to_ngx_error(r, VOD_BAD_REQUEST);
	...
	if (*start_pos == 'x') {
		...
		if (version <= 0) {
			ngx_log_error(
				NGX_LOG_ERR, r->connection->log, 0, "ngx_http_vod_parse_uri_file_name: failed to parse version"
			);
			return ngx_http_vod_status_to_ngx_error(r, VOD_BAD_REQUEST);
		}
	...
	ngx_log_error(
		NGX_LOG_ERR, r->connection->log, 0, "ngx_http_vod_parse_uri_file_name: did not consume the whole name"
	);
	return ngx_http_vod_status_to_ngx_error(r, VOD_BAD_REQUEST);
}
...
ngx_http_vod_parse_multi_uri(
	ngx_http_request_t* r, ngx_str_t* uri, ngx_str_t* multi_uri_suffix, ngx_http_vod_multi_uri_t* result
) {
	u_char* cur_pos;
	u_char* end_pos;
	...
	if (uri->len < multi_uri_suffix->len
	    || ngx_memcmp(
			   multi_uri_suffix->data,
			   uri->data + uri->len - multi_uri_suffix->len,
	...
	for (cur_pos = uri->data; cur_pos < end_pos; cur_pos++) {
		...
		if (last_comma_pos == NULL) {
			...
			if (part_index >= MAX_SUB_URIS) {
				ngx_log_error(
					NGX_LOG_ERR, r->connection->log, 0, "ngx_http_vod_parse_multi_uri: number of url parts exceeds the limit"
				);
				return ngx_http_vod_status_to_ngx_error(r, VOD_BAD_REQUEST);
			}
...
static ngx_http_vod_uri_param_def_t uri_param_defs[] = {
	{offsetof(ngx_http_vod_loc_conf_t, clip_to_param_name),
     "clip to",
     ngx_http_vod_parse_uint64_param,
     offsetof(media_clip_source_t, clip_to)},
	{offsetof(ngx_http_vod_loc_conf_t, clip_from_param_name),
     "clip from",
     ngx_http_vod_parse_uint64_param,
     offsetof(media_clip_source_t, clip_from)},
	...
	{offsetof(ngx_http_vod_loc_conf_t, time_shift_param_name),
     "time shift",
     ngx_http_vod_parse_time_shift_param,
...
ngx_http_vod_init_uri_params_hash(ngx_conf_t* cf, ngx_http_vod_loc_conf_t* conf) {
	...
	rc = ngx_http_vod_init_hash(cf, uri_param_defs, conf, "uri_params_hash", &conf->uri_params_hash);
	if (rc != NGX_OK) {
	...
	rc = ngx_http_vod_init_hash(
		cf, pd_uri_param_defs, conf, "pd_uri_params_hash", &conf->pd_uri_params_hash
	);
	if (rc != NGX_OK) {
		ngx_conf_log_error(
			NGX_LOG_EMERG, cf, 0, "failed to initialize progressive download uri params hash"
		);
		return rc;
	}
...
ngx_http_vod_extract_uri_params(
	...
	*result = &source_clip->base;
	...
	if (p == NULL) {
		ngx_log_debug0(
			NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "ngx_http_vod_extract_uri_params: ngx_palloc failed (1)"
		);
		return ngx_http_vod_status_to_ngx_error(r, VOD_ALLOC_FAILED);
	}
	...
	for (cur_pos = uri->data; cur_pos <= end_pos; cur_pos++) {
		...
		if (param_def == NULL) {
			param_def =
				ngx_hash_find(params_hash, cur_key_hash, param_name, param_name_pos - param_name);
			if (param_def != NULL) {
			...
			// first instance of a param takes priority
			if ((parsed_params_mask & (1 << param_index)) == 0) {
				...
				if (param_def->name_conf_offset == offsetof(ngx_http_vod_loc_conf_t, speed_param_name)) {
					...
					rc = rate_filter_create_from_string(
						&request_context, &cur_param, &source_clip->base, &rate_filter
					);
					if (rc != VOD_OK) {
					...
					if (rc != NGX_OK) {
						ngx_log_error(
							...
							r->connection->log,
							0,
							"ngx_http_vod_extract_uri_params: %s parser failed %i",
							param_def->name,
							rc
						);
						return rc;
	...
	if (source_clip->clip_from >= source_clip->clip_to) {
		ngx_log_error(
			...
			r->connection->log,
			0,
			"ngx_http_vod_extract_uri_params: clip from %uL is larger than clip to %uL",
			source_clip->clip_from,
			source_clip->clip_to
		);
		return ngx_http_vod_status_to_ngx_error(r, VOD_BAD_REQUEST);
	}
...
ngx_http_vod_parse_uri_path(
	...
	// must save the uri before calling ngx_http_vod_parse_multi_uri as it may change it
	media_set->uri = *uri;
	...
	if (rc != NGX_OK) {
		ngx_log_debug1(
			...
			r->connection->log,
			0,
			"ngx_http_vod_parse_uri_path: ngx_http_vod_parse_multi_uri failed %i",
			rc
		);
		return rc;
	}

	if (multi_uri.parts_count > 1 && request_params->sequence_ids[0].len == 0) {
	...
	cur_sequence = ngx_palloc(
		r->pool, (sizeof(*cur_sequence) + sizeof(*cur_source) + sizeof(*cur_clip_ptr)) * uri_count
	);
	if (cur_sequence == NULL) {
		ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "ngx_http_vod_parse_uri_path: ngx_palloc failed");
		return ngx_http_vod_status_to_ngx_error(r, VOD_ALLOC_FAILED);
	}
	...
	for (i = 0; i < multi_uri.parts_count; i++) {
		...
		if (rc != NGX_OK) {
			ngx_log_debug1(
				...
				r->connection->log,
				0,
				"ngx_http_vod_parse_uri_path: ngx_http_vod_merge_string_parts failed %i",
				rc
			);
			return rc;
		}

		rc = ngx_http_vod_extract_uri_params(
			r, params_hash, &cur_uri, cur_sequence, &clip_id, cur_source, &cur_clip
		);
		if (rc != NGX_OK) {
			ngx_log_debug1(
				NGX_LOG_DEBUG_HTTP,
				r->connection->log,
				0,
				"ngx_http_vod_parse_uri_path: ngx_http_vod_extract_uri_params failed %i",
				rc
			);
			return rc;
		}
		...
		for (media_type = 0; media_type < MEDIA_TYPE_COUNT; media_type++) {
			vod_track_mask_and_bits(
				track_mask_temp, cur_source->tracks_mask[media_type], request_params->tracks_mask[media_type]
			);
	...
	if (media_set->sequence_count <= 0) {
		ngx_log_error(
			NGX_LOG_ERR, r->connection->log, 0, "ngx_http_vod_parse_uri_path: request has no uris after track filtering"
		);
		return ngx_http_vod_status_to_ngx_error(r, VOD_BAD_REQUEST);
	}
