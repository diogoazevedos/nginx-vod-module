...
thumb_grabber_process_init(vod_log_t* log) {
	...
	if (encoder_codec == NULL) {
		vod_log_error(
			VOD_LOG_WARN, log, 0, "thumb_grabber_process_init: failed to get jpeg encoder, thumbnail capture is disabled"
		);
		return;
	}
...
thumb_grabber_init_decoder(
	request_context_t* request_context, media_info_t* media_info, AVCodecContext** result
) {
	AVCodecContext* decoder;
	int avrc;
	...
	if (decoder == NULL) {
		vod_log_error(
			VOD_LOG_ERR, request_context->log, 0, "thumb_grabber_init_decoder: avcodec_alloc_context3 failed"
		);
	...
	*result = decoder;
	...
	if (avrc < 0) {
		vod_log_error(
			VOD_LOG_ERR, request_context->log, 0, "thumb_grabber_init_decoder: avcodec_open2 failed %d", avrc
		);
...
thumb_grabber_init_encoder(
	request_context_t* request_context, uint32_t width, uint32_t height, AVCodecContext** result
) {
	AVCodecContext* encoder;
	int avrc;
	...
	if (encoder == NULL) {
		vod_log_error(
			VOD_LOG_ERR, request_context->log, 0, "thumb_grabber_init_encoder: avcodec_alloc_context3 failed"
		);
		return VOD_ALLOC_FAILED;
	}
	...
	*result = encoder;
	...
	if (avrc < 0) {
		vod_log_error(
			VOD_LOG_ERR, request_context->log, 0, "thumb_grabber_init_encoder: avcodec_open2 failed %d", avrc
		);
		return VOD_UNEXPECTED;
	}
...
thumb_grabber_truncate_frames(
	...
	if (track->frame_count <= 0) {
		vod_log_error(
			VOD_LOG_ERR, request_context->log, 0, "thumb_grabber_truncate_frames: did not find any frames (1)"
		);
	...
	for (index = 0;; cur_frame++, index++) {
		...
		if (cur_diff <= min_diff && (cur_frame->key_frame || (accurate && last_key_frame != NULL))) {
			min_index = index - last_key_frame_index;
			min_diff = cur_diff;
			min_part = last_key_frame_part;

			rc = min_part->frames_source->skip_frames(
				min_part->frames_source_context, last_key_frame - min_part->first_frame
			);
			if (rc != VOD_OK) {
				return rc;
			}
	...
	if (min_part == NULL) {
		vod_log_error(
			VOD_LOG_ERR, request_context->log, 0, "thumb_grabber_truncate_frames: did not find any frames (2)"
		);
		return VOD_UNEXPECTED;
	}
...
thumb_grabber_init_state(
	...
	if (decoder_codec[track->media_info.codec_id] == NULL) {
		vod_log_debug1(
			...
			request_context->log,
			0,
			"thumb_grabber_init_state: no decoder was initialized for codec %uD",
	...
	if (track->media_info.u.video.width <= 0 || track->media_info.u.video.height <= 0) {
		vod_log_error(
			VOD_LOG_ERR, request_context->log, 0, "thumb_grabber_init_state: input width/height is zero"
		);
		return VOD_BAD_DATA;
	}

	rc = thumb_grabber_truncate_frames(
		request_context, track, request_params->segment_time, accurate, &frame_index
	);
	if (rc != VOD_OK) {
		return rc;
	}
	...
	vod_log_debug1(
		VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "thumb_grabber_init_state: frame index is %uD", frame_index
	);

	state = vod_alloc(request_context->pool, sizeof(*state));
	if (state == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "thumb_grabber_init_state: vod_alloc failed (1)"
		);
		return VOD_ALLOC_FAILED;
	}
	...
	if (cln == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "thumb_grabber_init_state: vod_pool_cleanup_add failed"
		);
		return VOD_ALLOC_FAILED;
	}
	...
	if (state->decoded_frame == NULL) {
		vod_log_error(
			VOD_LOG_ERR, request_context->log, 0, "thumb_grabber_init_state: av_frame_alloc failed"
		);
		return VOD_ALLOC_FAILED;
	}
	...
	if (state->output_packet == NULL) {
		vod_log_error(
			VOD_LOG_ERR, request_context->log, 0, "thumb_grabber_init_state: av_packet_alloc failed"
		);
		return VOD_ALLOC_FAILED;
	}
...
thumb_grabber_decode_flush(thumb_grabber_state_t* state) {
	...
	if (avrc < 0) {
		vod_log_error(
			...
			0,
			"thumb_grabber_decode_flush: avcodec_send_packet failed %d",
			avrc
		);
		return VOD_BAD_DATA;
	}
	...
	for (; state->missing_frames > 0; state->missing_frames--) {
		...
		if (decoded_frame == NULL) {
			vod_log_error(
				VOD_LOG_ERR, state->request_context->log, 0, "thumb_grabber_decode_flush: av_frame_alloc failed"
			);
			return VOD_ALLOC_FAILED;
		}
		...
		if (avrc < 0) {
			...
			vod_log_error(
				...
				state->request_context->log,
				0,
				"thumb_grabber_decode_flush: avcodec_decode_video2 failed %d",
				avrc
			);
			return VOD_BAD_DATA;
		}
...
thumb_grabber_decode_frame(thumb_grabber_state_t* state, u_char* buffer) {
	...
	if (input_packet == NULL) {
		vod_log_error(
			VOD_LOG_ERR, state->request_context->log, 0, "thumb_grabber_decode_frame: av_packet_alloc failed"
		);
		return VOD_ALLOC_FAILED;
	}
	...
	if (avrc < 0) {
		vod_log_error(
			...
			state->request_context->log,
			0,
			"thumb_grabber_decode_frame: avcodec_send_packet failed %d",
			avrc
		);
		return VOD_BAD_DATA;
	}
	...
	if (avrc == AVERROR(EAGAIN)) {
		...
		vod_log_error(
			...
			state->request_context->log,
			0,
			"thumb_grabber_decode_frame: avcodec_receive_frame failed %d",
			avrc
		);
		return VOD_BAD_DATA;
	} else {
...
thumb_grabber_resize_frame(thumb_grabber_state_t* state) {
	...
	if (output_frame == NULL) {
		vod_log_error(
			VOD_LOG_ERR, state->request_context->log, 0, "thumb_grabber_resize_frame: av_frame_alloc failed"
		);
		rc = VOD_ALLOC_FAILED;
		goto end;
	}
	...
	if (avrc < 0) {
		vod_log_error(
			VOD_LOG_ERR, state->request_context->log, 0, "thumb_grabber_resize_frame: av_image_alloc failed"
		);
		rc = VOD_ALLOC_FAILED;
		goto end;
	}
...
thumb_grabber_write_frame(thumb_grabber_state_t* state) {
	...
	if (avrc < 0) {
		vod_log_error(
			...
			state->request_context->log,
			0,
			"thumb_grabber_write_frame: avcodec_send_frame failed %d",
			avrc
		);
		return VOD_UNEXPECTED;
	}
	...
	if (avrc < 0) {
		vod_log_error(
			...
			state->request_context->log,
			0,
			"thumb_grabber_write_frame: avcodec_receive_packet failed %d",
			avrc
		);
		return VOD_UNEXPECTED;
	}

	rc = state->write_callback(
		state->write_context, state->output_packet->data, state->output_packet->size
	);
	if (rc != VOD_OK) {
		return rc;
	}
		...
		if (!frame_done) {
			...
			if (state->frame_buffer == NULL) {
				state->frame_buffer = vod_alloc(
					state->request_context->pool, state->max_frame_size + VOD_BUFFER_PADDING_SIZE
				);
				if (state->frame_buffer == NULL) {
					vod_log_debug0(
						VOD_LOG_DEBUG_LEVEL, state->request_context->log, 0, "thumb_grabber_process: vod_alloc failed"
					);
					return VOD_ALLOC_FAILED;
				}
			}
