...
gain_filter_append_desc(u_char* p, media_clip_t* clip) {
	...
	return vod_sprintf(p, GAIN_FILTER_DESC_PATTERN, clip->sources[0]->id, num / 100, num % 100, clip->id);
}
...
vod_status_t
gain_filter_parse(void* ctx, vod_json_object_t* element, void** result) {
	...
	vod_log_debug0(VOD_LOG_DEBUG_LEVEL, context->request_context->log, 0, "gain_filter_parse: started");

	vod_memzero(params, sizeof(params));

	vod_json_get_object_values(element, &gain_filter_hash, params);
	...
	if (filter == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, context->request_context->log, 0, "gain_filter_parse: vod_alloc failed (1)"
		);
	...
	*result = &filter->base;
	...
	vod_log_debug2(
		...
		context->request_context->log,
		0,
		"gain_filter_parse: done, gain=%uD/%uD",
		filter->gain.num,
		filter->gain.denom
	);
...
vod_status_t
gain_filter_parser_init(vod_pool_t* pool, vod_pool_t* temp_pool) {
	vod_status_t rc;

	rc = vod_json_init_hash(
		pool, temp_pool, "gain_filter_hash", gain_filter_params, sizeof(gain_filter_params[0]), &gain_filter_hash
	);
	if (rc != VOD_OK) {
		return rc;
	}
