...
vod_status_t
dynamic_clip_parse(void* ctx, vod_json_object_t* element, void** result) {
	media_filter_parse_context_t* context = ctx;
	media_clip_dynamic_t* filter;
	vod_status_t rc;

	vod_log_debug0(VOD_LOG_DEBUG_LEVEL, context->request_context->log, 0, "dynamic_clip_parse: started");

	filter = vod_alloc(context->request_context->pool, sizeof(*filter));
	if (filter == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, context->request_context->log, 0, "dynamic_clip_parse: vod_alloc failed"
		);
	...
	filter->id.len = 0;
	rc = vod_json_parse_object_values(element, &dynamic_clip_hash, context, filter);
	if (rc != VOD_OK) {
		return rc;
	}
	...
	if (filter->id.len == 0) {
		vod_log_error(
			VOD_LOG_ERR, context->request_context->log, 0, "dynamic_clip_parse: \"id\" is mandatory for dynamic filter"
		);
	...
	*result = &filter->base;
	...
	vod_log_debug0(VOD_LOG_DEBUG_LEVEL, context->request_context->log, 0, "dynamic_clip_parse: done");

	return VOD_OK;
}

vod_status_t
dynamic_clip_parser_init(vod_pool_t* pool, vod_pool_t* temp_pool) {
	vod_status_t rc;

	rc = vod_json_init_hash(
		pool, temp_pool, "dynamic_clip_hash", dynamic_clip_params, sizeof(dynamic_clip_params[0]), &dynamic_clip_hash
	);
	if (rc != VOD_OK) {
		return rc;
	}
...
dynamic_clip_apply_mapping_json(
	...
	if (rc != VOD_JSON_OK) {
		vod_log_error(
			...
			request_context->log,
			0,
			"dynamic_clip_apply_mapping_json: failed to parse json %i: %s",
			rc,
			error
		);
		return VOD_BAD_MAPPING;
	...
	if (json.type != VOD_JSON_OBJECT) {
		vod_log_error(
			...
			request_context->log,
			0,
			"dynamic_clip_apply_mapping_json: invalid root element type %d expected object",
			json.type
		);
		return VOD_BAD_MAPPING;
	}
	...
	if (rc != VOD_OK) {
		vod_log_debug1(
			...
			request_context->log,
			0,
			"dynamic_clip_apply_mapping_json: concat_clip_parse failed %i",
			rc
		);
		return rc;
	}
	...
	if (concat_clip->type == MEDIA_CLIP_SOURCE) {
		...
		if (clip->base.sources == NULL) {
			vod_log_debug0(
				VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "dynamic_clip_apply_mapping_json: vod_alloc failed"
			);
			return VOD_ALLOC_FAILED;
		}
...
dynamic_clip_get_mapping_string(
	...
	if (p == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "dynamic_clip_get_mapping_string: vod_alloc failed"
		);
		return VOD_ALLOC_FAILED;
	}
	result->data = p;
	...
	for (cur_clip = dynamic_clips_head; cur_clip != NULL; cur_clip = cur_clip->next) {
		...
		for (i = 0; i < cur_clip->base.source_count; i++) {
			cur_source = (media_clip_source_t*)cur_clip->base.sources[i];
			p = vod_sprintf(p, "-%V-%uL", &cur_source->mapped_uri, cur_source->clip_to);
		}
	}
...
static vod_status_t
dynamic_clip_extract_token(request_context_t* request_context, u_char** cur, u_char* end, vod_str_t* result) {
	u_char* p = *cur;
	...
	if (p == NULL) {
		vod_log_error(
			VOD_LOG_ERR, request_context->log, 0, "dynamic_clip_extract_token: delimiter (-) not found"
		);
	...
	if (result->len <= 0) {
		vod_log_error(
			VOD_LOG_ERR, request_context->log, 0, "dynamic_clip_extract_token: zero length token"
		);
		return VOD_BAD_REQUEST;
	}
...
dynamic_clip_apply_mapping_string_clip(
	...
	range = vod_alloc(
		...
		(sizeof(range[0]) + sizeof(cur_source[0]) + sizeof(cur_source_ptr[0])) * source_count
	);
	if (range == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "dynamic_clip_apply_mapping_string_clip: vod_alloc failed"
		);
		return VOD_ALLOC_FAILED;
	}
	...
	for (i = 0; i < source_count; i++) {
		...
		vod_log_debug1(
			...
			request_context->log,
			0,
			"dynamic_clip_apply_mapping_string_clip: parsed clip source - clipId=%V",
			&clip_id
		);
		...
		for (prev = &media_set->dynamic_clips_head;; prev = &cur->next) {
			...
			if (cur == NULL) {
				vod_log_error(
					...
					request_context->log,
					0,
					"dynamic_clip_apply_mapping_string: dynamic clip \"%V\" not found",
					&clip_id
				);
				return VOD_BAD_REQUEST;
			}

			if (cur->id.len == clip_id.len && vod_memcmp(cur->id.data, clip_id.data, clip_id.len) == 0) {
				break;
			}
		}
