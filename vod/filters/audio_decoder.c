...
audio_decoder_process_init(vod_log_t* log) {
	...
	if (decoder_codec == NULL) {
		vod_log_error(
			VOD_LOG_WARN, log, 0, "audio_decoder_process_init: failed to get AAC decoder, audio decoding is disabled"
		);
		return;
	}
...
static vod_status_t
audio_decoder_init_decoder(audio_decoder_state_t* state, media_info_t* media_info) {
	...
	if (media_info->codec_id != VOD_CODEC_ID_AAC) {
		vod_log_error(
			...
			0,
			"audio_decoder_init_decoder: codec id %uD not supported",
			media_info->codec_id
		);
	...
	if (decoder == NULL) {
		vod_log_error(
			VOD_LOG_ERR, state->request_context->log, 0, "audio_decoder_init_decoder: avcodec_alloc_context3 failed"
		);
	...
	if (avrc < 0) {
		vod_log_error(
			...
			state->request_context->log,
			0,
			"audio_decoder_init_decoder: avcodec_open2 failed %d",
			avrc
		);
...
audio_decoder_init(
	...
	if (!initialized) {
		vod_log_error(
			VOD_LOG_ERR, request_context->log, 0, "audio_decoder_init: module failed to initialize successfully"
		);
		return VOD_UNEXPECTED;
	}
	...
	// init the decoder
	rc = audio_decoder_init_decoder(state, &track->media_info);
	if (rc != VOD_OK) {
		return rc;
	}
	...
	if (state->decoded_frame == NULL) {
		vod_log_error(VOD_LOG_ERR, request_context->log, 0, "audio_decoder_init: av_frame_alloc failed");
		return VOD_ALLOC_FAILED;
	}
	...
	state->cur_frame_part.frames_source->set_cache_slot_id(
		state->cur_frame_part.frames_source_context, cache_slot_id
	);
...
static vod_status_t
audio_decoder_decode_frame(audio_decoder_state_t* state, u_char* buffer, AVFrame** result) {
	...
	if (input_packet == NULL) {
		vod_log_error(
			VOD_LOG_ERR, state->request_context->log, 0, "audio_decoder_decode_frame: av_packet_alloc failed"
		);
		return VOD_ALLOC_FAILED;
	}
	...
	if (avrc < 0) {
		vod_log_error(
			...
			state->request_context->log,
			0,
			"audio_decoder_decode_frame: avcodec_send_packet failed %d",
			avrc
		);
		return VOD_BAD_DATA;
	}
	...
	if (avrc < 0) {
		vod_log_error(
			...
			state->request_context->log,
			0,
			"audio_decoder_decode_frame: avcodec_receive_frame failed %d",
			avrc
		);
		return VOD_BAD_DATA;
	}
...
vod_status_t
audio_decoder_get_frame(audio_decoder_state_t* state, AVFrame** result) {
	u_char* read_buffer;
	uint32_t read_size;
	vod_status_t rc;
	bool_t frame_done;
	...
	for (;;) {
		...
		if (!frame_done) {
			...
			if (state->frame_buffer == NULL) {
				...
				if (state->frame_buffer == NULL) {
					vod_log_debug0(
						VOD_LOG_DEBUG_LEVEL, state->request_context->log, 0, "audio_decoder_get_frame: vod_alloc failed"
					);
					return VOD_ALLOC_FAILED;
				}
			}
