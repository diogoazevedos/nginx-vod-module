...
filter_copy_track_to_clip(filters_init_state_t* state, media_track_t* track) {
	...
	*output_track = *track;
	...
	if (ref_track[media_type] == NULL) {
		...
		case MDP_MAX:
			if (output_track->media_info.duration_millis
			    > ref_track[media_type]->media_info.duration_millis) {
				ref_track[media_type] = output_track;
			}
			break;
		...
		case MDP_MIN:
			if (output_track->media_info.duration_millis > 0
			    && (ref_track[media_type]->media_info.duration_millis == 0
			        || output_track->media_info.duration_millis
			               < ref_track[media_type]->media_info.duration_millis)) {
				ref_track[media_type] = output_track;
			}
			break;
...
static void
filter_init_filtered_clip_from_source(filters_init_state_t* state, media_clip_source_t* source) {
	uint32_t media_type;
	media_track_t* cur_track;
...
static vod_status_t
filter_scale_video_tracks(
	filters_init_state_t* state, media_clip_t* clip, uint32_t speed_num, uint32_t speed_denom
) {
...
filter_validate_consistent_codecs(
	request_context_t* request_context, media_clip_filtered_t* first_clip, media_clip_filtered_t* cur_clip
) {
	...
	for (first_clip_track = first_clip->first_track, cur_clip_track = cur_clip->first_track;
		...
		if (first_clip_track->media_info.codec_id != cur_clip_track->media_info.codec_id) {
			vod_log_error(
				...
				cur_clip_track->media_info.codec_id,
				first_clip_track->media_info.codec_id
			);
...
vod_status_t
filter_init_filtered_clips(request_context_t* request_context, media_set_t* media_set, bool_t parsed_frames) {
	...
	output_clip = vod_alloc(
		request_context->pool, sizeof(output_clip[0]) * media_set->sequence_count * media_set->clip_count
	);
	if (output_clip == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "filter_init_filtered_clips: vod_alloc failed (1)"
		);
	...
	for (sequence = media_set->sequences; sequence < media_set->sequences_end; sequence++) {
		...
		for (cur_clip = sequence->clips; cur_clip < clips_end; cur_clip++) {
			...
			if (!media_clip_is_source(cur_clip[0]->type) && track_count[MEDIA_TYPE_AUDIO] > 1) {
				// audio filtering supports only a single output track
				track_count[MEDIA_TYPE_AUDIO] = 1;
			}
			...
			for (media_type = 0; media_type < MEDIA_TYPE_COUNT; media_type++) {
				if (sequence->track_count[media_type] != track_count[media_type]) {
					vod_log_error(
						...
						sequence->track_count[media_type],
						track_count[media_type],
						media_type
					);
	...
	if (init_state.output_track == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "filter_init_filtered_clips: vod_alloc failed (2)"
		);
		return VOD_ALLOC_FAILED;
	}
	...
	for (clip_index = 0; clip_index < media_set->clip_count; clip_index++) {
		for (sequence = media_set->sequences; sequence < media_set->sequences_end; sequence++) {
			...
			if (init_state.audio_reference_track != NULL) {
				...
				if (init_state.audio_reference_track_speed_num
			...
			if (clip_index > 0) {
				rc = filter_validate_consistent_codecs(
					request_context, sequence->filtered_clips, output_clip
				);
				if (rc != VOD_OK) {
					return rc;
				}
	...
	if (media_set->timing.durations == NULL) {
		media_set->timing.total_duration = segmenter_get_total_duration(
			media_set->segmenter_conf, media_set, media_set->sequences, media_set->sequences_end, MEDIA_TYPE_NONE
		);
	}
...
filter_init_state(
	...
	if (state == NULL) {
		vod_log_debug0(VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "filter_init_state: vod_alloc failed");
		return VOD_ALLOC_FAILED;
	}
