...
void
rate_filter_scale_track_timestamps(media_track_t* track, uint32_t speed_num, uint32_t speed_denom) {
	...
	track->media_info.duration_millis =
	...
	track->media_info.bitrate = (uint32_t)((track->total_frames_size * track->media_info.timescale * 8)
...
rate_filter_append_desc(u_char* p, media_clip_t* clip) {
	...
	return vod_sprintf(p, RATE_FILTER_DESC_PATTERN, clip->sources[0]->id, num / 100, num % 100, clip->id);
}
...
vod_status_t
rate_filter_parse(void* ctx, vod_json_object_t* element, void** result) {
	...
	vod_log_debug0(VOD_LOG_DEBUG_LEVEL, context->request_context->log, 0, "rate_filter_parse: started");

	vod_memzero(params, sizeof(params));

	vod_json_get_object_values(element, &rate_filter_hash, params);
	...
	if (rate->v.num.denom > 100) {
		vod_log_error(
			VOD_LOG_ERR, context->request_context->log, 0, "rate_filter_parse: invalid rate, only 2 decimal points are allowed"
		);
		return VOD_BAD_MAPPING;
	}

	if (rate->v.num.num < 0
	    || rate->v.num.denom > (uint64_t)rate->v.num.num * 2
	    || (uint64_t)rate->v.num.num > rate->v.num.denom * 2) {
		vod_log_error(
			VOD_LOG_ERR,
			context->request_context->log,
			0,
			"rate_filter_parse: invalid rate %L/%uL, must be between 0.5 and 2",
			rate->v.num.num,
			rate->v.num.denom
		);
		return VOD_BAD_MAPPING;
	...
	filter =
		vod_alloc(context->request_context->pool, sizeof(*filter) + sizeof(filter->base.sources[0]));
	if (filter == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, context->request_context->log, 0, "rate_filter_parse: vod_alloc failed (1)"
		);
	...
	if (old_range != NULL) {
		...
		if (new_range == NULL) {
			vod_log_debug0(
				VOD_LOG_DEBUG_LEVEL, context->request_context->log, 0, "rate_filter_parse: vod_alloc failed (2)"
			);
			return VOD_ALLOC_FAILED;
		}
	...
	rc = media_set_parse_clip(context, &source->v.obj, &filter->base, &filter->base.sources[0]);
	if (rc != VOD_JSON_OK) {
		return rc;
	}
	...
	*result = &filter->base;
	...
	vod_log_debug2(
		...
		context->request_context->log,
		0,
		"rate_filter_parse: done, rate=%uD/%uD",
		filter->rate.num,
		filter->rate.denom
	);
...
rate_filter_create_from_string(
	request_context_t* request_context, vod_str_t* str, media_clip_t* source, media_clip_rate_filter_t** result
) {
	media_clip_rate_filter_t* filter;
	vod_int_t num;
	...
	if (num < 0) {
		vod_log_error(
			...
			request_context->log,
			0,
			"rate_filter_create_from_string: failed to parse playback rate \"%V\", expecting a float with up to 2 digits precision",
			str
		);
	...
	if (num < 50 || num > 200) {
		vod_log_error(
			...
			request_context->log,
			0,
			"rate_filter_create_from_string: invalid playback rate value %i/100, must be between 0.5 and 2",
			num
		);
		return VOD_BAD_REQUEST;
	}
	...
	if (filter == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "rate_filter_create_from_string: vod_alloc failed"
		);
		return VOD_ALLOC_FAILED;
	}
...
vod_status_t
rate_filter_parser_init(vod_pool_t* pool, vod_pool_t* temp_pool) {
	vod_status_t rc;

	rc = vod_json_init_hash(
		pool, temp_pool, "rate_filter_hash", rate_filter_params, sizeof(rate_filter_params[0]), &rate_filter_hash
	);
	if (rc != VOD_OK) {
		return rc;
	}
