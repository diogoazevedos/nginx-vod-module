...
vod_status_t
concat_clip_parse(void* ctx, vod_json_object_t* element, void** result) {
	...
	vod_log_debug0(VOD_LOG_DEBUG_LEVEL, context->request_context->log, 0, "concat_clip_parse: started");

	// get the required fields
	vod_memzero(params, sizeof(params));
	vod_json_get_object_values(element, &concat_clip_hash, params);
	...
	if (params[CONCAT_PARAM_DURATIONS] == NULL) {
		vod_log_error(
			VOD_LOG_ERR, context->request_context->log, 0, "concat_clip_parse: \"durations\" is mandatory for concat"
		);
		return VOD_BAD_MAPPING;
	}
	...
	if (params[CONCAT_PARAM_TRACKS] != NULL) {
		...
		if (parse_utils_extract_track_tokens(src_str->data, end_pos, tracks_mask) != end_pos) {
			vod_log_error(
				VOD_LOG_ERR, context->request_context->log, 0, "concat_clip_parse: failed to parse tracks specification"
			);
			return VOD_BAD_MAPPING;
		}
	} else {
	...
	if (context->range == NULL) {
		...
		if (paths->count <= 0) {
			vod_log_error(
				VOD_LOG_ERR, context->request_context->log, 0, "concat_clip_parse: \"paths\" array is empty"
			);
			return VOD_BAD_MAPPING;
		}
		...
		if (sources == NULL) {
			vod_log_debug0(
				VOD_LOG_DEBUG_LEVEL, context->request_context->log, 0, "concat_clip_parse: vod_alloc failed (1)"
			);
		...
		if (params[CONCAT_PARAM_OFFSET] != NULL) {
			if (params[CONCAT_PARAM_OFFSET]->v.num.num < INT_MIN) {
				vod_log_error(
					...
					context->request_context->log,
					0,
					"concat_clip_parse: offset %L too small",
					params[CONCAT_PARAM_OFFSET]->v.num.num
				);
				return VOD_BAD_MAPPING;
			}
		...
		part = &durations->part;
		for (i = 0, cur_duration = part->first;; i++, cur_duration++, offset = next_offset) {
			...
			if (cur_duration_value < 0) {
				vod_log_error(
					VOD_LOG_ERR, context->request_context->log, 0, "concat_clip_parse: negative duration value"
				);
				return VOD_BAD_MAPPING;
			}
		...
		sources = vod_alloc(
			context->request_context->pool, (sizeof(sources[0]) + sizeof(range[0])) * clip_count
		);
		if (sources == NULL) {
			vod_log_debug0(
				VOD_LOG_DEBUG_LEVEL, context->request_context->log, 0, "concat_clip_parse: vod_alloc failed (2)"
			);
			return VOD_ALLOC_FAILED;
		}
		...
		if ((int64_t)end <= offset) {
			...
			// NOTE: there will always be a single clip in this case because if offset >= end, then
			// next_offset >= end, and the for loop above will stop after the first iteration
	...
	if (params[CONCAT_PARAM_BASE_PATH] != NULL) {
		base_path.data =
			vod_alloc(context->request_context->pool, params[CONCAT_PARAM_BASE_PATH]->v.str.len);
		if (base_path.data == NULL) {
			vod_log_debug0(
				VOD_LOG_DEBUG_LEVEL, context->request_context->log, 0, "concat_clip_parse: vod_alloc failed (3)"
			);
			return VOD_ALLOC_FAILED;
		}
		...
		if (rc != VOD_JSON_OK) {
			vod_log_error(
				...
				context->request_context->log,
				0,
				"concat_clip_parse: vod_json_decode_string failed %i",
				rc
			);
			return VOD_BAD_MAPPING;
		}
	...
	for (;;) {
		...
		if (dest_str.data == NULL) {
			vod_log_debug0(
				VOD_LOG_DEBUG_LEVEL, context->request_context->log, 0, "concat_clip_parse: vod_alloc failed (4)"
			);
			return VOD_ALLOC_FAILED;
		}
	...
	clip =
		vod_alloc(context->request_context->pool, sizeof(*clip) + sizeof(media_clip_t*) * clip_count);
	if (clip == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, context->request_context->log, 0, "concat_clip_parse: vod_alloc failed (5)"
		);
		return VOD_ALLOC_FAILED;
	}
	...
	*result = &clip->base;
	...
	vod_log_debug0(VOD_LOG_DEBUG_LEVEL, context->request_context->log, 0, "concat_clip_parse: done");
	return VOD_OK;
}

vod_status_t
concat_clip_concat(request_context_t* request_context, media_clip_t* clip) {
	...
	for (cur_source = clip->sources + clip->source_count - 2; cur_source >= clip->sources;
		...
		for (media_type = 0; media_type < MEDIA_TYPE_COUNT; media_type++) {
			if (src_clip->track_array.track_count[media_type]
...
vod_status_t
concat_clip_parser_init(vod_pool_t* pool, vod_pool_t* temp_pool) {
	vod_status_t rc;

	rc = vod_json_init_hash(
		pool, temp_pool, "concat_clip_hash", concat_clip_params, sizeof(concat_clip_params[0]), &concat_clip_hash
	);
	if (rc != VOD_OK) {
		return rc;
	}
