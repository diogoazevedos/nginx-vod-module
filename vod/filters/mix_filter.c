...
mix_filter_get_desc_size(media_clip_t* clip) {
	return sizeof(MIX_FILTER_DESC_PATTERN)
	     + VOD_INT32_LEN * 2
...
mix_filter_append_desc(u_char* p, media_clip_t* clip) {
	...
	for (sources_cur = clip->sources; sources_cur < sources_end; sources_cur++) {
		...
		p = vod_sprintf(p, INPUT_LINK_PATTERN, (*sources_cur)->id);

		source_count++;
	}

	return vod_sprintf(p, MIX_FILTER_DESC_PATTERN, source_count, clip->id);
}
...
vod_status_t
mix_filter_parse(void* ctx, vod_json_object_t* element, void** result) {
	media_filter_parse_context_t* context = ctx;
	media_clip_mix_filter_t* filter;
	vod_status_t rc;

	vod_log_debug0(VOD_LOG_DEBUG_LEVEL, context->request_context->log, 0, "mix_filter_parse: started");

	filter = vod_alloc(context->request_context->pool, sizeof(*filter));
	if (filter == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, context->request_context->log, 0, "mix_filter_parse: vod_alloc failed"
		);
	...
	rc = vod_json_parse_object_values(element, &mix_filter_hash, context, filter);
	if (rc != VOD_OK) {
		return rc;
	}
	...
	if (filter->base.source_count == 0) {
		vod_log_error(
			VOD_LOG_ERR, context->request_context->log, 0, "mix_filter_parse: \"sources\" is mandatory for mix filter"
		);
	...
	*result = &filter->base;
	...
	vod_log_debug0(VOD_LOG_DEBUG_LEVEL, context->request_context->log, 0, "mix_filter_parse: done");

	return VOD_OK;
}

vod_status_t
mix_filter_parser_init(vod_pool_t* pool, vod_pool_t* temp_pool) {
	vod_status_t rc;

	rc = vod_json_init_hash(
		pool, temp_pool, "mix_filter_hash", mix_filter_params, sizeof(mix_filter_params[0]), &mix_filter_hash
	);
	if (rc != VOD_OK) {
		return rc;
	}
