...
hevc_parser_sub_layer_hrd_parameters(
	bit_reader_state_t* reader, uint32_t cpb_cnt_minus1, uint32_t sub_pic_hrd_params_present_flag
) {
	uint32_t i;
...
hevc_parser_hrd_parameters(
	bit_reader_state_t* reader, bool_t common_inf_present_flag, int max_num_sublayers_minus1
) {
...
static void
hevc_parser_skip_vui_parameters(bit_reader_state_t* reader, hevc_sps_t* sps) {
...
static void
hevc_parser_skip_sps_range_extension(bit_reader_state_t* reader) {
...
static void
hevc_parser_parse_sps_scc_extension(bit_reader_state_t* reader, hevc_sps_t* sps) {
...
static void
hevc_parser_skip_sps_multilayer_extension(bit_reader_state_t* reader) {
	bit_read_stream_get_one(reader); // inter_view_mv_vert_constraint_flag
}

static void
hevc_parser_skip_sps_3d_extension(bit_reader_state_t* reader) {
	uint32_t d;
...
hevc_parser_skip_profile_tier_level(
	bit_reader_state_t* reader, bool_t profile_present_flag, uint32_t max_num_sublayers_minus1
) {
		...
		for (matrix_id = 0; matrix_id < 6; matrix_id += matrix_id_increment) {
			...
			if (!scaling_list_pred_mode_flag) {
				...
				if (size_id > 1) {
					...
					bit_read_stream_skip_signed_exp(reader);
				}
	...
	if (inter_ref_pic_set_prediction_flag) {
		if (st_rps_idx == sps->num_short_term_ref_pic_sets) {
			...
			if (delta_idx > st_rps_idx) {
				vod_log_error(
					VOD_LOG_ERR, ctx->request_context->log, 0, "hevc_parser_skip_st_ref_pic_set: invalid delta_idx_minus1"
				);
				return VOD_BAD_DATA;
			}
		}
		...
		if (abs_delta_rps < 1 || abs_delta_rps > 32768) {
			vod_log_error(
				VOD_LOG_ERR,
				ctx->request_context->log,
				0,
				"hevc_parser_skip_st_ref_pic_set: invalid abs_delta_rps %u",
				abs_delta_rps
			);
			return VOD_BAD_DATA;
		}
		...
		for (i = 0, k = num_negative_pics - 1; i < num_negative_pics >> 1; i++, k--) {
			delta_poc = rps->delta_poc[i];
			used = rps->used[i];
		...
		if (num_negative_pics >= HEVC_MAX_REFS || num_positive_pics >= HEVC_MAX_REFS) {
			vod_log_error(
				VOD_LOG_ERR, ctx->request_context->log, 0, "hevc_parser_skip_st_ref_pic_set: too many refs"
			);
			return VOD_BAD_DATA;
		}
...
static vod_status_t
hevc_parser_seq_parameter_set_rbsp(avc_hevc_parse_ctx_t* ctx, bit_reader_state_t* reader) {
	hevc_sps_t* sps;
	...
	if (sps_seq_parameter_set_id >= MAX_SPS_COUNT) {
		vod_log_error(
			...
			ctx->request_context->log,
			0,
			"hevc_parser_seq_parameter_set_rbsp: invalid sps id %uD",
	...
	sps->st_rps =
		vod_alloc(ctx->request_context->pool, sps->num_short_term_ref_pic_sets * sizeof(sps->st_rps[0]));
	if (sps->st_rps == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, ctx->request_context->log, 0, "hevc_parser_seq_parameter_set_rbsp: vod_alloc failed"
		);
		return VOD_ALLOC_FAILED;
	}
	...
	if (sps->long_term_ref_pics_present_flag) {
		...
		for (i = 0; i < sps->num_long_term_ref_pics_sps && !reader->stream.eof_reached; i++) {
			...
			sps->used_by_curr_pic_lt_sps_flag |=
				(bit_read_stream_get_one(reader) << i); // used_by_curr_pic_lt_sps_flag[i]
		}
	}
	...
	if (sps_extension_4bits) {
		if (reader->stream.eof_reached) {
			vod_log_error(
				VOD_LOG_ERR, ctx->request_context->log, 0, "hevc_parser_seq_parameter_set_rbsp: stream overflow"
			);
			return VOD_BAD_DATA;
		}
	...
	if (!avc_hevc_parser_rbsp_trailing_bits(reader)) {
		vod_log_error(
			VOD_LOG_ERR, ctx->request_context->log, 0, "hevc_parser_seq_parameter_set_rbsp: invalid trailing bits"
		);
		return VOD_BAD_DATA;
	}
...
hevc_parser_skip_pps_range_extension(
	bit_reader_state_t* reader, hevc_pps_t* pps, uint32_t transform_skip_enabled_flag
) {
...
static void
hevc_parser_skip_delta_dlt(bit_reader_state_t* reader, uint32_t pps_bit_depth_for_depth_layers) {
	...
	if (num_val_delta_dlt > 0) {
		...
		if (max_diff > min_diff) {
			for (k = 1; k < num_val_delta_dlt && !reader->stream.eof_reached; k++) {
				// delta_val_diff_minus_min[k]
				bit_read_stream_skip(reader, avc_hevc_parser_ceil_log2(max_diff - min_diff + 1));
			}
		}
	}
}
...
static void
hevc_parser_skip_pps_3d_extension(bit_reader_state_t* reader) {
		...
		for (i = 0; i < part_num_y && !reader->stream.eof_reached; i++) {
			...
			for (j = 0; j < 4; j++) {
				...
				if (coded_res_flag) {
					for (c = 0; c < 3; c++) {
						res_coeff_q =
							bit_read_stream_get_unsigned_exp(reader); // [idxShiftY][idx_cb][idx_cr][j][c]
						res_coeff_r =
							bit_read_stream_get(reader, cm_res_ls_bits); // [idxShiftY][idx_cb][idx_cr][j][c]
...
static void
hevc_parser_skip_colour_mapping_table(bit_reader_state_t* reader) {
	...
	hevc_parser_skip_colour_mapping_octants(
		reader, cm_octant_depth, 1 << cm_y_part_num_log2, cm_res_ls_bits, 0, 0, 0, 0, 1 << cm_octant_depth
	);
}

static void
hevc_parser_skip_pps_multilayer_extension(bit_reader_state_t* reader) {
...
static void
hevc_parser_skip_pps_scc_extension(bit_reader_state_t* reader, hevc_pps_t* pps) {
...
static vod_status_t
hevc_parser_pic_parameter_set_rbsp(avc_hevc_parse_ctx_t* ctx, bit_reader_state_t* reader) {
	hevc_pps_t* pps;
	...
	pps_pic_parameter_set_id = bit_read_stream_get_unsigned_exp(reader);
	...
	if (pps_pic_parameter_set_id >= MAX_PPS_COUNT) {
		vod_log_error(
			...
			ctx->request_context->log,
			0,
			"hevc_parser_pic_parameter_set_rbsp: invalid pps id %uD",
	...
	pps_seq_parameter_set_id = bit_read_stream_get_unsigned_exp(reader);
	if (pps_seq_parameter_set_id >= ctx->sps.nelts) {
		vod_log_error(
			VOD_LOG_ERR,
			ctx->request_context->log,
			0,
			"hevc_parser_pic_parameter_set_rbsp: invalid sps id %uD",
	...
	if (pps->sps == NULL) {
		vod_log_error(
			...
			ctx->request_context->log,
			0,
			"hevc_parser_pic_parameter_set_rbsp: non-existing sps id %uD",
			pps_seq_parameter_set_id
		);
		return VOD_BAD_DATA;
	}
	...
	if (pps_extension_4bits) {
		if (reader->stream.eof_reached) {
			vod_log_error(
				VOD_LOG_ERR, ctx->request_context->log, 0, "hevc_parser_pic_parameter_set_rbsp: stream overflow"
			);
			return VOD_BAD_DATA;
		}
	...
	if (!avc_hevc_parser_rbsp_trailing_bits(reader)) {
		vod_log_error(
			VOD_LOG_ERR, ctx->request_context->log, 0, "hevc_parser_pic_parameter_set_rbsp: invalid trailing bits"
		);
		return VOD_BAD_DATA;
	}
...
hevc_parser_parse_extra_data(
	void* context, vod_str_t* extra_data, uint32_t* nal_packet_size_length, uint32_t* min_packet_size
) {
	...
	for (type_count = *cur_pos++; type_count > 0; type_count--) {
		...
		for (; count > 0; count--) {
			...
			if (unit_size < HEVC_NAL_HEADER_SIZE) {
				vod_log_error(
					VOD_LOG_ERR, ctx->request_context->log, 0, "hevc_parser_parse_extra_data: unit smaller than header size"
				);
				return VOD_BAD_DATA;
			}
			...
			rc = avc_hevc_parser_emulation_prevention_decode(
				ctx->request_context, &reader, cur_pos, unit_size
			);
			if (rc != VOD_OK) {
				return rc;
			}
...
hevc_parser_skip_ref_pic_lists_modification(
	bit_reader_state_t* reader, uint32_t slice_type, uint32_t* num_ref_idx, int num_pic_total_curr
) {
...
hevc_parser_skip_pred_weight_table(
	bit_reader_state_t* reader, uint8_t slice_type, uint32_t* num_ref_idx, int chroma_array_type
) {
...
vod_status_t
hevc_parser_get_slice_header_size(void* context, const u_char* buffer, uint32_t size, uint32_t* result) {
	...
	rc = avc_hevc_parser_emulation_prevention_decode(
		ctx->request_context, &reader, buffer + HEVC_NAL_HEADER_SIZE, size - HEVC_NAL_HEADER_SIZE
	);
	if (rc != VOD_OK) {
		return rc;
	}
	...
	if (slice_pic_parameter_set_id >= ctx->pps.nelts) {
		vod_log_error(
			...
			ctx->request_context->log,
			0,
			"hevc_parser_get_slice_header_size: invalid pps id %uD",
	...
	if (pps == NULL) {
		vod_log_error(
			...
			ctx->request_context->log,
			0,
			"hevc_parser_get_slice_header_size: non-existing pps id %uD",
			slice_pic_parameter_set_id
		);
		return VOD_BAD_DATA;
	}
	...
	if (!dependent_slice_segment_flag) {
		...
		if (nal_unit_type != HEVC_NAL_IDR_W_RADL && nal_unit_type != HEVC_NAL_IDR_N_LP) {
			...
			if (!short_term_ref_pic_set_sps_flag) {
				rc = hevc_parser_skip_st_ref_pic_set(
					ctx, &reader, sps, sps->num_short_term_ref_pic_sets, &st_rps_buf
				);
				if (rc != VOD_OK) {
					return rc;
				}
				...
				if (sps->num_short_term_ref_pic_sets > 1) {
					short_term_ref_pic_set_idx = bit_read_stream_get(
						&reader, avc_hevc_parser_ceil_log2(sps->num_short_term_ref_pic_sets)
					);
					...
					if (i < num_long_term_sps) {
						if (sps->num_long_term_ref_pics_sps > 1) {
							lt_idx_sps = bit_read_stream_get(
								&reader, avc_hevc_parser_ceil_log2(sps->num_long_term_ref_pics_sps)
							);
							lt_used_by_curr_pic_sum +=
								(sps->used_by_curr_pic_lt_sps_flag >> lt_idx_sps) & 1;
						}
					} else {
						bit_read_stream_skip(&reader, sps->log2_max_pic_order_cnt_lsb); // poc_lsb_lt[i]
						lt_used_by_curr_pic_sum +=
							bit_read_stream_get_one(&reader); // used_by_curr_pic_lt_flag[i]
					}
		...
		if (slice_type == HEVC_SLICE_P || slice_type == HEVC_SLICE_B) {
			...
			if (pps->lists_modification_present_flag) {
				num_pic_total_curr = (st_rps != NULL ? hevc_parser_st_rps_frame_nb_refs(st_rps) : 0)
				                   + lt_used_by_curr_pic_sum
				                   + pps->pps_curr_pic_ref_enabled_flag;

				if (num_pic_total_curr > 1) {
					hevc_parser_skip_ref_pic_lists_modification(
						&reader, slice_type, num_ref_idx, num_pic_total_curr
					);
				}
			}
			if (slice_type == HEVC_SLICE_B) {
			...
			if ((pps->weighted_pred_flag && slice_type == HEVC_SLICE_P)
				...
				hevc_parser_skip_pred_weight_table(
					...
					slice_type,
					num_ref_idx,
					!sps->separate_colour_plane_flag || sps->chroma_format_idc != 3
				);
			}
	...
	{
		vod_log_error(
			VOD_LOG_ERR, ctx->request_context->log, 0, "hevc_parser_get_slice_header_size: invalid alignment one bit"
		);
		return VOD_BAD_DATA;
	}
	...
	while (reader.cur_bit > 0 && !reader.stream.eof_reached) {
		...
		{
			vod_log_error(
				VOD_LOG_ERR, ctx->request_context->log, 0, "hevc_parser_get_slice_header_size: invalid alignment zero bit"
			);
			return VOD_BAD_DATA;
		}
	}
	...
	if (reader.stream.eof_reached) {
		vod_log_error(
			VOD_LOG_ERR, ctx->request_context->log, 0, "hevc_parser_get_slice_header_size: bit stream overflow"
		);
		return VOD_BAD_DATA;
	}
	...
	*result = HEVC_NAL_HEADER_SIZE + (reader.stream.cur_pos - start_pos);
	...
	if (start_pos != buffer + HEVC_NAL_HEADER_SIZE) {
		*result += avc_hevc_parser_emulation_prevention_encode_bytes(start_pos, reader.stream.cur_pos);
	}
...
uint8_t
hevc_parser_get_transfer_characteristics(void* context) {
	avc_hevc_parse_ctx_t* ctx = context;
