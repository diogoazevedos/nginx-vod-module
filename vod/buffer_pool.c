...
buffer_pool_create(vod_pool_t* pool, vod_log_t* log, size_t buffer_size, size_t count) {
	...
	if ((buffer_size & 0x0F) != 0) {
		vod_log_error(
			VOD_LOG_ERR, log, 0, "buffer_pool_create: invalid size %uz must be a multiple of 16", buffer_size
		);
		return NULL;
	}
	...
	if (buffer_pool == NULL) {
		vod_log_debug0(VOD_LOG_DEBUG_LEVEL, log, 0, "buffer_pool_create: vod_alloc failed (1)");
		return NULL;
	}

	cur_buffer = vod_alloc(pool, buffer_size * count);
	if (cur_buffer == NULL) {
		vod_log_debug0(VOD_LOG_DEBUG_LEVEL, log, 0, "buffer_pool_create: vod_alloc failed (2)");
		return NULL;
	}
...
buffer_pool_alloc(request_context_t* request_context, buffer_pool_t* buffer_pool, size_t* buffer_size) {
	...
	if (cln == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "buffer_pool_alloc: vod_pool_cleanup_add failed"
		);
		return NULL;
	}
