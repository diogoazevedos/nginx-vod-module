...
write_buffer_flush(write_buffer_state_t* state, bool_t reallocate) {
	...
	if (state->cur_pos > state->start_pos) {
		rc = state->write_callback(
			state->write_context, state->start_pos, state->cur_pos - state->start_pos
		);
		if (rc != VOD_OK) {
			vod_log_debug1(
				VOD_LOG_DEBUG_LEVEL,
				...
				0,
				"write_buffer_flush: write_callback failed %i",
				rc
			);
			return rc;
		}
	...
	if (reallocate) {
		...
		if (state->start_pos == NULL) {
			vod_log_debug0(
				VOD_LOG_DEBUG_LEVEL, state->request_context->log, 0, "write_buffer_flush: vod_alloc failed"
			);
...
vod_status_t
write_buffer_get_bytes(write_buffer_state_t* state, size_t min_size, size_t* size, u_char** buffer) {
	vod_status_t rc;
	...
	if (min_size > (size_t)(state->end_pos - state->start_pos)) {
		vod_log_error(
			VOD_LOG_ERR, state->request_context->log, 0, "write_buffer_get_bytes: invalid size request %uz", min_size
		);
...
vod_status_t
write_buffer_write(write_buffer_state_t* state, const u_char* buffer, size_t size) {
	vod_status_t rc;
