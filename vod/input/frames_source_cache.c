...
frames_source_cache_init(
	...
	if (state == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "frames_source_cache_init: vod_alloc failed"
		);
...
frames_source_cache_read(void* ctx, u_char** buffer, uint32_t* size, bool_t* frame_done) {
	...
	if (!read_cache_get_from_cache(state->read_cache_state, &state->req, buffer, size)) {
		return VOD_AGAIN;
	}
