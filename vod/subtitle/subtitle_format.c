...
vod_status_t
subtitle_reader_init(request_context_t* request_context, void** ctx) {
	subtitle_reader_state_t* state;

	state = vod_alloc(request_context->pool, sizeof(*state));
	if (state == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "subtitle_reader_init: vod_alloc failed"
		);
		return VOD_ALLOC_FAILED;
	}

	state->first_time = TRUE;
	// XXXXX: support configuring different metadata size limits per format
	state->size_limit = 2 * 1024 * 1024;
...
subtitle_parse(
	...
	if (metadata == NULL) {
		vod_log_debug0(VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "subtitle_parse: vod_alloc failed");
		return VOD_ALLOC_FAILED;
	}
	...
	*result = &metadata->base;
	...
	// filter by language
	if (parse_params->langs_mask != NULL && !vod_is_bit_set(parse_params->langs_mask, tags.language)) {
		metadata->base.tracks.nelts = 0;
		return VOD_OK;
	}
	...
	if (vod_array_init(&metadata->base.tracks, request_context->pool, 1, sizeof(*track)) != VOD_OK) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "subtitle_parse: vod_array_init failed"
		);
		return VOD_ALLOC_FAILED;
	}
