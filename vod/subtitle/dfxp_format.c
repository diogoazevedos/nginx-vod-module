...
static vod_status_t
dfxp_reader_init(request_context_t* request_context, vod_str_t* buffer, size_t max_metadata_size, void** ctx) {
...
dfxp_parse_timestamp(u_char* ts) {
	...
	// NOTE: according to spec, hours must be at least 2 digits, but some samples have only one so
...
dfxp_xml_sax_error(void* data, const char* msg, ...) {
	...
	vod_log_error(
		VOD_LOG_ERR, request_context->log, 0, "dfxp_xml_sax_error: libxml2 error: %*s", n + 1, buf
	);
}
...
dfxp_xml_schema_error(void* data, const char* msg, ...) {
	...
	vod_log_error(
		VOD_LOG_WARN, request_context->log, 0, "dfxp_xml_schema_error: libxml2 error: %*s", n + 1, buf
	);
}
...
dfxp_parse(
	...
	if (cln == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "dfxp_parse: vod_pool_cleanup_add failed"
		);
	...
	if (ctxt == NULL) {
		vod_log_error(VOD_LOG_ERR, request_context->log, 0, "dfxp_parse: xmlCreateDocParserCtxt failed");
		return VOD_ALLOC_FAILED;
	}
	...
	if (xmlParseDocument(ctxt) != 0 || ctxt->myDoc == NULL || !ctxt->wellFormed) {
		vod_log_debug0(VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "dfxp_parse: xml parsing failed");
		if (ctxt->myDoc != NULL) {
	...
	return subtitle_parse(
		request_context, parse_params, source, doc, dfxp_get_duration(doc), metadata_part_count, result
	);
}
...
static vod_status_t
dfxp_get_frame_body(request_context_t* request_context, xmlNode* cur_node, vod_str_t* result) {
	size_t alloc_size;
	u_char* start;
	u_char* end;
	...
	if (start == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "dfxp_get_frame_body: vod_alloc failed"
		);
		return VOD_ALLOC_FAILED;
	}
...
dfxp_parse_frames(
	...
	if (vod_array_init(&frames, request_context->pool, 5, sizeof(*cur_frame)) != VOD_OK) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "dfxp_parse_frames: vod_array_init failed"
		);
		return VOD_ALLOC_FAILED;
	}
	...
	for (cur_node = xmlDocGetRootElement(metadata->context);; cur_node = cur_node->next) {
		...
		if (cur_frame == NULL) {
			vod_log_debug0(
				VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "dfxp_parse_frames: vod_array_push failed"
			);
			return VOD_ALLOC_FAILED;
		}
