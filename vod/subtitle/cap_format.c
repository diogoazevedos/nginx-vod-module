...
cap_parse(
	...
	return subtitle_parse(
		request_context, parse_params, source, NULL, cap_get_duration(source), metadata_part_count, result
	);
}
...
cap_parse_frames(
	...
	if (vod_array_init(&frames, request_context->pool, 5, sizeof(*cur_frame)) != VOD_OK) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "cap_parse_frames: vod_array_init failed"
		);
	...
	for (cur_pos = cap_get_next_block(cur_pos, end_pos);; cur_pos = next) {
		...
		if (cur_frame == NULL) {
			vod_log_debug0(
				VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "cap_parse_frames: vod_array_push failed"
			);
			return VOD_ALLOC_FAILED;
		}
		...
		if (p == NULL) {
			vod_log_debug0(
				VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "cap_parse_frames: vod_alloc failed"
			);
			return VOD_ALLOC_FAILED;
		}
