...
#define WEBVTT_CUE_TIMINGS_MAX_SIZE \
	(WEBVTT_TIMESTAMP_MAX_SIZE * 2 + sizeof(WEBVTT_TIMESTAMP_DELIM) - 1)
// iOS11 doesn't play when the first vtt segment is smaller than 10 bytes
#define WEBVTT_MIN_SEGMENT_SIZE (10)
...
webvtt_builder_build(
	...
	for (cur_track = first_track; cur_track < media_set->filtered_tracks_end; cur_track++) {
		result_size +=
			cur_track->total_frames_size + WEBVTT_CUE_TIMINGS_MAX_SIZE * cur_track->frame_count;
	}
	...
	if (p == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "webvtt_builder_build: vod_alloc failed"
		);
	...
	if (base_time > 0) {
		...
		p = vod_copy(
			p,
			first_track->media_info.extra_data.data + WEBVTT_HEADER_LEN,
