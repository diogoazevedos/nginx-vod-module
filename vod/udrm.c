...
udrm_parse_response(
	...
	if (rc != VOD_JSON_OK) {
		vod_log_error(
			VOD_LOG_ERR, request_context->log, 0, "udrm_parse_response: vod_json_parse failed %i: %s", rc, error
		);
		return VOD_BAD_DATA;
	}
	...
	if (parsed_info.type != VOD_JSON_ARRAY
		...
		vod_log_error(
			VOD_LOG_ERR, request_context->log, 0, "udrm_parse_response: expected an array containing a single object"
		);
		return VOD_BAD_DATA;
	}
	...
	if (drm_info_values[DRM_INFO_PARAM_KEY] == NULL || drm_info_values[DRM_INFO_PARAM_KEY_ID] == NULL) {
		vod_log_error(
			VOD_LOG_ERR, request_context->log, 0, "udrm_parse_response: missing fields, \"key\", \"key_id\" are mandatory"
		);
		return VOD_BAD_DATA;
	}
	...
	if (result == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "udrm_parse_response: vod_alloc failed (1)"
		);
		return VOD_ALLOC_FAILED;
	}

	rc = parse_utils_parse_fixed_base64_string(
		&drm_info_values[DRM_INFO_PARAM_KEY]->v.str, result->key, sizeof(result->key)
	);
	if (rc != VOD_JSON_OK) {
		vod_log_error(
			VOD_LOG_ERR,
			request_context->log,
			0,
			"udrm_parse_response: parse_utils_parse_fixed_base64_string(key) failed %i",
			rc
		);
		return VOD_BAD_DATA;
	}
	...
	rc = parse_utils_parse_fixed_base64_string(
		&drm_info_values[DRM_INFO_PARAM_KEY_ID]->v.str, result->key_id, sizeof(result->key_id)
	);
	if (rc != VOD_JSON_OK) {
		vod_log_error(
			VOD_LOG_ERR,
			request_context->log,
			0,
			"udrm_parse_response: parse_utils_parse_fixed_base64_string(key_id) failed %i",
			rc
		);
		return VOD_BAD_DATA;
	}
	...
	if (drm_info_values[DRM_INFO_PARAM_IV] != NULL) {
		rc = parse_utils_parse_fixed_base64_string(
			&drm_info_values[DRM_INFO_PARAM_IV]->v.str, result->iv, sizeof(result->iv)
		);
		if (rc != VOD_JSON_OK) {
			vod_log_error(
				VOD_LOG_ERR,
				request_context->log,
				0,
				"udrm_parse_response: parse_utils_parse_fixed_base64_string(iv) failed %i",
				rc
			);
			return VOD_BAD_DATA;
		}
	...
	if (pssh_array->type != VOD_JSON_OBJECT) {
		vod_log_error(
			...
			request_context->log,
			0,
			"udrm_parse_response: invalid pssh element type %d expected object",
			pssh_array->type
		);
		return VOD_BAD_DATA;
	}
	...
	result->pssh_array.first =
		vod_alloc(request_context->pool, sizeof(*result->pssh_array.first) * result->pssh_array.count);
	if (result->pssh_array.first == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "udrm_parse_response: vod_alloc failed (2)"
		);
		return VOD_ALLOC_FAILED;
	}
	...
	for (cur_input_pssh = part->first, cur_output_pssh = result->pssh_array.first;;
		...
		if (pssh_values[PSSH_PARAM_SYSTEM_ID] == NULL || pssh_values[PSSH_PARAM_DATA] == NULL) {
			vod_log_error(
				VOD_LOG_ERR,
				request_context->log,
				0,
		...
		rc = parse_utils_parse_guid_string(
			&pssh_values[PSSH_PARAM_SYSTEM_ID]->v.str, cur_output_pssh->system_id
		);
		if (rc != VOD_JSON_OK) {
			vod_log_error(
				VOD_LOG_ERR,
				request_context->log,
				0,
				"udrm_parse_response: parse_utils_parse_guid_string(uuid) failed %i",
				rc
			);
			return VOD_BAD_DATA;
		}
		...
		if (base64_decode_pssh) {
			rc = parse_utils_parse_variable_base64_string(
				request_context->pool, &pssh_values[PSSH_PARAM_DATA]->v.str, &cur_output_pssh->data
			);
			if (rc != VOD_JSON_OK) {
				vod_log_error(
					VOD_LOG_ERR,
					request_context->log,
					0,
					"udrm_parse_response: parse_utils_parse_variable_base64_string(data) failed %i",
					rc
				);
				return VOD_BAD_DATA;
			}
		} else {
			cur_output_pssh->data.data =
				vod_pstrdup(request_context->pool, &pssh_values[PSSH_PARAM_DATA]->v.str);
			if (cur_output_pssh->data.data == NULL) {
				vod_log_debug0(
					VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "udrm_parse_response: vod_pstrdup failed"
				);
				return VOD_ALLOC_FAILED;
			}
...
udrm_init_parser(vod_pool_t* pool, vod_pool_t* temp_pool) {
	...
	rc = vod_json_init_hash(
		pool, temp_pool, "drm_info_keys_hash", drm_info_keys_def, sizeof(drm_info_keys_def[0]), &drm_info_keys_hash
	);
	if (rc != VOD_OK) {
		return rc;
	}
	...
	rc = vod_json_init_hash(
		pool, temp_pool, "pssh_keys_hash", pssh_keys_def, sizeof(pssh_keys_def[0]), &pssh_keys_hash
	);
	if (rc != VOD_OK) {
		return rc;
	}
