...
ebml_read_num(ebml_context_t* context, uint64_t* result, size_t max_size, int remove_first_bit) {
	...
	if (context->cur_pos >= context->end_pos) {
		vod_log_error(VOD_LOG_ERR, context->request_context->log, 0, "ebml_read_num: stream overflow (1)");
		return VOD_BAD_DATA;
	}
	...
	if (num_size > max_size) {
		vod_log_error(
			...
			0,
			"ebml_read_num: number size %uz greater than the limit %uz",
			num_size,
			max_size
		);
		return VOD_BAD_DATA;
	...
	if (bytes_to_read > context->end_pos - context->cur_pos) {
		vod_log_error(VOD_LOG_ERR, context->request_context->log, 0, "ebml_read_num: stream overflow (2)");
		return VOD_BAD_DATA;
	}
...
ebml_read_size(ebml_context_t* context, uint64_t* result, bool_t truncate) {
	...
	if (rc < 0) {
		vod_log_debug1(
			VOD_LOG_DEBUG_LEVEL, context->request_context->log, 0, "ebml_read_size: ebml_read_num failed %i", rc
		);
		return rc;
	}
...
ebml_read_float(ebml_context_t* context, size_t size, double* result) {
	...
	default:
		vod_log_error(
			VOD_LOG_ERR, context->request_context->log, 0, "ebml_read_float: invalid float size %uz", size
		);
		return VOD_BAD_DATA;
	}
...
ebml_parse_element(ebml_context_t* context, ebml_spec_t* spec, void* dest) {
	...
	if (rc != VOD_OK) {
		vod_log_debug1(
			...
			context->request_context->log,
			0,
			"ebml_parse_element: ebml_read_size failed, id=0x%uxD",
			spec->id
		);
		return rc;
	}
	...
	if (max_size && size > max_size) {
		vod_log_error(
			...
			context->request_context->log,
			0,
			"ebml_parse_element: invalid size %uz for element 0x%uxD",
			size,
			spec->id
		);
		return VOD_BAD_DATA;
	...
	case EBML_UINT:
		...
		if (rc != VOD_OK) {
			vod_log_debug1(
				...
				context->request_context->log,
				0,
				"ebml_parse_element: ebml_read_uint failed, id=0x%uxD",
				spec->id
			);
			return rc;
		}
	...
	case EBML_FLOAT:
		...
		if (rc != VOD_OK) {
			vod_log_debug1(
				...
				context->request_context->log,
				0,
				"ebml_parse_element: ebml_read_float failed, id=0x%uxD",
				spec->id
			);
			return rc;
		}
	...
	case EBML_MASTER:
		...
		if (rc != VOD_OK) {
			vod_log_debug1(
				...
				context->request_context->log,
				0,
				"ebml_parse_element: ebml_parse_master failed, id=0x%uxD",
				spec->id
			);
			return rc;
		}
	...
	case EBML_CUSTOM:
		...
		if (rc != VOD_OK) {
			vod_log_debug1(
				...
				context->request_context->log,
				0,
				"ebml_parse_element: parser failed, id=0x%uxD",
				spec->id
			);
			return rc;
		}
	...
	default:;
		vod_log_error(
			...
			context->request_context->log,
			0,
			"ebml_parse_element: unsupported type, id=0x%uxD",
			spec->id
		);
	}
...
ebml_parse_single(ebml_context_t* context, ebml_spec_t* spec, void* dest) {
	...
	if (rc < 0) {
		vod_log_debug1(
			...
			context->request_context->log,
			0,
			"ebml_parse_single: ebml_read_id failed, rc=%i",
			rc
		);
		return rc;
	}
	...
	if (rc != VOD_OK) {
		vod_log_debug1(
			...
			context->request_context->log,
			0,
			"ebml_parse_single: ebml_parse_element failed, id=0x%uxL",
			id
		);
		return rc;
	}
	...
	while (context->cur_pos < context->end_pos) {
		...
		if (rc != VOD_OK) {
			vod_log_debug1(
				...
				context->request_context->log,
				0,
				"ebml_parse_master: ebml_parse_single failed, rc=%i",
				rc
			);
			return rc;
		}
...
ebml_parse_header(ebml_context_t* context, ebml_header_t* header) {
	...
	if (rc != VOD_OK) {
		vod_log_debug1(
			...
			context->request_context->log,
			0,
			"ebml_parse_header: ebml_parse_single failed, rc=%i",
			rc
		);
		return rc;
	}
