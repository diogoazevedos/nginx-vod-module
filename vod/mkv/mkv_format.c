...
/*
 * When reading the frames, need to read some extra in order to have the headers of the first frame
 * of the next cluster:
 *
 *    [field]                [max size]
 *    MKV_ID_CLUSTER         (4)
...
static vod_status_t
mkv_parse_frame_estimate_bitrate(ebml_context_t* context, ebml_spec_t* spec, void* dst);
...
mkv_metadata_reader_init(
	...
	if (!mkv_is_doctype_supported(&header.doctype)) {
		vod_log_debug1(
			...
			request_context->log,
			0,
			"mkv_metadata_reader_init: unsupported doctype \"%V\"",
			&header.doctype
		);
		return VOD_NOT_FOUND;
	}
	...
	if (state == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "mkv_metadata_reader_init: vod_alloc failed"
		);
...
mkv_parse_seek_entry(ebml_context_t* context, ebml_spec_t* spec, void* dst) {
	...
	if (rc != VOD_OK) {
		vod_log_debug1(
			...
			0,
			"mkv_parse_seek_entry: ebml_parse_master failed %i",
			rc
		);
		return rc;
	}
...
static vod_status_t
mkv_get_file_layout(
	request_context_t* request_context, const u_char* buffer, size_t size, mkv_file_layout_t* result
) {
	ebml_header_t header;
	vod_status_t rc;
	ebml_context_t context;
	...
	if (rc < 0) {
		vod_log_debug1(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "mkv_get_file_layout: ebml_read_id failed %i", rc
		);
		return rc;
	}
	...
	if (segment_id != MKV_ID_SEGMENT) {
		vod_log_error(
			...
			request_context->log,
			0,
			"mkv_get_file_layout: expected segment element, got 0x%uxL",
			segment_id
		);
		return VOD_BAD_DATA;
	}
	...
	if (rc < 0) {
		vod_log_debug1(
			...
			request_context->log,
			0,
			"mkv_get_file_layout: ebml_read_num(segment_size) failed %i",
			rc
		);
		return rc;
	}
	...
	if (is_unknown_size(result->base.segment_size, rc)) {
		vod_log_error(VOD_LOG_ERR, request_context->log, 0, "mkv_get_file_layout: segment size is unknown");
		return VOD_BAD_DATA;
	}
	...
	if (rc != VOD_OK) {
		vod_log_debug1(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "mkv_get_file_layout: failed to parse seekhead %i", rc
		);
		return rc;
	}
	...
	for (i = 0; i < SECTION_FILE_COUNT; i++) {
		if (result->positions[i].pos == 0) {
			vod_log_error(
				VOD_LOG_ERR, request_context->log, 0, "mkv_get_file_layout: missing position for index %d", i
			);
			return VOD_BAD_DATA;
		}
	...
	qsort(result->positions, SECTION_FILE_COUNT, sizeof(result->positions[0]), mkv_compare_section_positions);
	...
	for (; state->section < SECTION_FILE_COUNT; state->section++) {
		...
		if (position->pos < offset || position->pos + 16 >= offset + buffer->len) {
			if (state->state >= MRS_READ_SECTION_HEADER && state->section == initial_section) {
				vod_log_error(
					VOD_LOG_ERR, state->request_context->log, 0, "mkv_metadata_reader_read: truncated file (1)"
				);
				return VOD_BAD_DATA;
			}
		...
		if (rc < 0) {
			vod_log_debug0(
				VOD_LOG_DEBUG_LEVEL, state->request_context->log, 0, "mkv_metadata_reader_read: failed to parse section id"
			);
			return rc;
		}
		...
		if (id != position->id) {
			vod_log_error(
				...
				0,
				"mkv_metadata_reader_read: expected id 0x%uxD got 0x%uxL",
				position->id,
				id
			);
			return VOD_BAD_DATA;
		...
		if (rc < 0) {
			vod_log_debug1(
				...
				state->request_context->log,
				0,
				"mkv_metadata_reader_read: ebml_read_num(section_size) failed %i",
				rc
			);
			return rc;
		}
		...
		if (is_unknown_size(size, rc)) {
			vod_log_error(
				...
				state->request_context->log,
				0,
				"mkv_metadata_reader_read: section 0x%uxD has unknown size",
				position->id
			);
			return VOD_BAD_DATA;
		}
		...
		if (data_pos + size > offset + buffer->len) {
			if (state->state >= MRS_READ_SECTION_DATA && state->section == initial_section) {
				vod_log_error(
					VOD_LOG_ERR, state->request_context->log, 0, "mkv_metadata_reader_read: truncated file (2)"
				);
				return VOD_BAD_DATA;
			}
...
mkv_metadata_parse(
	...
	if (rc != VOD_OK) {
		vod_log_debug1(
			...
			request_context->log,
			0,
			"mkv_metadata_parse: ebml_parse_master(info) failed %i",
			rc
		);
		return rc;
	}
	...
	if (info.timescale == 0) {
		vod_log_error(VOD_LOG_ERR, request_context->log, 0, "mkv_metadata_parse: timescale is zero");
		return VOD_BAD_DATA;
	}
	...
	if (timescale * info.timescale != NANOS_PER_SEC) {
		vod_log_error(
			...
			info.timescale,
			(uint64_t)NANOS_PER_SEC
		);
		return VOD_BAD_DATA;
	}

	if (info.duration == 0) {
		vod_log_error(VOD_LOG_ERR, request_context->log, 0, "mkv_metadata_parse: duration is zero");
		return VOD_BAD_DATA;
	}

	if (info.duration > (uint64_t)MAX_DURATION_SEC * timescale) {
		vod_log_error(
			VOD_LOG_ERR, request_context->log, 0, "mkv_metadata_parse: duration %uL too big", info.duration
		);
		return VOD_BAD_DATA;
	}
	...
	if (metadata == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "mkv_metadata_parse: vod_alloc failed"
		);
		return VOD_ALLOC_FAILED;
	}
	...
	if (vod_array_init(&metadata->base.tracks, request_context->pool, 2, sizeof(*cur_track)) != VOD_OK) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "mkv_metadata_parse: vod_array_init failed"
		);
		return VOD_ALLOC_FAILED;
	}
	...
	while (context.cur_pos < context.end_pos) {
		...
		if (rc != VOD_OK) {
			vod_log_debug1(
				...
				request_context->log,
				0,
				"mkv_metadata_parse: ebml_parse_single(track) failed %i",
				rc
			);
			return rc;
		}
		...
		for (cur_codec = mkv_codec_types; cur_codec->mkv_codec_id.len; cur_codec++) {
			if (cur_codec->mkv_codec_id.len == track.codec_id.len
			    && vod_memcmp(cur_codec->mkv_codec_id.data, track.codec_id.data, track.codec_id.len)
			           == 0) {
				break;
			}
		}
		...
		if (cur_codec->extra_data_required && track.codec_private.len == 0) {
			vod_log_error(
				VOD_LOG_ERR, request_context->log, 0, "mkv_metadata_parse: no extra data was parsed for track"
			);
			return VOD_BAD_DATA;
		}
		...
		if (metadata->base.tracks.nelts > MAX_TRACK_COUNT) {
			vod_log_error(
				...
				request_context->log,
				0,
				"mkv_metadata_parse: track count exceeded the limit of %i",
				(vod_int_t)MAX_TRACK_COUNT
			);
		...
		if (cur_track == NULL) {
			vod_log_debug0(
				VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "mkv_metadata_parse: vod_array_push failed"
			);
			return VOD_ALLOC_FAILED;
		}
		...
		case MKV_TRACK_TYPE_VIDEO:
			cur_track->media_info.min_frame_duration =
				rescale_time(track.default_duration, NANOS_PER_SEC, timescale);
			if (cur_track->media_info.min_frame_duration == 0) {
				vod_log_error(
					VOD_LOG_ERR, request_context->log, 0, "mkv_metadata_parse: min frame duration is zero"
				);
				return VOD_BAD_DATA;
			}
		...
		case MKV_TRACK_TYPE_AUDIO:
			...
			case VOD_CODEC_ID_AAC:
				...
				rc = codec_config_mp4a_config_parse(
					request_context, &track.codec_private, &cur_track->media_info
				);
				if (rc != VOD_OK) {
					return rc;
				}
			...
			case VOD_CODEC_ID_MP3:
				cur_track->media_info.u.audio.object_type_id =
					cur_track->media_info.u.audio.sample_rate > 24000 ? 0x6B : 0x69;
				break;
			}
			break;
		}
		...
		// NOTE: setting the duration of all tracks to the file duration, since there is no
		// efficient way to get the duration of a track
		cur_track->media_info.duration = (uint64_t)info.duration;
		cur_track->media_info.full_duration = cur_track->media_info.duration;
		cur_track->media_info.duration_millis =
			rescale_time(cur_track->media_info.duration, timescale, 1000);
		...
		rc = media_format_finalize_track(request_context, parse_params->parse_type, &cur_track->media_info);
		if (rc != VOD_OK) {
			return rc;
		}
	}
	...
	if (metadata_parts[SECTION_LAYOUT].len != sizeof(metadata->base_layout)) {
		vod_log_error(
			...
			request_context->log,
			0,
			"mkv_metadata_parse: invalid layout size %uz",
			metadata_parts[SECTION_LAYOUT].len
		);
...
mkv_get_read_frames_request(
	...
	align_timestamps = TRUE; // XXXXX: conf param

	// XXXXX: optimize this - it may be possible to use the cuetime as the cluster timestamp, and
	// start mid-cluster another possible optimization is to read in fixed sizes until the segment
	// is complete (may reduce the total read size)
	...
	for (;;) {
		...
		if (context.cur_pos < context.end_pos) {
			...
			if (rc != VOD_OK) {
				vod_log_debug1(
					...
					request_context->log,
					0,
					"mkv_get_read_frames_request: ebml_parse_single failed %i",
					rc
				);
				return rc;
			}
		...
		if (read_req->read_offset == ULLONG_MAX) {
			if (align_timestamps && metadata->start_time <= index.time) {
				metadata->start_time = index.time;
				read_req->read_offset = index.cluster_pos;
				initial_time = index.time;
			} else if (metadata->start_time < index.time && prev_index.cluster_pos != ULLONG_MAX) {
	...
	// since mkv does not contain a duration field per frame, we need to read the timestamp of one
	// additional frame per track
	all_tracks_mask = (1 << metadata->base.tracks.nelts) - 1;
	if (all_tracks_mask & ~seen_tracks_mask) {
		// some needed tracks were not included in the index - increase the read size by 1 second in
		// order to read another frame for each track
		...
		// all required tracks were found in the index, increase the read size by a fixed amount in
		// order to contain the headers of the next frame
	...
	vod_log_debug6(
		...
		read_req->read_offset,
		read_req->read_offset + read_req->read_size,
		read_req->read_size,
		extra_read_size,
		metadata->start_time,
		metadata->end_time
...
mkv_sort_gop_frames(vod_array_t* gop_frames) {
	...
	// NOTE: using bubble sort since the frames are expected to be nearly sorted specifically, if
	// the stream does not use B-frames it will be completely sorted

	for (index1 = 0; index1 < limit; index1++) {
		done = TRUE;
		for (index2 = limit - index1, frame1 = frames; index2 > 0; index2--, frame1 = frame2) {
			frame2 = frame1 + 1;
...
static uint64_t
mkv_estimate_next_frame_timecode(request_context_t* request_context, mkv_frame_parse_track_context_t* context) {
	mkv_frame_timecode_t* cur_frame;
	mkv_frame_timecode_t* last_frame;
	uint32_t laces;
...
mkv_parse_frame_estimate_bitrate(ebml_context_t* context, ebml_spec_t* spec, void* dst) {
	...
	mkv_estimate_bitrate_context_t* estimate_context =
		vod_container_of(context, mkv_estimate_bitrate_context_t, context);
	...
	if (rc < 0) {
		vod_log_debug1(
			...
			context->request_context->log,
			0,
			"mkv_parse_frame_estimate_bitrate: ebml_read_num(track_number) failed %i",
			rc
		);
		return rc;
	}
	...
	if (context->cur_pos + 3 > context->end_pos) {
		vod_log_error(
			VOD_LOG_ERR, context->request_context->log, 0, "mkv_parse_frame_estimate_bitrate: block too small"
		);
		return VOD_BAD_DATA;
	}
...
mkv_parse_frames_estimate_bitrate(
	...
	if (context.first_track == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "mkv_parse_frames_estimate_bitrate: vod_alloc failed (1)"
		);
		return VOD_ALLOC_FAILED;
	}
	...
	for (i = 0; i < base->tracks.nelts; i++) {
		...
		if (cur_track->media_info.bitrate == 0
			...
			cur_track->media_info.bitrate =
				track_context->total_frames_size
				* base->timescale
				* 8
				/ (track_context->max_frame_timecode - track_context->min_frame_timecode);
		}
...
mkv_parse_laces(ebml_context_t* context, uint8_t flags, uint32_t* lace_sizes) {
	...
	if (context->cur_pos >= context->end_pos) {
		vod_log_error(
			VOD_LOG_ERR, context->request_context->log, 0, "mkv_parse_laces: overflow while reading number of laces"
		);
		return VOD_BAD_DATA;
	}
	...
	case 0x1: // xiph
		...
		for (i = 0; i < laces - 1; i++) {
			do {
				if (context->cur_pos >= context->end_pos) {
					vod_log_error(
						VOD_LOG_ERR, context->request_context->log, 0, "mkv_parse_laces: overflow while reading xiph lace size"
					);
					return VOD_BAD_DATA;
				}
	...
	case 0x2: // fixed size
		...
		if (size % laces != 0) {
			vod_log_error(
				...
				context->request_context->log,
				0,
				"mkv_parse_laces: invalid fixed size lace, size=%uD, laces=%uD",
				size,
				laces
			);
			return VOD_BAD_DATA;
	...
	case 0x3: // EBML
		...
		if (rc < 0) {
			vod_log_debug1(
				...
				context->request_context->log,
				0,
				"mkv_parse_laces: ebml_read_num(initial lace size) failed %i",
				rc
			);
			return rc;
		}
		...
		if (num > VOD_MAX_UINT32_VALUE) {
			vod_log_error(
				VOD_LOG_ERR, context->request_context->log, 0, "mkv_parse_laces: invalid ebml lace size %uL", num
			);
			return VOD_BAD_DATA;
		}
		...
		for (i = 1; i < laces - 1; i++) {
			...
			if (rc < 0) {
				vod_log_debug1(
					...
					context->request_context->log,
					0,
					"mkv_parse_laces: ebml_read_num(lace size delta) failed %i",
					rc
				);
				return rc;
			}
			...
			if (delta > (int64_t)(VOD_MAX_UINT32_VALUE - lace_sizes[i - 1])) {
				vod_log_error(
					...
					context->request_context->log,
					0,
					"mkv_parse_laces: invalid ebml lace delta %L too big",
					delta
				);
				return VOD_BAD_DATA;
			}
			...
			if (delta < -(int64_t)lace_sizes[i - 1]) {
				vod_log_error(
					...
					context->request_context->log,
					0,
					"mkv_parse_laces: invalid ebml lace delta %L too small",
					delta
				);
				return VOD_BAD_DATA;
			}
	...
	if (size < total) {
		vod_log_error(
			...
			context->request_context->log,
			0,
			"mkv_parse_laces: laces total size %uz overflows size left %uD",
			total,
			size
		);
		return VOD_BAD_DATA;
...
static vod_status_t
mkv_parse_frame(
	mkv_frame_parse_context_t* frame_parse_context, ebml_context_t* context, mkv_cluster_t* cluster, int key_frame
) {
	...
	if (rc < 0) {
		vod_log_debug1(
			...
			context->request_context->log,
			0,
			"mkv_parse_frame: ebml_read_num(track_number) failed %i",
			rc
		);
		return rc;
	}
	...
	if (context->cur_pos + 3 > context->end_pos) {
		vod_log_error(VOD_LOG_ERR, context->request_context->log, 0, "mkv_parse_frame: block too small");
		return VOD_BAD_DATA;
	}
	...
	if (track_context->gop_frames.nelts > MAX_GOP_FRAMES) {
		vod_log_error(
			VOD_LOG_ERR, context->request_context->log, 0, "mkv_parse_frame: gop size exceeds the limit"
		);
		return VOD_BAD_DATA;
	}
	...
	if (gop_frame == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, context->request_context->log, 0, "mkv_parse_frame: vod_array_push failed"
		);
		return VOD_ALLOC_FAILED;
	}
	...
	if (!key_frame) {
		...
		case FRS_WAIT_START_KEY_FRAME:
			if (frame_timecode < frame_parse_context->start_time
			    || track_context
			           != frame_parse_context->first_track) // wait for keyframe on the first track only (will be video in case of muxed stream)
			{
				return VOD_OK;
			}
		...
		case FRS_DONE:
			...
			vod_log_debug3(
				...
				track_number,
				frame_timecode,
				(uint64_t)(uintptr_t)context->cur_pos + context->offset_delta
			);
	...
	if (rc < 0) {
		vod_log_debug1(
			...
			context->request_context->log,
			0,
			"mkv_parse_frame: failed to parse lace sizes %i",
			rc
		);
		return rc;
	}
	...
	for (i = 0; i < laces; i++) {
		...
		if (track_context->frame_count >= frame_parse_context->max_frame_count) {
			vod_log_error(
				...
				context->request_context->log,
				0,
				"mkv_parse_frame: frame count exceeds the limit %uD",
				frame_parse_context->max_frame_count
			);
			return VOD_BAD_DATA;
		}
		...
		if (laces > 1) {
			vod_log_debug7(
				...
				frame_timecode,
				key_frame,
				cur_frame->offset + context->offset_delta,
				cur_frame->size,
				i + 1,
				laces
			);
...
static vod_status_t
mkv_block_group(ebml_context_t* context, ebml_spec_t* spec, void* dst) {
	mkv_frame_parse_context_t* frame_parse_context =
		vod_container_of(context, mkv_frame_parse_context_t, context);
	...
	if (rc != VOD_OK) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, context->request_context->log, 0, "mkv_block_group: ebml_parse_master(block group) failed"
		);
		return rc;
	}
	...
	if (block_group.block.len <= 0) {
		vod_log_error(
			VOD_LOG_ERR, context->request_context->log, 0, "mkv_block_group: block group without block element"
		);
		return VOD_BAD_DATA;
	}
	...
	return mkv_parse_frame(
		frame_parse_context, &block_context, cluster, block_group_context.references == 0
	);
}

static vod_status_t
mkv_reference_block(ebml_context_t* context, ebml_spec_t* spec, void* dst) {
	mkv_block_group_context_t* block_group_context =
		vod_container_of(context, mkv_block_group_context_t, context);
...
static vod_status_t
mkv_simple_block(ebml_context_t* context, ebml_spec_t* spec, void* dst) {
	mkv_frame_parse_context_t* frame_parse_context =
		vod_container_of(context, mkv_frame_parse_context_t, context);
	mkv_cluster_t* cluster = dst;
...
mkv_parse_frames(
	...
	if (frame_parse_context.first_track == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "mkv_parse_frames: vod_alloc failed (1)"
		);
		return VOD_ALLOC_FAILED;
	}
	...
	for (i = 0; i < base->tracks.nelts; i++) {
		...
		track_context->frames.first_frame =
			vod_alloc(request_context->pool, FRAMES_PER_PART * sizeof(input_frame_t));
		if (track_context->frames.first_frame == NULL) {
			vod_log_debug0(
				VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "mkv_parse_frames: vod_alloc failed (2)"
			);
			return VOD_ALLOC_FAILED;
		}

		// initialize the gop frames array
		if (vod_array_init(&track_context->gop_frames, request_context->pool, 60, sizeof(mkv_frame_timecode_t))
		    != VOD_OK) {
			vod_log_debug0(
				VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "mkv_parse_frames: vod_array_init failed"
			);
			return VOD_ALLOC_FAILED;
		}
	...
	if (rc != VOD_OK && rc != VOD_DONE) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "mkv_parse_frames: ebml_parse_master(clusters) failed"
		);
		return rc;
	}
	...
	for (i = 0; i < base->tracks.nelts; i++) {
		...
		if (track_context->frame_count > 0) {
			if (!track_context->done) {
				...
				if (gop_frame == NULL) {
					vod_log_debug0(
						VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "mkv_parse_frames: vod_array_push failed"
					);
					return VOD_ALLOC_FAILED;
				}
...
mkv_prepare_read_frames_request(
	...
	if ((parse_params->parse_type & PARSE_FLAG_FRAMES_ALL) != 0) {
		...
		metadata->start_time =
			rescale_time(parse_params->range->start, 1000, metadata->base.timescale);
	...
	rc = mkv_get_read_frames_request(request_context, metadata, end_margin, read_req);
	if (rc == VOD_OK) {
		return VOD_OK;
	}
...
mkv_read_frames(
	...
	if (frame_data == NULL
		...
		rc = mkv_prepare_read_frames_request(request_context, base, parse_params, segmenter, read_req);
		if (rc != VOD_OK) {
			return rc;
		}
	}
