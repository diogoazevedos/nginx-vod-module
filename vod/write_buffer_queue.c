...
// chosen to be a multiple of mpegTS packet size and AES block size
#define BUFFER_SIZE (188 * 16 * 32)
...
write_buffer_queue_get_buffer(write_buffer_queue_t* queue, uint32_t size, void* writer_context) {
	...
	if (write_buffer->start_pos == NULL) {
		...
		write_buffer->start_pos =
			buffer_pool_alloc(queue->request_context, queue->output_buffer_pool, &buffer_size);
		if (write_buffer->start_pos == NULL) {
			return NULL;
		}
...
write_buffer_queue_send(write_buffer_queue_t* queue, off_t max_offset) {
	...
	while (!vod_queue_empty(&queue->buffers)) {
		...
		rc = queue->write_callback(
			queue->write_context, cur_buffer->start_pos, cur_buffer->cur_pos - cur_buffer->start_pos
		);
		if (rc != VOD_OK) {
			vod_log_debug1(
				VOD_LOG_DEBUG_LEVEL,
				queue->request_context->log,
				0,
				"write_buffer_queue_send: write_callback failed %i",
				rc
			);
			return rc;
		}
	...
	while (!vod_queue_empty(&queue->buffers)) {
		...
		if (rc != VOD_OK) {
			vod_log_debug1(
				...
				queue->request_context->log,
				0,
				"write_buffer_queue_flush: write_callback failed %i",
				rc
			);
			return rc;
		}
