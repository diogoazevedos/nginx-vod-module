...
static json_object_value_def_t media_clip_source_params[] = {
	...
	{vod_string("path"),
     VOD_JSON_STRING,
     offsetof(media_clip_source_t, mapped_uri),
	...
	{vod_string("encryptionScheme"),
     VOD_JSON_STRING,
     offsetof(media_clip_source_t, encryption.scheme),
     media_set_parse_encryption_scheme},
	{vod_string("encryptionKey"),
     VOD_JSON_STRING,
     offsetof(media_clip_source_t, encryption.key),
     media_set_parse_base64_string},
	{vod_string("encryptionIv"),
     VOD_JSON_STRING,
     offsetof(media_clip_source_t, encryption.iv),
     media_set_parse_base64_string},
	{vod_string("sourceType"),
     VOD_JSON_STRING,
...
static json_object_value_def_t media_sequence_params[] = {
	...
	{vod_string("clips"),
     VOD_JSON_ARRAY,
     offsetof(media_sequence_t, unparsed_clips),
     media_set_parse_clips_array},
	{vod_string("language"),
     VOD_JSON_STRING,
     offsetof(media_sequence_t, tags.lang_str),
     media_set_parse_null_term_string},
	{vod_string("label"),
     VOD_JSON_STRING,
     offsetof(media_sequence_t, tags.label),
     media_set_parse_null_term_string},
	{vod_string("default"), VOD_JSON_BOOL, offsetof(media_sequence_t, tags.is_default), media_set_parse_bool},
	{vod_string("autoselect"),
     VOD_JSON_BOOL,
     offsetof(media_sequence_t, tags.is_autoselect),
     media_set_parse_bool},
	{vod_string("characteristics"),
     VOD_JSON_STRING,
     offsetof(media_sequence_t, tags.characteristics),
     media_set_parse_null_term_string},
...
static json_object_key_def_t media_set_params[] = {
	...
	{vod_string("consistentSequenceMediaInfo"),
     VOD_JSON_BOOL,
     MEDIA_SET_PARAM_CONSISTENT_SEQUENCE_MEDIA_INFO},
...
static vod_str_t encryption_schemes[] = {vod_string("cenc"), vod_string("aes-cbc"), vod_null_string};
...
static vod_status_t
media_set_parse_durations(request_context_t* request_context, vod_json_array_t* array, media_set_t* media_set) {
	vod_array_part_t* part;
	uint32_t* output_cur;
	...
	if (array->count < 1 || array->count > MAX_CLIPS) {
		vod_log_error(
			...
			request_context->log,
			0,
			"media_set_parse_durations: invalid number of elements in the durations array %uz",
			array->count
		);
	...
	if (array->type != VOD_JSON_INT) {
		vod_log_error(
			...
			request_context->log,
			0,
			"media_set_parse_durations: invalid duration type %d expected int",
			array->type
		);
		return VOD_BAD_MAPPING;
	}
	...
	output_cur =
		vod_alloc(request_context->pool, sizeof(media_set->timing.durations[0]) * array->count);
	if (output_cur == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "media_set_parse_durations: vod_alloc failed"
		);
	...
	for (cur_pos = part->first;; cur_pos++, output_cur++) {
		...
		if (cur_value <= 0) {
			vod_log_error(
				...
				request_context->log,
				0,
				"media_set_parse_durations: invalid duration %L must be positive",
				cur_value
			);
			return VOD_BAD_MAPPING;
		}
		...
		if (cur_value > MAX_CLIP_DURATION) {
			vod_log_error(
				...
				request_context->log,
				0,
				"media_set_parse_durations: clip duration %L too large",
				cur_value
			);
			return VOD_BAD_MAPPING;
		}
	...
	if (total_duration > MAX_SEQUENCE_DURATION) {
		vod_log_error(
			...
			request_context->log,
			0,
			"media_set_parse_durations: total duration %uL too large",
			total_duration
		);
		return VOD_BAD_MAPPING;
	}
...
static vod_status_t
media_set_parse_int64(void* ctx, vod_json_value_t* value, void* dest) {
	*(uint64_t*)dest = value->v.num.num;
	return VOD_OK;
}

static vod_status_t
media_set_parse_source_type(void* ctx, vod_json_value_t* value, void* dest) {
	...
	if (value->v.str.len == sizeof("file") - 1
		...
		*(media_clip_source_type_t*)dest = MEDIA_CLIP_SOURCE_HTTP;
		...
		vod_log_error(
			...
			0,
			"media_set_parse_source_type: invalid sourceType %V",
			&value->v.str
		);
		return VOD_BAD_MAPPING;
	}
...
static vod_status_t
media_set_parse_string_array(void* ctx, vod_json_value_t* value, void* dest) {
	...
	if (array->type != VOD_JSON_STRING) {
		vod_log_error(
			...
			context->request_context->log,
			0,
			"media_set_parse_string_array: invalid array type %d expected string",
			array->type
		);
		return VOD_BAD_MAPPING;
	}
	...
	while (part != NULL) {
		...
		if (destination == NULL) {
			vod_log_debug0(
				VOD_LOG_DEBUG_LEVEL, context->request_context->log, 0, "media_set_parse_string_array: vod_array_push failed"
			);
			return VOD_ALLOC_FAILED;
		}
...
static vod_status_t
media_set_parse_encryption_scheme(void* ctx, vod_json_value_t* value, void* dest) {
	media_clip_source_enc_scheme_t* result = dest;
	media_filter_parse_context_t* context = ctx;
	vod_str_t* cur;

	for (cur = encryption_schemes; cur->len; cur++) {
		if (value->v.str.len == cur->len
		    && vod_strncasecmp(value->v.str.data, cur->data, cur->len) == 0) {
	...
	vod_log_error(
		...
		context->request_context->log,
		0,
		"media_set_parse_encryption_scheme: invalid scheme %V",
		&value->v.str
	);
	return VOD_BAD_MAPPING;
}
...
static vod_status_t
media_set_parse_base64_string(void* ctx, vod_json_value_t* value, void* dest) {
	media_filter_parse_context_t* context = ctx;
	vod_status_t rc;
	...
	if (rc != VOD_OK) {
		vod_log_error(
			...
			context->request_context->log,
			0,
			"media_set_parse_base64_string: failed to parse %V",
			&value->v.str
		);
		return VOD_BAD_MAPPING;
	}
...
vod_status_t
media_set_parse_null_term_string(void* ctx, vod_json_value_t* value, void* dest) {
	...
	if (result.data == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "media_set_parse_null_term_string: vod_alloc failed"
		);
		return VOD_ALLOC_FAILED;
	}
	result.len = 0;
	...
	if (rc != VOD_JSON_OK) {
		vod_log_error(
			...
			request_context->log,
			0,
			"media_set_parse_null_term_string: vod_json_decode_string failed %i",
			rc
		);
		return VOD_BAD_MAPPING;
	}
...
static vod_status_t
media_set_parse_tracks_spec(void* ctx, vod_json_value_t* value, void* dest) {
	media_filter_parse_context_t* context = ctx;
	...
	if (parse_utils_extract_track_tokens(value->v.str.data, end_pos, tracks_mask) != end_pos) {
		vod_log_error(
			VOD_LOG_ERR, context->request_context->log, 0, "media_set_parse_tracks_spec: failed to parse tracks specification"
		);
		return VOD_BAD_MAPPING;
	}
...
static vod_status_t
media_set_parse_clips_array(void* ctx, vod_json_value_t* value, void* dest) {
	media_set_parse_sequences_context_t* context = ctx;
	vod_json_array_t* array = &value->v.arr;
	...
	if (array->type != VOD_JSON_OBJECT) {
		vod_log_error(
			...
			context->request_context->log,
			0,
			"media_set_parse_clips_array: invalid clip type %d expected object",
			array->type
		);
		return VOD_BAD_MAPPING;
	}
...
static vod_status_t
media_set_parse_bitrate(void* ctx, vod_json_value_t* value, void* dest) {
	media_set_parse_sequences_context_t* context = ctx;
	...
	for (; cur_element < last_element; cur_element++) {
		...
		if (cur_element->value.type != VOD_JSON_INT) {
			vod_log_error(
				...
				context->request_context->log,
				0,
				"media_set_parse_bitrate: invalid element type %d expected int",
				cur_element->value.type
			);
			return VOD_BAD_MAPPING;
		}
...
static vod_status_t
media_set_parse_bool(void* ctx, vod_json_value_t* value, void* dest) {
...
media_set_map_source(request_context_t* request_context, u_char* string, media_clip_source_t* source) {
	...
	if (rc != VOD_JSON_OK) {
		vod_log_error(
			VOD_LOG_ERR, request_context->log, 0, "media_set_map_source: failed to parse json %i: %s", rc, error
		);
		return VOD_BAD_MAPPING;
	}
	...
	if (json.type != VOD_JSON_OBJECT) {
		vod_log_error(
			...
			request_context->log,
			0,
			"media_set_map_source: invalid root element type %d expected object",
			json.type
		);
		return VOD_BAD_MAPPING;
	}
	...
	case (size_t)-1:
		vod_log_error(
			VOD_LOG_ERR, request_context->log, 0, "media_set_map_source: missing path in source object"
		);
		return VOD_BAD_MAPPING;

	case 0:
		vod_log_error(
			VOD_LOG_ERR, request_context->log, 0, "media_set_map_source: empty path in source object"
		);
...
static vod_status_t
media_set_parse_source(void* ctx, vod_json_object_t* element, void** result) {
	...
	if (source == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, context->base.request_context->log, 0, "media_set_parse_source: vod_alloc failed"
		);
		return VOD_ALLOC_FAILED;
	}
	...
	case (size_t)-1:
		vod_log_error(
			VOD_LOG_ERR, context->base.request_context->log, 0, "media_set_parse_source: missing path in source object"
		);
		return VOD_BAD_MAPPING;
	...
	case 0:
		vod_log_error(
			VOD_LOG_ERR,
			context->base.request_context->log,
			0,
			"media_set_parse_source: empty path in source object %V",
			&context->media_set->uri
		);
		return VOD_NOT_FOUND;
	}
...
vod_status_t
media_set_parse_filter_sources(void* ctx, vod_json_value_t* value, void* dest) {
	media_filter_parse_context_t* context = ctx;
	vod_array_part_t* part;
	...
	if (sources->count < 1 || sources->count > MAX_SOURCES) {
		vod_log_error(
			...
			context->request_context->log,
			0,
			"media_set_parse_filter_sources: invalid number of elements in the sources array %uz",
			sources->count
		);
		return VOD_BAD_MAPPING;
	}
	...
	if (sources->type != VOD_JSON_OBJECT) {
		vod_log_error(
			...
			context->request_context->log,
			0,
			"media_set_parse_filter_sources: invalid source type %d expected object",
			sources->type
		);
		return VOD_BAD_MAPPING;
	}
	...
	filter->sources =
		vod_alloc(context->request_context->pool, sizeof(filter->sources[0]) * filter->source_count);
	if (filter->sources == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, context->request_context->log, 0, "media_set_parse_filter_sources: vod_alloc failed"
		);
		return VOD_ALLOC_FAILED;
	}

	part = &sources->part;
	for (sources_cur = part->first, output = filter->sources;; sources_cur++, output++) {
		...
		rc = media_set_parse_clip(ctx, sources_cur, filter, output);
		if (rc != VOD_OK) {
			return rc;
		}
	}
...
media_set_parse_closed_captions(
	request_context_t* request_context, media_set_t* media_set, vod_json_array_t* array
) {
	...
	if (array->type != VOD_JSON_OBJECT && array->count > 0) {
		vod_log_error(
			...
			request_context->log,
			0,
			"media_set_parse_closed_captions: invalid closed caption type %d expected object",
			array->type
		);
		return VOD_BAD_MAPPING;
	}
	...
	if (array->count > MAX_CLOSED_CAPTIONS) {
		vod_log_error(
			...
			request_context->log,
			0,
			"media_set_parse_closed_captions: invalid number of elements in the closed captions array %uz",
			array->count
		);
		return VOD_BAD_MAPPING;
	}
	...
	if (cur_output == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "media_set_parse_closed_captions: vod_alloc failed"
		);
		return VOD_ALLOC_FAILED;
	}
	...
	for (cur_pos = part->first;; cur_pos++) {
		...
		if (params[MEDIA_CLOSED_CAPTIONS_PARAM_ID] == NULL) {
			vod_log_error(
				VOD_LOG_ERR, request_context->log, 0, "media_set_parse_closed_captions: missing id in closed captions object"
			);
			return VOD_BAD_MAPPING;
		}
		...
		rc = media_set_parse_null_term_string(
			&request_context, params[MEDIA_CLOSED_CAPTIONS_PARAM_ID], &cur_output->id
		);
		if (rc != VOD_OK) {
			return rc;
		}
		...
		rc = media_set_parse_null_term_string(
			&request_context, params[MEDIA_CLOSED_CAPTIONS_PARAM_LABEL], &cur_output->label
		);
		if (rc != VOD_OK) {
			return rc;
		}
		...
		if (params[MEDIA_CLOSED_CAPTIONS_PARAM_LANGUAGE] != NULL) {
			rc = media_set_parse_null_term_string(
				&request_context, params[MEDIA_CLOSED_CAPTIONS_PARAM_LANGUAGE], &cur_output->language
			);
			if (rc != VOD_OK) {
				return rc;
			}
...
media_set_parse_sequences(
	...
	if (array->count < 1 || array->count > MAX_SEQUENCES) {
		vod_log_error(
			...
			request_context->log,
			0,
			"media_set_parse_sequences: invalid number of sequences %uz",
			array->count
		);
		return VOD_BAD_MAPPING;
	}
	...
	if (array->type != VOD_JSON_OBJECT) {
		vod_log_error(
			...
			request_context->log,
			0,
			"media_set_parse_sequences: invalid sequence type %d expected object",
			array->type
		);
		return VOD_BAD_MAPPING;
	}
	...
	cur_output =
		vod_alloc(request_context->pool, sizeof(media_set->sequences[0]) * required_sequences_num);
	if (cur_output == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "media_set_parse_sequences: vod_alloc failed"
		);
		return VOD_ALLOC_FAILED;
	}
	...
	for (cur_pos = part->first;; cur_pos++, index++) {
		...
		rc = vod_json_parse_object_values(cur_pos, &media_sequence_hash, &context, cur_output);
		if (rc != VOD_OK) {
			return rc;
		}
		...
		if (cur_output->unparsed_clips == NULL) {
			vod_log_error(
				VOD_LOG_ERR, request_context->log, 0, "media_set_parse_sequences: missing clips in sequence object"
			);
			return VOD_BAD_MAPPING;
		}
		...
		if (cur_output->tags.lang_str.len >= LANG_ISO639_3_LEN) {
			cur_output->tags.language =
				lang_parse_iso639_3_code(iso639_3_str_to_int(cur_output->tags.lang_str.data));
			if (cur_output->tags.language != 0) {
				cur_output->tags.lang_str.data =
					(u_char*)lang_get_rfc_5646_name(cur_output->tags.language);
	...
	if (media_set->sequence_count <= 0) {
		vod_log_error(
			VOD_LOG_ERR, request_context->log, 0, "media_set_parse_sequences: request has no sequences"
		);
...
media_set_parse_sequence_clips(
	media_set_parse_context_t* context, vod_array_part_t* part, media_clip_t*** clips
) {
	vod_json_object_t* cur_pos;
	...
	output_cur = vod_alloc(
		context->base.request_context->pool, sizeof(output_cur[0]) * context->clip_ranges.clip_count
	);
	if (output_cur == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, context->base.request_context->log, 0, "media_set_parse_sequence_clips: vod_alloc failed"
		);
		return VOD_ALLOC_FAILED;
	}
	...
	*clips = output_cur;
	...
	for (cur_pos = (vod_json_object_t*)part->first + index;; cur_pos++) {
		...
		rc = media_set_parse_clip(context, cur_pos, NULL, output_cur);
		if (rc != VOD_OK) {
			return rc;
		}
...
static vod_status_t
media_set_parse_sequences_clips(media_set_parse_context_t* context) {
	...
	for (sequence = media_set->sequences; sequence < media_set->sequences_end; sequence++) {
		...
		rc = media_set_parse_sequence_clips(context, sequence->unparsed_clips, &sequence->clips);
		if (rc != VOD_OK) {
			return rc;
		}
	}
...
vod_status_t
media_set_parser_init(vod_pool_t* pool, vod_pool_t* temp_pool) {
...
static vod_status_t
media_set_init_continuous_clip_times(request_context_t* request_context, media_clip_timing_t* timing) {
	uint32_t* cur_duration;
	uint64_t* dest;
	uint64_t* dest_end;
	...
	if (dest == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "media_set_init_continuous_clip_times: vod_alloc failed"
		);
		return VOD_ALLOC_FAILED;
	}
...
media_set_parse_clip_times(
	request_context_t* request_context, media_set_t* media_set, vod_json_value_t** params
) {
	vod_json_array_t* array;
	vod_array_part_t* part;
	int64_t* src;
	...
	dest = vod_alloc(
		request_context->pool, sizeof(media_set->timing.original_times[0]) * media_set->timing.total_count
	);
	if (dest == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "media_set_parse_clip_times: vod_alloc failed"
		);
		return VOD_ALLOC_FAILED;
	}
	...
	if (array->type != VOD_JSON_INT) {
		vod_log_error(
			VOD_LOG_ERR, request_context->log, 0, "media_set_parse_clip_times: clipTimes must be an array of integers"
		);
		return VOD_BAD_MAPPING;
	}
	...
	for (cur_duration = media_set->timing.durations; dest < dest_end; dest++, src++, cur_duration++) {
...
media_set_live_init_clip_times(
	...
	if (params[MEDIA_SET_PARAM_CLIP_TIMES] == NULL || !media_set->use_discontinuity) {
		// first clip time
		if (params[MEDIA_SET_PARAM_FIRST_CLIP_TIME] == NULL) {
			vod_log_error(
				VOD_LOG_ERR, request_context->log, 0, "media_set_live_init_clip_times: firstClipTime missing in live playlist"
			);
			return VOD_BAD_MAPPING;
		}
...
media_set_parse_first_clip_start_offset(
	request_context_t* request_context, media_clip_timing_t* timing, int64_t first_clip_start_offset
) {
...
media_set_parse_live_params(
	...
	if (media_set->use_discontinuity) {
		if (params[MEDIA_SET_PARAM_INITIAL_SEGMENT_INDEX] != NULL) {
			media_set->initial_segment_index =
				params[MEDIA_SET_PARAM_INITIAL_SEGMENT_INDEX]->v.num.num - 1;
		}
		...
		if (params[MEDIA_SET_PARAM_INITIAL_CLIP_INDEX] != NULL) {
			...
			if (request_params->clip_index != INVALID_CLIP_INDEX && request_params->clip_index != 0) {
				vod_log_error(
					VOD_LOG_ERR,
					request_context->log,
					0,
					"media_set_parse_live_params: invalid clip index %uD with single clip",
					request_params->clip_index
				);
				return VOD_BAD_REQUEST;
			}
	...
	if (media_set->timing.segment_base_time != SEGMENT_BASE_TIME_RELATIVE
		...
		vod_log_error(
			...
			media_set->timing.segment_base_time,
			media_set->timing.first_time
		);
		return VOD_BAD_MAPPING;
	}
...
static int64_t
media_set_apply_live_window_duration_param(int64_t live_window_duration, int64_t live_window_duration_param) {
	// ignore values that are not positive
	if (live_window_duration_param <= 0) {
		return live_window_duration;
	}

	// ignore the json param if it has a greater absolute value than the conf, retain the sign of
	// the conf value (0 is treated like -inf)
...
media_set_parse_notifications(
	...
	if (array->count > MAX_NOTIFICATIONS) {
		vod_log_error(
			...
			request_context->log,
			0,
			"media_set_parse_notifications: invalid number of elements in the notifications array %uz",
			array->count
		);
		return VOD_BAD_MAPPING;
	}
	...
	if (array->type != VOD_JSON_OBJECT) {
		vod_log_error(
			...
			request_context->log,
			0,
			"media_set_parse_notifications: invalid notification type %d expected object",
			array->type
		);
		return VOD_BAD_MAPPING;
	}
	...
	for (cur_pos = part->first;; cur_pos++) {
		...
		if (params[MEDIA_NOTIFICATION_PARAM_OFFSET] == NULL) {
			vod_log_error(
				VOD_LOG_ERR, request_context->log, 0, "media_set_parse_notifications: missing offset in notification object"
			);
			return VOD_BAD_MAPPING;
		}
		...
		if (notification == NULL) {
			vod_log_debug0(
				VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "media_set_parse_notifications: vod_alloc failed"
			);
			return VOD_ALLOC_FAILED;
		}
...
media_set_init_look_ahead_segments(
	request_context_t* request_context, media_set_t* media_set, get_clip_ranges_params_t* get_ranges_params
) {
	...
	cur_output = vod_alloc(request_context->pool, sizeof(cur_output[0]) * MAX_LOOK_AHEAD_SEGMENTS);
	if (cur_output == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "media_set_init_look_ahead_segments: vod_alloc failed"
		);
		return VOD_ALLOC_FAILED;
	}
	...
	while (get_ranges_params->segment_index < segment_index_limit) {
		...
		cur_output->duration = (media_set->timing.times[clip_ranges.max_clip_index]
	...
	for (cur_pos = part->first;; cur_pos++) {
		...
		if (cur_duration <= 0 || cur_duration > MAX_CLIP_DURATION) {
			vod_log_error(
				...
				request_context->log,
				0,
				"media_set_sum_key_frame_durations: ignoring invalid key frame duration %L",
				cur_duration
			);
			return VOD_BAD_MAPPING;
		}
		...
		if (next_sum > limit) {
			if ((void*)cur_pos <= part->first) {
				// NOTE: prev_part can't be null, because the first duration was already tested
				// outside this function to be less than the limit
				part = prev_part;
				part->next = NULL;
			} else {
				part->last = cur_pos;
...
media_set_parse_sequence_key_frame_offsets(
	request_context_t* request_context, media_sequence_t* sequence, media_clip_timing_t* timing
) {
	...
	if (timing->total_count > 1) {
		duration_part =
			vod_alloc(request_context->pool, sizeof(*duration_part) * (timing->total_count - 1));
		if (duration_part == NULL) {
			vod_log_debug0(
				VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "media_set_parse_sequence_key_frame_offsets: vod_alloc failed"
			);
			return VOD_ALLOC_FAILED;
		}
	}
	...
	for (cur_pos = part->first, cur_duration = timing->durations, cur_clip_time = timing->times;;
		...
		vod_json_get_object_values(cur_pos, &media_clip_hash, params);
		...
		if (last_part == NULL) {
			...
			if (durations->first >= durations->last || *(int64_t*)durations->first > limit) {
		...
		rc = media_set_sum_key_frame_durations(
			request_context, durations, limit, &last_part, &sum_durations
		);
		if (rc != VOD_OK) {
			return rc;
		}
	...
	if (sequence->key_frame_durations != NULL) {
		...
		vod_log_debug1(
			...
			request_context->log,
			0,
			"media_set_parse_sequence_key_frame_offsets: first_key_frame_offset %L",
			sequence->first_key_frame_offset
		);
		part = sequence->key_frame_durations;
		for (cur_kf_duration = part->first;; cur_kf_duration++) {
			...
			if (last_kf_duration != *cur_kf_duration && kf_count > 0) {
				vod_log_debug2(
					...
					request_context->log,
					0,
					"media_set_parse_sequence_key_frame_offsets: duration %L x %uD",
					last_kf_duration,
					kf_count
				);
				kf_count = 0;
		...
		vod_log_debug1(
			...
			request_context->log,
			0,
			"media_set_parse_sequence_key_frame_offsets: last_key_frame_time %L",
			sequence->last_key_frame_time
		);
	}
#endif // VOD_DEBUG
...
static vod_status_t
media_set_parse_key_frame_offsets(request_context_t* request_context, media_set_t* media_set) {
	media_sequence_t* sequence;
	vod_status_t rc;

	for (sequence = media_set->sequences; sequence < media_set->sequences_end; sequence++) {
		rc = media_set_parse_sequence_key_frame_offsets(request_context, sequence, &media_set->timing);
		if (rc != VOD_OK) {
			return rc;
		}
	}
...
static uint64_t
media_set_start_relative_offset_to_absolute(media_clip_timing_t* timing, uint64_t time_left) {
	...
	for (clip_index = 0;; clip_index++) {
		...
		if (clip_index >= timing->total_count - 1) {
			// NOTE: this is not supposed to happen since the relative offset was verified to be
...
static uint64_t
media_set_end_relative_offset_to_absolute(media_clip_timing_t* timing, uint64_t time_left) {
	uint32_t clip_duration;
	uint32_t clip_index;
...
media_set_parse_segment_duration(
	request_context_t* request_context, int64_t segment_duration, segmenter_conf_t** result
) {
	segmenter_conf_t* segmenter;

	if (segment_duration < MIN_SEGMENT_DURATION || segment_duration > MAX_SEGMENT_DURATION) {
		vod_log_error(
			VOD_LOG_ERR,
			request_context->log,
			0,
	...
	if (segmenter == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "media_set_parse_segment_duration: vod_alloc failed"
		);
		return VOD_ALLOC_FAILED;
	}
	...
	*segmenter = **result;
	...
	segmenter->max_segment_duration =
		vod_max(segment_duration, segmenter->max_bootstrap_segment_duration);
...
media_set_apply_clip_from(
	...
	for (clip_index = 0;; clip_index++) {
		if (clip_index >= timing->total_count) {
			vod_log_error(
				...
				request_context->log,
				0,
				"media_set_apply_clip_from: clip from %uL exceeds last clip end time",
				clip_from
			);
			return VOD_BAD_REQUEST;
		}
	...
	if (clip_from > original_clip_time) {
		...
		// NOTE: aligning to keyframes only in case of vod, since in live, alignment to keyframes
		// will happen in segmenter_get_live_window
		sequence = &media_set->sequences[0];
		if (sequence->key_frame_durations != NULL && media_set->type == MEDIA_SET_VOD) {
			// align to key frames
			initial_offset =
				timing->first_time + sequence->first_key_frame_offset - timing->times[clip_index];
			...
			if ((void*)align_context.cur_pos >= align_context.part->last) {
				...
				align_context.part->count =
					(int64_t*)align_context.part->last - (int64_t*)align_context.part->first;
			}
...
static vod_status_t
media_set_apply_clip_to(request_context_t* request_context, media_set_t* media_set, uint64_t clip_to) {
	align_to_key_frames_context_t align_context;
	media_clip_timing_t* timing = &media_set->timing;
	media_sequence_t* sequence;
	uint64_t clip_time;
	...
	if (clip_to > clip_time) {
		...
		vod_log_error(
			...
			request_context->log,
			0,
			"media_set_apply_clip_to: clip to %uL before first clip start time",
			clip_to
		);
		return VOD_BAD_REQUEST;
	}
...
media_set_parse_json(
	...
	if (rc != VOD_JSON_OK) {
		vod_log_error(
			VOD_LOG_ERR, request_context->log, 0, "media_set_parse_json: failed to parse json %i: %s", rc, error
		);
		return VOD_BAD_MAPPING;
	}
	...
	if (override != NULL) {
		...
		if (rc != VOD_JSON_OK) {
			vod_log_error(
				...
				request_context->log,
				0,
				"media_set_parse_json: failed to parse override json %i: %s",
				rc,
				error
			);
			return VOD_BAD_REQUEST;
	...
	if (json.type != VOD_JSON_OBJECT) {
		vod_log_error(
			...
			request_context->log,
			0,
			"media_set_parse_json: invalid root element type %d expected object",
			json.type
		);
		return VOD_BAD_MAPPING;
	}
	...
	vod_json_get_object_values(&json.v.obj, &media_set_hash, params);

	if (params[MEDIA_SET_PARAM_SEQUENCES] == NULL) {
		vod_log_error(
			VOD_LOG_ERR, request_context->log, 0, "media_set_parse_json: \"sequences\" element is missing"
		);
		return VOD_BAD_MAPPING;
	}
	...
	if (params[MEDIA_SET_PARAM_SEGMENT_DURATION] != NULL) {
		rc = media_set_parse_segment_duration(
			request_context, params[MEDIA_SET_PARAM_SEGMENT_DURATION]->v.num.num, &segmenter
		);
		if (rc != VOD_OK) {
			return rc;
		}
	...
	if (params[MEDIA_SET_PARAM_ID] != NULL) {
		rc = media_set_parse_null_term_string(&request_context, params[MEDIA_SET_PARAM_ID], &result->id);
		if (rc != VOD_OK) {
			return rc;
		}
	}
	...
	if (params[MEDIA_SET_PARAM_CLOSED_CAPTIONS] != NULL) {
		rc = media_set_parse_closed_captions(
			request_context, result, &params[MEDIA_SET_PARAM_CLOSED_CAPTIONS]->v.arr
		);
		if (rc != VOD_OK) {
			return rc;
		}
	...
	if (params[MEDIA_SET_PARAM_DURATIONS] == NULL) {
		...
		if (request_params->clip_index != INVALID_CLIP_INDEX && request_params->clip_index != 0) {
			vod_log_error(
				...
				request_context->log,
				0,
				"media_set_parse_json: invalid clip index %uD with single clip",
				request_params->clip_index
			);
			return VOD_BAD_REQUEST;
		}
		...
		rc = media_set_parse_sequences(
			request_context, result, &params[MEDIA_SET_PARAM_SEQUENCES]->v.arr, request_params
		);
		if (rc != VOD_OK) {
			return rc;
		}
	...
	if (params[MEDIA_SET_PARAM_PLAYLIST_TYPE] == NULL
	    || (params[MEDIA_SET_PARAM_PLAYLIST_TYPE]->v.str.len == playlist_type_vod.len
	        && vod_strncasecmp(
				   ...
				   playlist_type_vod.data,
				   playlist_type_vod.len
			   ) == 0)) {
		result->presentation_end = TRUE;
	} else {
	...
	if (!result->use_discontinuity
		...
		vod_log_error(
			...
			request_context->log,
			0,
			"media_set_parse_json: clip index %uD not allowed in continuous mode",
			request_params->clip_index
		);
		return VOD_BAD_REQUEST;
	}
	...
	// durations
	rc = media_set_parse_durations(request_context, &params[MEDIA_SET_PARAM_DURATIONS]->v.arr, result);
	if (rc != VOD_OK) {
		return rc;
	}
	...
	if (result->type == MEDIA_SET_LIVE) {
		rc = media_set_parse_live_params(request_context, request_params, segmenter, params, result);
		if (rc != VOD_OK) {
			return rc;
		}
		...
		rc = media_set_init_continuous_clip_times(request_context, &result->timing);
		if (rc != VOD_OK) {
			return rc;
		}
	...
	if (request_params->segment_index != INVALID_SEGMENT_INDEX
		...
		if (request_params->segment_time != INVALID_SEGMENT_TIME
			...
			if (result->use_discontinuity
				...
				margin = media_set_is_clip_start(&result->timing, request_params->segment_time)
				           ? 0
				           : SEGMENT_FROM_TIMESTAMP_MARGIN;

				request_params->segment_index = segmenter_get_segment_index_no_discontinuity(
					segmenter, request_params->segment_time - result->timing.segment_base_time + margin
				);
			}
		}
		...
		if (request_params->segment_index != INVALID_SEGMENT_INDEX) {
			...
			if (result->use_discontinuity) {
				...
				rc = segmenter_get_start_end_ranges_discontinuity(&get_ranges_params, &context.clip_ranges);
				if (rc != VOD_OK) {
					return rc;
				}
			} else {
				...
				rc = segmenter_get_start_end_ranges_no_discontinuity(
					&get_ranges_params, &context.clip_ranges
				);
				if (rc != VOD_OK) {
					return rc;
				}
				...
				if (result->type == MEDIA_SET_LIVE
					...
					rc = media_set_init_look_ahead_segments(request_context, result, &get_ranges_params);
					if (rc != VOD_OK) {
						return rc;
					}
				}
			...
			result->initial_segment_clip_relative_index =
				context.clip_ranges.clip_relative_segment_index;
		} else {
			// thumb
			if (request_params->segment_time_type != SEGMENT_TIME_ABSOLUTE) {
				...
				if (request_params->segment_time_type == SEGMENT_TIME_END_RELATIVE) {
					request_params->segment_time =
						media_set_end_relative_offset_to_absolute(&result->timing, segment_time);
				} else {
					request_params->segment_time =
						media_set_start_relative_offset_to_absolute(&result->timing, segment_time);
				}
				return VOD_REDIRECT;
			}

			get_ranges_params.time = request_params->segment_time;
			rc = segmenter_get_start_end_ranges_gop(&get_ranges_params, &context.clip_ranges);
			if (rc != VOD_OK) {
				return rc;
			}
		}
		...
		result->segment_start_time =
			context.clip_ranges.clip_time + context.clip_ranges.clip_ranges[0].start;
			...
			if (result->type == MEDIA_SET_LIVE) {
				...
				if ((request_flags & REQUEST_FLAG_LOOK_AHEAD_SEGMENTS) != 0) {
					...
					if (result->live_window_duration > 0) {
						result->live_window_duration +=
							segmenter->segment_duration * MAX_LOOK_AHEAD_SEGMENTS;
				...
				if (result->timing.total_count > MAX_CLIPS_PER_REQUEST) {
					vod_log_error(
						...
						request_context->log,
						0,
						"media_set_parse_json: clip count %uD exceeds the limit per request",
						result->timing.total_count
					);
					return VOD_BAD_REQUEST;
				}
				...
				if (params[MEDIA_SET_PARAM_REFERENCE_CLIP_INDEX] != NULL) {
					context.clip_ranges.min_clip_index =
						params[MEDIA_SET_PARAM_REFERENCE_CLIP_INDEX]->v.num.num - 1;
