...
vod_dynamic_buf_init(vod_dynamic_buf_t* buffer, request_context_t* request_context, size_t initial_size) {
	...
	if (buffer->start == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "vod_dynamic_buf_init: vod_alloc failed"
		);
...
vod_dynamic_buf_reserve(vod_dynamic_buf_t* buffer, size_t size) {
	...
	if (new_buffer == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, buffer->request_context->log, 0, "vod_dynamic_buf_reserve: vod_alloc failed"
		);
		return VOD_ALLOC_FAILED;
	}
