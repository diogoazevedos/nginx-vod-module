...
#define rescale_time_neg(time, cur_scale, new_scale)        \
	((time) >= 0 ? rescale_time(time, cur_scale, new_scale) \
	             : -rescale_time(-(time), cur_scale, new_scale))
...
#define vod_track_mask_get_number_of_set_bits(mask) \
	vod_get_number_of_set_bits_in_mask((mask), MAX_TRACK_COUNT)
...
#define PARSE_FLAG_PEAK_BITRATE (0x04000000)

// flag groups
#define PARSE_FLAG_FRAMES_ALL      \
	(PARSE_FLAG_FRAMES_DURATION    \
	 | PARSE_FLAG_FRAMES_PTS_DELAY \
	 | PARSE_FLAG_FRAMES_SIZE      \
	 | PARSE_FLAG_FRAMES_OFFSET    \
	 | PARSE_FLAG_FRAMES_IS_KEY)
#define PARSE_FLAG_FRAMES_ALL_EXCEPT_OFFSETS \
	(PARSE_FLAG_FRAMES_DURATION              \
	 | PARSE_FLAG_FRAMES_PTS_DELAY           \
	 | PARSE_FLAG_FRAMES_SIZE                \
	 | PARSE_FLAG_FRAMES_IS_KEY)
...
#define PARSE_FLAG_DURATION_LIMITS_AND_TOTAL_SIZE \
	(PARSE_FLAG_DURATION_LIMITS | PARSE_FLAG_TOTAL_SIZE_ESTIMATE)
...
#define VOD_CH_LAYOUT_7POINT1_WIDE_BACK \
	(VOD_CH_LAYOUT_5POINT1_BACK | VOD_CH_FRONT_LEFT_OF_CENTER | VOD_CH_FRONT_RIGHT_OF_CENTER)
...
// functions
vod_status_t
media_format_finalize_track(request_context_t* request_context, int parse_type, media_info_t* media_info);
