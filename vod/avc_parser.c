...
avc_parser_parse_vui_parameters(avc_sps_t* sps, bit_reader_state_t* reader) {
	...
	timing_info_present_flag = bit_read_stream_get_one(reader);
...
avc_parser_seq_parameter_set_rbsp(avc_hevc_parse_ctx_t* ctx, bit_reader_state_t* reader) {
	...
	profile_idc = bit_read_stream_get(reader, 8);
	...
	if (seq_parameter_set_id >= MAX_SPS_COUNT) {
		vod_log_error(
			...
			0,
			"avc_parser_seq_parameter_set_rbsp: invalid sps id %uD",
	...
	case 1:
		...
		num_ref_frames_in_pic_order_cnt_cycle = bit_read_stream_get_unsigned_exp(reader);
	...
	frame_cropping_flag = bit_read_stream_get_one(reader);
	...
	if (!avc_hevc_parser_rbsp_trailing_bits(reader)) {
		vod_log_error(
			VOD_LOG_ERR, ctx->request_context->log, 0, "avc_parser_seq_parameter_set_rbsp: invalid trailing bits"
		);
		return VOD_BAD_DATA;
	}
...
// PPS
static vod_status_t
avc_parser_pic_parameter_set_rbsp(avc_hevc_parse_ctx_t* ctx, bit_reader_state_t* reader) {
	...
	if (pic_parameter_set_id >= MAX_PPS_COUNT) {
		vod_log_error(
			...
			ctx->request_context->log,
			0,
			"avc_parser_pic_parameter_set_rbsp: invalid pps id %uD",
	...
	if (seq_parameter_set_id >= ctx->sps.nelts) {
		vod_log_error(
			...
			ctx->request_context->log,
			0,
			"avc_parser_pic_parameter_set_rbsp: invalid sps id %uD",
			seq_parameter_set_id
		);
		return VOD_BAD_DATA;
	}
	...
	if (pps->sps == NULL) {
		vod_log_error(
			...
			ctx->request_context->log,
			0,
			"avc_parser_pic_parameter_set_rbsp: non-existing sps id %uD",
			seq_parameter_set_id
		);
		return VOD_BAD_DATA;
	}
	...
	if (pps->num_slice_groups_minus1 > 0) {
		...
		if (pps->slice_group_map_type == 0) {
			...
			for (i = 0; i <= pic_size_in_map_units_minus1 && !reader->stream.eof_reached; i++) {
				bit_read_stream_skip(
					reader,
					avc_hevc_parser_ceil_log2(pps->num_slice_groups_minus1 + 1)
				); // slice_group_id[i]
			}
		}
	}
	...
	if (reader->stream.eof_reached) {
		vod_log_error(
			VOD_LOG_ERR, ctx->request_context->log, 0, "avc_parser_pic_parameter_set_rbsp: stream overflow"
		);
		return VOD_BAD_DATA;
	}
	...
	if (!avc_hevc_parser_rbsp_trailing_bits(reader)) {
		vod_log_error(
			VOD_LOG_ERR, ctx->request_context->log, 0, "avc_parser_pic_parameter_set_rbsp: invalid trailing bits"
		);
		return VOD_BAD_DATA;
	}
...
avc_parser_parse_extra_data(
	...
	if (extra_data->len < sizeof(avcc_config_t)) {
		vod_log_error(
			...
			ctx->request_context->log,
			0,
			"avc_parser_parse_extra_data: extra data size %uz too small",
			extra_data->len
		);
		return VOD_BAD_DATA;
	}
	...
	for (i = 0; i < 2; i++) { // once for SPS, once for PPS
		...
		for (unit_count = (*cur_pos++ & 0x1f); unit_count; unit_count--) {
			...
			if (unit_size < AVC_NAL_HEADER_SIZE) {
				vod_log_error(
					VOD_LOG_ERR, ctx->request_context->log, 0, "avc_parser_parse_extra_data: unit of zero size"
				);
				return VOD_BAD_DATA;
			}
...
// slice header
static void
avc_parser_skip_ref_pic_list_modification(bit_reader_state_t* reader, uint8_t slice_type) {
...
static void
avc_parser_skip_ref_pic_list_mvc_modification(bit_reader_state_t* reader, uint8_t slice_type) {
	bool_t ref_pic_list_modification_flag_l0;
	bool_t ref_pic_list_modification_flag_l1;
	uint8_t modification_of_pic_nums_idc;
...
avc_parser_skip_pred_weight_table(
	bit_reader_state_t* reader, uint8_t slice_type, int* num_ref_idx, int chroma_array_type
) {
...
static void
avc_parser_skip_dec_ref_pic_marking(bit_reader_state_t* reader, uint8_t nal_unit_type) {
...
vod_status_t
avc_parser_get_slice_header_size(void* context, const u_char* buffer, uint32_t size, uint32_t* result) {
	avc_hevc_parse_ctx_t* ctx = context;
	bit_reader_state_t reader;
	...
	rc = avc_hevc_parser_emulation_prevention_decode(
		ctx->request_context, &reader, buffer + AVC_NAL_HEADER_SIZE, size - AVC_NAL_HEADER_SIZE
	);
	if (rc != VOD_OK) {
		return rc;
	}
	...
	if (slice_type > 9) {
		vod_log_error(
			...
			ctx->request_context->log,
			0,
			"avc_parser_get_slice_header_size: invalid slice type %uD",
			(uint32_t)slice_type
		);
		return VOD_BAD_DATA;
	}
	...
	if (pps_id >= ctx->pps.nelts) {
		vod_log_error(
			...
			ctx->request_context->log,
			0,
			"avc_parser_get_slice_header_size: invalid pps id %uD",
			pps_id
		);
		return VOD_BAD_DATA;
	}
	...
	if (pps == NULL) {
		vod_log_error(
			...
			ctx->request_context->log,
			0,
			"avc_parser_get_slice_header_size: non-existing pps id %uD",
			pps_id
		);
		return VOD_BAD_DATA;
	}
	...
	if (reader.stream.eof_reached) {
		vod_log_error(
			VOD_LOG_ERR, ctx->request_context->log, 0, "avc_parser_get_slice_header_size: bit stream overflow"
		);
		return VOD_BAD_DATA;
	}
...
avc_parser_is_slice(void* context, uint8_t nal_type, bool_t* is_slice) {
	...
	case AVC_NAL_DPC:
		vod_log_error(
			VOD_LOG_ERR, ctx->request_context->log, 0, "avc_parser_is_slice: nal types 2-4 are not supported"
		);
		return VOD_BAD_DATA;
...
uint8_t
avc_parser_get_transfer_characteristics(void* context) {
	avc_hevc_parse_ctx_t* ctx = context;
