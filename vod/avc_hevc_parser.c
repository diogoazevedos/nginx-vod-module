...
avc_hevc_parser_emulation_prevention_decode(
	...
	if (output == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "avc_hevc_parser_emulation_prevention_decode: vod_alloc failed"
		);
...
vod_status_t
avc_hevc_parser_init_ctx(request_context_t* request_context, void** result) {
	avc_hevc_parse_ctx_t* ctx;

	ctx = vod_alloc(request_context->pool, sizeof(*ctx));
	if (ctx == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "avc_hevc_parser_init_ctx: vod_alloc failed"
		);
		return VOD_ALLOC_FAILED;
	}
	...
	if (vod_array_init(&ctx->sps, request_context->pool, 1, sizeof(void*)) != VOD_OK) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "avc_hevc_parser_init_ctx: vod_array_init failed (1)"
		);
		return VOD_ALLOC_FAILED;
	}
	...
	if (vod_array_init(&ctx->pps, request_context->pool, 1, sizeof(void*)) != VOD_OK) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "avc_hevc_parser_init_ctx: vod_array_init failed (2)"
		);
		return VOD_ALLOC_FAILED;
	}
