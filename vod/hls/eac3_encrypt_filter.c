...
eac3_encrypt_start_frame(media_filter_context_t* context, output_frame_t* frame) {
	...
	if (frame->size < sizeof(state->header)) {
		vod_log_error(
			...
			0,
			"eac3_encrypt_start_frame: frame size %uD too small",
			frame->size
		);
		return VOD_BAD_DATA;
	}
		...
		if (state->header_left > 0) {
			...
			if (state->header[0] != 0x0b || state->header[1] != 0x77) {
				vod_log_error(
					...
					(uint32_t)state->header[0],
					(uint32_t)state->header[1]
				);
				return VOD_BAD_DATA;
			}
			...
			if (sync_frame_size < sizeof(state->header) || sync_frame_size > state->frame_size_left) {
				vod_log_error(
					...
					context->request_context->log,
					0,
					"eac3_encrypt_write: invalid sync frame size %uD",
					sync_frame_size
				);
				return VOD_BAD_DATA;
			}
			...
			if (state->frame_size_left > 0 && state->frame_size_left < sizeof(state->header)) {
				vod_log_error(
					VOD_LOG_ERR,
					context->request_context->log,
					0,
					"eac3_encrypt_write: invalid frame size left %uD",
...
vod_status_t
eac3_encrypt_filter_init(media_filter_t* filter, media_filter_context_t* context) {
	...
	if (state == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "eac3_encrypt_filter_init: vod_alloc failed"
		);
