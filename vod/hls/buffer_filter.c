...
// 28 = ceil(188/7) + 1, 188 = ts packet size, 7 = min audio frame size (adts header)
#define BUFFERED_FRAMES_QUEUE_SIZE (28)
...
buffer_filter_start_frame(media_filter_context_t* context, output_frame_t* frame) {
	...
	default:
		vod_log_error(
			...
			0,
			"buffer_filter_start_frame: invalid state %d",
			state->cur_state
		);
...
buffer_filter_force_flush(media_filter_context_t* context, bool_t last_stream_frame) {
	...
	if (!state->align_frames) {
		...
		// when the frames are not aligned, need to write each frame separately since the pts that
		// is outputted may be the pts of some previous frame
		p = state->start_pos;

		while (state->buffered_frames.write_pos != state->buffered_frames.read_pos) {
			if (p > state->start_pos) {
				rc = mpegts_encoder_start_sub_frame(
					context, &state->buffered_frames.data[state->buffered_frames.read_pos].frame
				);
				if (rc != VOD_OK) {
					return rc;
				}
...
buffer_filter_write(media_filter_context_t* context, const u_char* buffer, uint32_t size) {
	...
	default:
		vod_log_error(
			...
			context->request_context->log,
			0,
			"buffer_filter_write: invalid state %d",
			state->cur_state
		);
...
vod_status_t
buffer_filter_init(media_filter_t* filter, media_filter_context_t* context, bool_t align_frames, uint32_t size) {
	...
	if (state == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "buffer_filter_init: vod_alloc failed (1)"
		);
	...
	*filter = buffer_filter;
	...
	if (state->start_pos == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "buffer_filter_init: vod_alloc failed (2)"
		);
		return VOD_ALLOC_FAILED;
	}
