...
// constants
// clang-format off
static const u_char avc_aud_nal_packet[] = {
	0x00, 0x00, 0x00, 0x01, // start code prefix
	0x09,                   // nal_unit_type = 9 (AUD), nal_ref_idc = 0
	0xF0,                   // primary_pic_type = 7 (all pic types), rbsp_stop_one_bit = 1
};

static const u_char hevc_aud_nal_packet[] = {
	0x00, 0x00, 0x00, 0x01, // start code prefix
	0x46, 0x01,             // nal_unit_type = 35 (AUD), nuh_layer_id = 0, temporal_id_plus1 = 1
	0x50,                   // pic_type = 2 (any slice type), rbsp_stop_one_bit = 1
};
// clang-format on

static const u_char nal_marker[] = {0x00, 0x00, 0x00, 0x01};

static const u_char zero_padding[64] = {0};

vod_status_t
mp4_to_annexb_set_media_info(media_filter_context_t* context, media_info_t* media_info) {
	...
	if (state->nal_packet_size_length < 1 || state->nal_packet_size_length > 4) {
		vod_log_error(
			...
			context->request_context->log,
			0,
			"mp4_to_annexb_set_media_info: invalid nal packet size length %uD",
...
mp4_to_annexb_simulation_supported(media_info_t* media_info) {
	/*
	 * When the packet size field length is 4 we can bound the output size - since every 4-byte
	 * length field is transformed to a \0\0\0\x01 or \0\0\x01 NAL marker, the output size is <= the
	 * input size. When the packet size field length is less than 4, the output size may be greater
	 * than input size by the number of NAL packets. Since we don't know this number in advance we
	 * have no way to bound the output size. Luckily, ffmpeg always uses 4 byte size fields - see
	 * ff_isom_write_avcc
	 */
...
mp4_to_annexb_start_frame(media_filter_context_t* context, output_frame_t* frame) {
	...
	if (frame->size > 0 && frame->size <= state->nal_packet_size_length) {
		vod_log_error(
			...
			context->request_context->log,
			0,
			"mp4_to_annexb_start_frame: invalid frame size %uD",
			frame->size
		);
		return VOD_BAD_DATA;
	}

	// not counting the AUD or extra data since they are written here
	...
	while (buffer < buffer_end) {
		...
		case STATE_PACKET_SIZE:
			...
			if (state->packet_size_left <= 0) {
				vod_log_error(
					VOD_LOG_ERR, context->request_context->log, 0, "mp4_to_annexb_write: zero size packet"
				);
				return VOD_BAD_DATA;
			}
...
#if (VOD_HAVE_OPENSSL_EVP)
			if (state->sample_aes) {
				rc = sample_aes_avc_start_nal_unit(context, unit_type, state->packet_size_left);
				if (rc != VOD_OK) {
					return rc;
				}
			}
	...
	if (state->nal_packet_size_length == 4 && !state->sample_aes) {
		if (state->frame_size_left < 0) {
			vod_log_error(
				...
				context->request_context->log,
				0,
				"mp4_to_annexb_flush_frame: frame exceeded the calculated size by %D bytes",
				-state->frame_size_left
			);
...
vod_status_t
mp4_to_annexb_init(
	media_filter_t* filter, media_filter_context_t* context, hls_encryption_params_t* encryption_params
) {
	...
	if (state == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "mp4_to_annexb_init: vod_alloc failed"
		);
	...
	if (encryption_params->type == HLS_ENC_SAMPLE_AES) {
		rc = sample_aes_avc_filter_init(filter, context, encryption_params->key, encryption_params->iv);
		if (rc != VOD_OK) {
			return rc;
		}
