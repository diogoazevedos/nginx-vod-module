...
frame_encrypt_start_frame(media_filter_context_t* context, output_frame_t* frame) {
	...
	if (state->max_encrypt_offset > CLEAR_LEAD_SIZE) {
		...
		if (1 != EVP_EncryptInit_ex(state->cipher, EVP_aes_128_cbc(), NULL, state->key, state->iv)) {
			vod_log_error(
				VOD_LOG_ERR, context->request_context->log, 0, "frame_encrypt_start_frame: EVP_EncryptInit_ex failed"
			);
...
frame_encrypt_write(media_filter_context_t* context, const u_char* buffer, uint32_t size) {
	...
	while (state->cur_offset < offset_limit) {
		...
		if (1 != EVP_EncryptUpdate(state->cipher, encrypted_buffer, &out_size, buffer, cur_size)) {
			vod_log_error(
				VOD_LOG_ERR, context->request_context->log, 0, "frame_encrypt_write: EVP_EncryptUpdate failed"
			);
...
frame_encrypt_filter_init(
	...
	if (state == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "frame_encrypt_filter_init: vod_alloc failed"
		);
		return VOD_ALLOC_FAILED;
	}
	...
	if (cln == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "frame_encrypt_filter_init: vod_pool_cleanup_add failed"
		);
		return VOD_ALLOC_FAILED;
	}
	...
	if (state->cipher == NULL) {
		vod_log_error(
			VOD_LOG_ERR, request_context->log, 0, "frame_encrypt_filter_init: EVP_CIPHER_CTX_new failed"
		);
		return VOD_ALLOC_FAILED;
	}
