...
static vod_status_t
hls_muxer_init_track(hls_muxer_state_t* state, hls_muxer_stream_state_t* cur_stream, media_track_t* track) {
	vod_status_t rc;
	...
	cur_stream->first_frame_time_offset =
		hls_rescale_millis(track->clip_start_time) + track->first_frame_time_offset;
	cur_stream->clip_from_frame_offset = track->clip_from_frame_offset;

	switch (track->media_info.media_type) {
	case MEDIA_TYPE_VIDEO:
		rc = mp4_to_annexb_set_media_info(&cur_stream->filter_context, &track->media_info);
		if (rc != VOD_OK) {
			return rc;
		}
		...
		if (state->align_pts) {
			cur_stream->first_frame_time_offset -=
				vod_min(INITIAL_DTS, track->media_info.u.video.initial_pts_delay);
		}
		break;

	case MEDIA_TYPE_AUDIO:
		if (track->media_info.codec_id == VOD_CODEC_ID_AAC) {
			rc = adts_encoder_set_media_info(&cur_stream->filter_context, &track->media_info);
			if (rc != VOD_OK) {
				return rc;
			}
		}
...
static bool_t
hls_muxer_simulation_supported(media_set_t* media_set, hls_encryption_params_t* encryption_params) {
	media_track_t* track;
...
hls_muxer_init_id3_stream(
	...
	rc = hls_muxer_init_stream(state, conf, cur_stream, NULL, init_streams_state);
	if (rc != VOD_OK) {
		return rc;
	}
	...
	if (context == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, state->request_context->log, 0, "hls_muxer_init_id3_stream: vod_alloc failed"
		);
...
hls_muxer_init_base(
	...
	*simulation_supported = hls_muxer_simulation_supported(media_set, encryption_params);
	...
	rc = mpegts_encoder_init_streams(
		request_context, encryption_params, &init_streams_state, segment_index
	);
	if (rc != VOD_OK) {
		return rc;
	}
	...
	state->first_stream = vod_alloc(
		request_context->pool, sizeof(*state->first_stream) * (media_set->total_track_count + 1)
	);
	if (state->first_stream == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "hls_muxer_init_base: vod_alloc failed"
		);
		return VOD_ALLOC_FAILED;
	}
	...
	for (cur_stream = state->first_stream; cur_stream < state->last_stream; cur_stream++, track++) {
		rc = hls_muxer_init_stream(state, conf, cur_stream, track, &init_streams_state);
		if (rc != VOD_OK) {
			return rc;
		}
		...
		case MEDIA_TYPE_VIDEO:
			...
			case MDP_MIN:
				if (track->media_info.duration_millis > 0
				    && (state->video_duration == 0
				        || track->media_info.duration_millis < state->video_duration)) {
					state->video_duration = track->media_info.duration_millis;
				}
				break;
			}

			rc = mp4_to_annexb_init(&cur_stream->filter, &cur_stream->filter_context, encryption_params);
			if (rc != VOD_OK) {
				return rc;
			}
			break;
		...
		case MEDIA_TYPE_AUDIO:
			if (conf->interleave_frames) {
				...
				rc = frame_joiner_init(&cur_stream->filter, &cur_stream->filter_context);
				if (rc != VOD_OK) {
					return rc;
				}
			} else {
				...
				rc = buffer_filter_init(
					&cur_stream->filter, &cur_stream->filter_context, conf->align_frames, DEFAULT_PES_PAYLOAD_SIZE
				);
				if (rc != VOD_OK) {
					return rc;
				}
			...
			if (track->media_info.codec_id == VOD_CODEC_ID_AAC) {
				rc = adts_encoder_init(&cur_stream->filter, &cur_stream->filter_context);
				if (rc != VOD_OK) {
					return rc;
				}
			}
			...
			if (encryption_params->type == HLS_ENC_SAMPLE_AES) {
				...
				rc = frame_encrypt_filter_init(
					&cur_stream->filter, &cur_stream->filter_context, encryption_params
				);
				if (rc != VOD_OK) {
					return rc;
				}
				...
				if (track->media_info.codec_id == VOD_CODEC_ID_EAC3) {
					rc = eac3_encrypt_filter_init(&cur_stream->filter, &cur_stream->filter_context);
					if (rc != VOD_OK) {
						return rc;
					}
				}
...
hls_muxer_init_segment(
	...
	if (state == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "hls_muxer_init_segment: vod_alloc failed"
		);
		return VOD_ALLOC_FAILED;
	}

	// init the write queue
	write_buffer_queue_init(&state->queue, request_context, write_callback, write_context, reuse_buffers);

	rc = hls_muxer_init_base(
		state, request_context, conf, encryption_params, segment_index, media_set, &simulation_supported, response_header
	);
	if (rc != VOD_OK) {
		return rc;
	}
...
hls_muxer_choose_stream(hls_muxer_state_t* state, hls_muxer_stream_state_t** result) {
	...
	for (;;) {
		for (cur_stream = state->first_stream; cur_stream < state->last_stream; cur_stream++) {
			...
			if (min_dts == NULL
			    || cur_stream->next_frame_time_offset < min_dts->next_frame_time_offset) {
				min_dts = cur_stream;
			}
		}
...
hls_muxer_simulation_flush_delayed_streams(
	hls_muxer_state_t* state, hls_muxer_stream_state_t* selected_stream, uint64_t frame_dts
) {
	hls_muxer_stream_state_t* cur_stream;
	uint64_t buffer_dts;
	...
	for (cur_stream = state->first_stream; cur_stream < state->last_stream; cur_stream++) {
		if (selected_stream == cur_stream
		    || cur_stream->filter_context.context[MEDIA_FILTER_BUFFER] == NULL) {
			continue;
		}
...
hls_muxer_simulation_write_frame(
	hls_muxer_stream_state_t* selected_stream, input_frame_t* cur_frame, uint64_t cur_frame_dts, bool_t last_frame
) {
	output_frame_t output_frame;
...
static void
hls_muxer_simulation_set_segment_limit(hls_muxer_state_t* state, uint64_t segment_end, uint32_t timescale) {
	hls_muxer_stream_state_t* cur_stream;

	for (cur_stream = state->first_stream; cur_stream < state->last_stream; cur_stream++) {
		cur_stream->segment_limit =
			(segment_end * HLS_TIMESCALE) / timescale - cur_stream->clip_from_frame_offset;
		cur_stream->is_first_segment_frame = TRUE;
	}
}

static void
hls_muxer_simulation_set_segment_limit_unlimited(hls_muxer_state_t* state) {
	hls_muxer_stream_state_t* cur_stream;
...
hls_muxer_simulate_get_iframes(
	...
	rc = hls_muxer_init_base(
		&state, request_context, muxer_conf, encryption_params, 0, media_set, &simulation_supported, NULL
	);
	if (rc != VOD_OK) {
		return rc;
	}
	...
	for (;;) {
		...
		hls_muxer_simulation_write_frame(selected_stream, cur_frame, cur_frame_dts, last_frame);
		...
		if (!selected_stream->is_first_segment_frame && selected_stream->prev_key_frame) {
			// get the frame time
			cur_frame_time =
				rescale_time(selected_stream->prev_frame_pts, HLS_TIMESCALE, 1000); // in millis
			if (frame_size != 0) {
				if (cur_frame_time > frame_start_time) {
					callback(
						context, frame_segment_index, cur_frame_time - frame_start_time, frame_start, frame_size
					);
				}
			} else {
		...
		if (last_frame && cur_frame[0].key_frame) {
			// get the frame time
			cur_frame_time =
				rescale_time(cur_frame_time_offset + cur_frame[0].pts_delay, HLS_TIMESCALE, 1000); // in millis
			if (frame_size != 0) {
				if (cur_frame_time > frame_start_time) {
					callback(
						context, frame_segment_index, cur_frame_time - frame_start_time, frame_start, frame_size
	...
	if (state->media_set->clip_count > 1) {
		...
		if (rc != VOD_OK) {
			vod_log_error(
				...
				state->request_context->log,
				0,
				"hls_muxer_simulation_reset: unexpected - hls_muxer_reinit_tracks failed %i",
				rc
			);
		}
	} else {
