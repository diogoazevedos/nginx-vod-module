...
vod_status_t
sample_aes_avc_filter_init(media_filter_t* filter, media_filter_context_t* context, u_char* key, u_char* iv) {
	...
	if (state == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "sample_aes_avc_filter_init: vod_alloc failed"
		);
	...
	if (cln == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "sample_aes_avc_filter_init: vod_pool_cleanup_add failed"
		);
		return VOD_ALLOC_FAILED;
	}
	...
	if (state->cipher == NULL) {
		vod_log_error(
			VOD_LOG_ERR, request_context->log, 0, "sample_aes_avc_filter_init: EVP_CIPHER_CTX_new failed"
		);
		return VOD_ALLOC_FAILED;
	}
...
static vod_status_t
sample_aes_avc_write_emulation_prevention(media_filter_context_t* context, const u_char* buffer, uint32_t size) {
...
vod_status_t
sample_aes_avc_start_nal_unit(media_filter_context_t* context, int unit_type, uint32_t unit_size) {
	sample_aes_avc_filter_state_t* state = get_context(context);

	if ((unit_type != AVC_NAL_SLICE && unit_type != AVC_NAL_IDR_SLICE)
	    || unit_size <= MAX_UNENCRYPTED_UNIT_SIZE) {
		state->encrypt = FALSE;
		return VOD_OK;
	}
	...
	if (1 != EVP_EncryptInit_ex(state->cipher, EVP_aes_128_cbc(), NULL, state->key, state->iv)) {
		vod_log_error(
			VOD_LOG_ERR, context->request_context->log, 0, "sample_aes_avc_start_nal_unit: EVP_EncryptInit_ex failed"
		);
		return VOD_ALLOC_FAILED;
	}
...
vod_status_t
sample_aes_avc_filter_write_nal_body(media_filter_context_t* context, const u_char* buffer, uint32_t size) {
	sample_aes_avc_filter_state_t* state = get_context(context);
	uint32_t end_offset;
	uint32_t cur_size;
	...
	for (end_offset = state->cur_offset + size; state->cur_offset < end_offset;
		...
		if (1 != EVP_EncryptUpdate(state->cipher, encrypted_block, &out_size, buffer, cur_size)) {
			vod_log_error(
				VOD_LOG_ERR, context->request_context->log, 0, "sample_aes_avc_filter_write_nal_body: EVP_EncryptUpdate failed"
			);
