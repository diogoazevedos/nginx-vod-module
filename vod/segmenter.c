...
segmenter_init_config(segmenter_conf_t* conf, vod_pool_t* pool) {
	...
	if (conf->segment_duration < MIN_SEGMENT_DURATION || conf->segment_duration > MAX_SEGMENT_DURATION) {
		return VOD_BAD_DATA;
	}
	...
	conf->max_segment_duration = vod_max(conf->segment_duration, conf->max_bootstrap_segment_duration);
...
segmenter_get_segment_count_last_short(segmenter_conf_t* conf, uint64_t duration_millis) {
	...
	if (duration_millis > conf->bootstrap_segments_total_duration) {
		...
		if (duration_millis
		    > (uint64_t)conf->segment_duration * (UINT_MAX - conf->bootstrap_segments_count - 2)) {
...
segmenter_get_segment_count_last_rounded(segmenter_conf_t* conf, uint64_t duration_millis) {
	...
	if (duration_millis >= conf->bootstrap_segments_total_duration) {
		...
		result = conf->bootstrap_segments_count
		       + (duration_millis + conf->segment_duration / 2) / conf->segment_duration;
		if (result < 1) {
			result = 1;
		}
	} else {
...
segmenter_get_start_end_offsets(
	segmenter_conf_t* conf, uint32_t segment_index, uint64_t* start, uint64_t* end
) {
	if (segment_index < conf->bootstrap_segments_count) {
		*start = conf->bootstrap_segments_start[segment_index];
		*end = conf->bootstrap_segments_end[segment_index];
...
uint32_t
segmenter_get_segment_index_no_discontinuity(segmenter_conf_t* conf, uint64_t time_millis) {
	...
	for (cur_duration = timing->durations;; cur_duration++) {
		if (cur_duration >= end_duration) {
			vod_log_error(
				...
				request_context->log,
				0,
				"segmenter_get_segment_index_discontinuity: invalid segment time %uD (1)",
				time_millis
			);
		...
		if (time_millis < clip_time) {
			vod_log_error(
				...
				request_context->log,
				0,
				"segmenter_get_segment_index_discontinuity: invalid segment time %uD (2)",
				time_millis
			);
			return VOD_BAD_REQUEST;
		}
		...
		if (clip_segment_limit == INVALID_SEGMENT_COUNT) {
			vod_log_error(
				VOD_LOG_ERR, request_context->log, 0, "segmenter_get_segment_index_discontinuity: segment count is invalid"
			);
			return VOD_BAD_DATA;
		}
...
segmenter_get_start_end_ranges_gop(get_clip_ranges_params_t* params, get_clip_ranges_result_t* result) {
	...
	for (cur_duration = clip_durations, cur_clip_time = params->timing.times;;
		...
		if (cur_duration >= end_duration) {
			vod_log_error(
				...
				request_context->log,
				0,
				"segmenter_get_start_end_ranges_gop: invalid time %uL (1)",
				time
			);
			return VOD_BAD_REQUEST;
		}
		...
		if (time < clip_time) {
			vod_log_error(
				...
				request_context->log,
				0,
				"segmenter_get_start_end_ranges_gop: invalid time %uL (2)",
				time
			);
			return VOD_BAD_REQUEST;
		}
	...
	if (cur_clip_range == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "segmenter_get_start_end_ranges_gop: vod_alloc failed"
		);
		return VOD_ALLOC_FAILED;
	}
...
segmenter_get_start_end_ranges_no_discontinuity(
	...
	if (segment_count == INVALID_SEGMENT_COUNT) {
		vod_log_error(
			...
			request_context->log,
			0,
			"segmenter_get_start_end_ranges_no_discontinuity: segment count is invalid for total duration %uL",
			end_time
		);
		return VOD_BAD_DATA;
	}
	...
	segmenter_get_start_end_offsets(params->conf, params->segment_index, &start, &end);
	...
	if (params->key_frame_durations != NULL) {
		...
		end = segmenter_align_to_key_frames(
			&align_context, end, last_segment_end != ULLONG_MAX ? last_segment_end + 1 : last_segment_end
		);
	...
	cur_clip_range =
		vod_alloc(request_context->pool, sizeof(result->clip_ranges[0]) * result->clip_count);
	if (cur_clip_range == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "segmenter_get_start_end_ranges_no_discontinuity: vod_alloc failed"
		);
		return VOD_ALLOC_FAILED;
	}
	...
	clip_initial_segment_index = segmenter_get_segment_index_no_discontinuity(
		params->conf, cur_clip_range->original_clip_time - segment_base_time
	);
...
segmenter_get_start_end_ranges_discontinuity(
	...
	if (params->timing.segment_base_time == SEGMENT_BASE_TIME_RELATIVE) {
		...
		for (cur_duration = params->timing.durations;; cur_duration++) {
			if (cur_duration >= end_duration) {
				vod_log_error(
					...
					request_context->log,
					0,
					"segmenter_get_start_end_ranges_discontinuity: invalid segment index %uD (1)",
					segment_index
				);
				return VOD_BAD_REQUEST;
			}
			...
			if (cur_segment_limit == INVALID_SEGMENT_COUNT) {
				vod_log_error(
					VOD_LOG_ERR, request_context->log, 0, "segmenter_get_start_end_ranges_discontinuity: invalid segment count"
				);
				return VOD_BAD_DATA;
			}
		...
		segmenter_get_start_end_offsets(conf, segment_index, &start, &end);
		...
		for (cur_duration = params->timing.durations, cur_clip_time = params->timing.times;;
			...
			if (cur_duration >= end_duration) {
				vod_log_error(
					...
					request_context->log,
					0,
					"segmenter_get_start_end_ranges_discontinuity: invalid segment index %uD (2)",
					segment_index
				);
				return VOD_BAD_REQUEST;
			}
		...
		clip_initial_segment_index = segmenter_get_segment_index_no_discontinuity(
			conf, params->timing.original_times[clip_index] - params->timing.segment_base_time
		);

		cur_segment_limit =
			conf->get_segment_count(conf, clip_time + clip_duration - params->timing.segment_base_time);
		if (cur_segment_limit == INVALID_SEGMENT_COUNT) {
			vod_log_error(
				VOD_LOG_ERR, request_context->log, 0, "segmenter_get_start_end_ranges_discontinuity: segment count is invalid"
			);
			return VOD_BAD_DATA;
		}
	}
	...
	if (cur_clip_range == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "segmenter_get_start_end_ranges_discontinuity: vod_alloc failed"
		);
		return VOD_ALLOC_FAILED;
	}
...
segmenter_get_live_window_start_end(
	...
	if (end_clip_index == UINT_MAX) {
		...
		if (end_time <= timing->first_time) {
			vod_log_error(
				...
				request_context->log,
				0,
				"segmenter_get_live_window_start_end: end time %uL smaller than first clip time %uL",
				end_time,
				timing->first_time
			);
			return VOD_BAD_MAPPING;
	...
	if (!media_set->presentation_end
		...
		if (end_time <= timing->times[end_clip_index]) {
			...
			if (end_clip_index <= 0) {
				vod_log_error(
					VOD_LOG_ERR, request_context->log, 0, "segmenter_get_live_window_start_end: empty window (2)"
				);
				return VOD_BAD_MAPPING;
			}
			end_clip_index--;
	...
	if (!media_set->original_use_discontinuity
		...
		start_time = segment_base_time
		           + vod_div_ceil(start_time - segment_base_time, conf->segment_duration)
		...
		if (sequence->key_frame_durations != NULL && start_time > timing->times[start_clip_index]) {
			align_context.request_context = request_context;
			align_context.part = sequence->key_frame_durations;
			align_context.offset = timing->first_time + sequence->first_key_frame_offset;
			align_context.cur_pos = align_context.part->first;
		...
		if (start_time >= clip_end_time) {
			...
			if (start_clip_index > end_clip_index) {
				vod_log_error(
					VOD_LOG_ERR, request_context->log, 0, "segmenter_get_live_window_start_end: empty window (1)"
				);
				return VOD_BAD_MAPPING;
			}
			start_clip_offset = 0;
	...
	if (end_time <= start_time) {
		vod_log_error(
			VOD_LOG_ERR, request_context->log, 0, "segmenter_get_live_window_start_end: empty window (3)"
		);
		return VOD_BAD_MAPPING;
	}
...
segmenter_get_live_window(
	...
	if (media_set->use_discontinuity) {
		rc = segmenter_get_live_window_start_end(request_context, conf, media_set, timing, &window);
		if (rc != VOD_OK) {
			return rc;
		}
		...
		if (timing->segment_base_time == SEGMENT_BASE_TIME_RELATIVE) {
			...
			for (durations_cur = timing->durations; durations_cur < durations_end; durations_cur++) {
				media_set->initial_segment_index +=
					vod_div_ceil(*durations_cur, conf->segment_duration);
			}

			media_set->initial_segment_clip_relative_index =
				window.start_clip_offset / conf->segment_duration;
			media_set->initial_segment_index += media_set->initial_segment_clip_relative_index;
		} else {
			clip_initial_segment_index = segmenter_get_segment_index_no_discontinuity(
				conf, timing->times[window.start_clip_index] - timing->segment_base_time
			);
			initial_segment_index = segmenter_get_segment_index_no_discontinuity(
				conf, window.start_time - timing->segment_base_time
			);
			media_set->initial_segment_clip_relative_index =
				initial_segment_index - clip_initial_segment_index;
		}
	} else {
		...
		rc = segmenter_get_live_window_start_end(request_context, conf, media_set, &temp_timing, &window);
		if (rc != VOD_OK) {
			return rc;
		}
		...
		for (; window.end_clip_offset > *durations_cur; durations_cur++) {
			window.end_clip_offset -= *durations_cur;
	...
	if (parse_all_clips) {
		...
		if (timing->total_count > MAX_CLIPS_PER_REQUEST) {
			vod_log_error(
				...
				request_context->log,
				0,
				"segmenter_get_live_window: clip count %uD exceeds the limit per request",
				timing->total_count
			);
			return VOD_BAD_REQUEST;
		}
...
segmenter_get_segment_durations_add(
	segmenter_get_segment_durations_context_t* context, uint32_t segment_duration, uint32_t segment_count
) {
	segment_duration_item_t* cur_item;
	...
	if (context->align.part != NULL) {
		next_aligned_time =
			segmenter_align_to_key_frames(&context->align, context->cur_time, context->clip_end_time);
...
segmenter_get_segment_durations_estimate_internal(
	...
	if (sequence->key_frame_durations != NULL) {
		...
		alloc_count = conf->bootstrap_segments_count
		            + 2 * timing->total_count
	...
	if (result->items == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "segmenter_get_segment_durations_estimate_internal: vod_alloc failed"
		);
		return VOD_ALLOC_FAILED;
	}
	...
	for (;;) {
		...
		if (timing->segment_base_time != SEGMENT_BASE_TIME_RELATIVE) {
			...
			new_segment_index = segmenter_get_segment_index_no_discontinuity(
				conf, *cur_clip_time - timing->segment_base_time
			);
			...
			clip_segment_limit =
				conf->get_segment_count(conf, context.clip_end_time - timing->segment_base_time);
			if (clip_segment_limit == INVALID_SEGMENT_COUNT) {
				vod_log_error(
					VOD_LOG_ERR,
					request_context->log,
			...
			clip_segment_limit = clip_segment_limit > context.segment_index ? clip_segment_limit - 1
			...
			if (context.segment_index < clip_segment_limit) {
				...
				segmenter_get_segment_durations_add(&context, segment_duration, 1);
			}
		} else {
			...
			// get segment limit for the current clip
			clip_segment_limit =
				conf->get_segment_count(conf, segment_start + alignment_offset + cur_clip_duration);
			if (clip_segment_limit == INVALID_SEGMENT_COUNT) {
				vod_log_error(
					VOD_LOG_ERR,
					request_context->log,
		...
		segment_count =
			sequence->key_frame_durations != NULL ? 1 : clip_segment_limit - context.segment_index;
		while (context.segment_index < clip_segment_limit) {
			segmenter_get_segment_durations_add(&context, segment_duration, segment_count);
		}

		// last segment
		segmenter_get_segment_durations_add(&context, context.clip_end_time - context.cur_time, 1);
	...
	if (result->segment_count > MAX_SEGMENT_COUNT) {
		vod_log_error(
			...
			request_context->log,
			0,
			"segmenter_get_segment_durations_estimate_internal: segment count %uD is invalid",
...
segmenter_get_segment_durations_estimate(
	...
	if (media_set->timing.durations != NULL) {
		...
		if (media_set->use_discontinuity) {
			...
			result->end_time = media_set->timing.times[media_set->timing.total_count - 1]
		...
		duration_millis =
			segmenter_get_total_duration(conf, media_set, sequence, sequences_end, media_type);
	}
	...
	return segmenter_get_segment_durations_estimate_internal(
		request_context, conf, &timing, duration_millis, media_set->initial_segment_index, sequence, result
	);
}

static void
segmenter_boundary_iterator_init(
	segmenter_boundary_iterator_context_t* context, segmenter_conf_t* conf, uint32_t segment_count
) {
	context->conf = conf;
...
segmenter_boundary_iterator_skip(segmenter_boundary_iterator_context_t* context, uint32_t count) {
	...
	if (context->segment_index > context->conf->bootstrap_segments_count) {
		context->last_boundary = context->conf->bootstrap_segments_total_duration
		                       + (context->segment_index - context->conf->bootstrap_segments_count)
		                             * context->conf->segment_duration;
	}
}
...
segmenter_get_segment_durations_accurate(
	...
	if (media_set->timing.durations != NULL) {
		...
		return segmenter_get_segment_durations_estimate(
			request_context, conf, media_set, sequence, media_type, result
		);
	}
			...
			if (ref_track == NULL) {
				...
				case MDP_MIN:
					if (cur_track->media_info.duration_millis > 0
					    && (duration_millis == 0
					        || cur_track->media_info.duration_millis < duration_millis)) {
						ref_track = cur_track;
						duration_millis = cur_track->media_info.duration_millis;
					}
					break;
	...
	if (main_track == NULL) {
		vod_log_error(
			VOD_LOG_ERR, request_context->log, 0, "segmenter_get_segment_durations_accurate: didn't get any tracks"
		);
	...
	if (result->segment_count > MAX_SEGMENT_COUNT) {
		vod_log_error(
			...
			request_context->log,
			0,
			"segmenter_get_segment_durations_accurate: segment count %uD is invalid",
			result->segment_count
		);
		return VOD_BAD_DATA;
	}
	...
	if (result->items == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "segmenter_get_segment_durations_accurate: vod_alloc failed"
		);
		return VOD_ALLOC_FAILED;
	}
	...
	if (conf->bootstrap_segments_count > 0) {
		...
		for (; cur_frame < last_frame; cur_frame++) {
			while (accum_duration >= segment_limit
				...
				segment_limit =
					rescale_time(conf->bootstrap_segments_end[segment_index], 1000, result->timescale);
			}
...
post_bootstrap:
	...
	if (main_track->media_info.duration_millis < duration_millis && !align_to_key_frames) {
		...
		total_duration = rescale_time(
			ref_track->media_info.duration, ref_track->media_info.timescale, result->timescale
		);

		while (accum_duration < total_duration && segment_index + 1 < result->segment_count) {
