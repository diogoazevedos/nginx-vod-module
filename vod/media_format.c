...
media_format_finalize_track(request_context_t* request_context, int parse_type, media_info_t* media_info) {
	...
	case MEDIA_TYPE_VIDEO:
		...
		if (media_info->codec_id == VOD_CODEC_ID_AVC) {
			rc = avc_parser_parse_extra_data(parser_ctx, &media_info->extra_data, NULL, NULL);
			if (rc != VOD_OK) {
				return rc;
			}

			media_info->u.video.transfer_characteristics =
				avc_parser_get_transfer_characteristics(parser_ctx);
		} else {
			rc = hevc_parser_parse_extra_data(parser_ctx, &media_info->extra_data, NULL, NULL);
			if (rc != VOD_OK) {
				return rc;
			}
	...
	if ((parse_type & PARSE_FLAG_CODEC_NAME) != 0) {
		...
		if (media_info->codec_name.data == NULL) {
			vod_log_error(
				VOD_LOG_ERR, request_context->log, 0, "media_format_finalize_track: failed to allocate codec name"
			);
		...
		if (rc != VOD_OK) {
			vod_log_debug0(
				VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "media_format_finalize_track: failed to get codec name"
			);
			return rc;
		}
	}
	...
	if ((parse_type & (PARSE_FLAG_EXTRA_DATA | PARSE_FLAG_EXTRA_DATA_SIZE)) != 0) {
		if ((parse_type & PARSE_FLAG_EXTRA_DATA_PARSE) != 0
			...
			if (get_nal_units != NULL) {
				...
				if (rc != VOD_OK) {
					vod_log_debug1(
						...
						request_context->log,
						0,
						"media_format_finalize_track: get_nal_units failed %i",
						rc
					);
					return rc;
				}
			...
			new_extra_data =
				vod_alloc(request_context->pool, media_info->extra_data.len + VOD_BUFFER_PADDING_SIZE);
			if (new_extra_data == NULL) {
				vod_log_debug0(
					VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "media_format_finalize_track: vod_alloc failed"
				);
				return VOD_ALLOC_FAILED;
			}
