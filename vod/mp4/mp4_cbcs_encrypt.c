...
typedef struct {
	vod_status_t (*init_ctx)(request_context_t* request_context, void** ctx);

	vod_status_t (*parse_extra_data)(
		void* ctx, vod_str_t* extra_data, uint32_t* nal_packet_size_length, uint32_t* min_packet_size
	);

	vod_status_t (*is_slice)(void* ctx, uint8_t nal_type, bool_t* is_slice);

	vod_status_t (*get_slice_header_size)(void* ctx, const u_char* buffer, uint32_t size, uint32_t* result);
...
mp4_cbcs_encrypt_init_cipher(mp4_cbcs_encrypt_state_t* state) {
	...
	if (cln == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "mp4_cbcs_encrypt_init_cipher: vod_pool_cleanup_add failed"
		);
	...
	if (state->cipher == NULL) {
		vod_log_error(
			VOD_LOG_ERR, request_context->log, 0, "mp4_cbcs_encrypt_init_cipher: EVP_CIPHER_CTX_new failed"
		);
		return VOD_ALLOC_FAILED;
	}
...
mp4_cbcs_encrypt_reset_cipher(mp4_cbcs_encrypt_state_t* state) {
	if (1 != EVP_EncryptInit_ex(state->cipher, EVP_aes_128_cbc(), NULL, state->key, state->iv)) {
		vod_log_error(
			VOD_LOG_ERR, state->request_context->log, 0, "mp4_cbcs_encrypt_reset_cipher: EVP_EncryptInit_ex failed"
		);
		return VOD_ALLOC_FAILED;
	}
...
static vod_status_t
mp4_cbcs_encrypt_write_encrypted(mp4_cbcs_encrypt_state_t* state, u_char* buffer, uint32_t size) {
	vod_status_t rc;
	u_char* output;
	size_t output_size;
	int written;
	...
	rc = write_buffer_get_bytes(
		&state->write_buffer, aes_round_up_to_block_exact(size), &output_size, &output
	);
	if (rc != VOD_OK) {
		return rc;
	}

	if (1 != EVP_EncryptUpdate(state->cipher, output, &written, buffer, size)) {
		vod_log_error(
			VOD_LOG_ERR, state->request_context->log, 0, "mp4_cbcs_encrypt_write_encrypted: EVP_EncryptUpdate failed"
		);
...
mp4_cbcs_encrypt_init_stream_state(
	...
	stream_state->last_track =
		media_set->filtered_tracks + (media_set->total_track_count * media_set->clip_count);
...
mp4_cbcs_encrypt_start_frame(mp4_cbcs_encrypt_stream_state_t* stream_state) {
	...
	if (stream_state->cur_frame >= stream_state->last_frame) {
		vod_log_error(
			VOD_LOG_ERR, stream_state->state->request_context->log, 0, "mp4_cbcs_encrypt_start_frame: no more frames"
		);
		return VOD_BAD_DATA;
	}
	...
	while (cur_pos < buffer_end) {
		...
		case STATE_PACKET_SIZE:
			if (stream_state->base.frame_size_left <= 0) {
				...
				if (stream_state->base.frame_size_left < stream_state->min_packet_size) {
					vod_log_error(
						...
						stream_state->base.frame_size_left,
						stream_state->min_packet_size
					);
					return VOD_BAD_DATA;
				}
			...
			if (stream_state->packet_size_left <= 0) {
				vod_log_error(
					VOD_LOG_ERR, state->request_context->log, 0, "mp4_cbcs_encrypt_video_write_buffer: zero size packet"
				);
				return VOD_BAD_DATA;
			}

			if (stream_state->packet_size_left
			...
			stream_state->base.frame_size_left -=
				stream_state->nal_packet_size_length + stream_state->packet_size_left;
		...
		case STATE_NAL_TYPE:
			...
			rc = write_buffer_get_bytes(
				&state->write_buffer, stream_state->nal_packet_size_length + 1, NULL, &output
			);
			if (rc != VOD_OK) {
				return rc;
			}
			...
			*output++ = nal_type; // NAL type
			...
			rc = stream_state->slice_parser.is_slice(
				stream_state->slice_parser_context, nal_type, &is_slice
			);
			if (rc != VOD_OK) {
				return rc;
			}
		...
		case STATE_SLICE_HEADER:
			...
			stream_state->slice_header_pos =
				vod_copy(stream_state->slice_header_pos, cur_pos, write_size);
			cur_pos += write_size;
			...
			if (stream_state->packet_size_left - slice_header_size < AES_BLOCK_SIZE) {
				...
				rc = write_buffer_write(
					&state->write_buffer, stream_state->slice_header + 1, slice_header_buf_size - 1
				);
				if (rc != VOD_OK) {
					return rc;
				}
			...
			rc = write_buffer_write(
				&state->write_buffer, stream_state->slice_header + 1, slice_header_size
			);
			if (rc != VOD_OK) {
				return rc;
			}
			...
			rc = mp4_cbcs_encrypt_write_encrypted(
				state, stream_state->slice_header + 1 + slice_header_size, write_size
			);
			if (rc != VOD_OK) {
				return rc;
			}
		...
		case STATE_PACKET_ENCRYPT:
			if (stream_state->packet_size_left > 0
				...
				size_left = buffer_end - cur_pos;
				write_size = stream_state->packet_size_left
				           - (stream_state->next_block_size_left - AES_BLOCK_SIZE);
				write_size = vod_min(write_size, size_left);

				rc = mp4_cbcs_encrypt_write_encrypted(state, cur_pos, write_size);
				if (rc != VOD_OK) {
					return rc;
				}
...
mp4_cbcs_encrypt_video_get_fragment_writer(
	mp4_cbcs_encrypt_state_t* state, media_set_t* media_set, media_track_t* track, segment_writer_t* segment_writer
) {
	mp4_cbcs_encrypt_video_stream_state_t* stream_state;
	request_context_t* request_context = state->request_context;
	vod_status_t rc;
	...
	if (stream_state == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "mp4_cbcs_encrypt_video_get_fragment_writer: vod_alloc failed"
		);
		return VOD_ALLOC_FAILED;
	}
	...
	mp4_cbcs_encrypt_init_stream_state(&stream_state->base, state, media_set, track);
...
mp4_cbcs_encrypt_audio_get_fragment_writer(
	...
	if (stream_state == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "mp4_cbcs_encrypt_audio_get_fragment_writer: vod_alloc failed"
		);
		return VOD_ALLOC_FAILED;
	}

	mp4_cbcs_encrypt_init_stream_state(stream_state, state, media_set, track);
...
mp4_cbcs_encrypt_get_writers(
	...
	state = vod_alloc(
		request_context->pool, sizeof(*state) + sizeof(segment_writers[0]) * media_set->total_track_count
	);
	if (state == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "mp4_cbcs_encrypt_get_writers: vod_alloc failed"
		);
		return VOD_ALLOC_FAILED;
	}
	...
	write_buffer_init(
		&state->write_buffer, request_context, segment_writer->write_tail, segment_writer->context, FALSE
	);
	...
	for (i = 0; i < media_set->total_track_count; i++) {
		...
		case MEDIA_TYPE_VIDEO:
			rc = mp4_cbcs_encrypt_video_get_fragment_writer(
				state, media_set, cur_track, cur_segment_writer
			);
			break;
