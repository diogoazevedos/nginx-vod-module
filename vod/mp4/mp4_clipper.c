...
#define write_full_atom(write_context, index, atom)                                    \
	if (full_atom_size(atom) > 0) {                                                    \
		mp4_clipper_write_tail(                                                        \
			write_context, index, (u_char*)full_atom_start(atom), full_atom_size(atom) \
		);                                                                             \
	}
...
mp4_clipper_clip_duration(
	request_context_t* request_context, media_parse_params_t* parse_params, uint64_t* duration, uint32_t timescale
) {
	uint64_t clip_from;
	uint64_t length;

	if (timescale == 0) {
		vod_log_error(VOD_LOG_ERR, request_context->log, 0, "mp4_clipper_clip_duration: timescale is zero");
		return VOD_BAD_DATA;
	}
	...
	if (*duration < clip_from) {
		vod_log_error(
			...
			request_context->log,
			0,
			"mp4_clipper_clip_duration: duration %uL less than clip from %uL",
			*duration,
			clip_from
		);
...
mp4_clipper_mvhd_clip_data(
	process_moov_context_t* context, atom_info_t* atom_info, mvhd_clip_result_t* result, uint32_t* timescale
) {
	...
	if (atom_info->size < sizeof(*atom)) {
		vod_log_error(
			...
			0,
			"mp4_clipper_mvhd_clip_data: atom size %uL too small",
			atom_info->size
		);
		return VOD_BAD_DATA;
	}
	...
	if (atom->version[0] == 1) {
		if (atom_info->size < sizeof(*atom64)) {
			vod_log_error(
				...
				context->request_context->log,
				0,
				"mp4_clipper_mvhd_clip_data: atom size %uL too small (2)",
				atom_info->size
			);
			return VOD_BAD_DATA;
		}
...
// tkhd
static vod_status_t
mp4_clipper_tkhd_clip_data(process_moov_context_t* context, atom_info_t* atom_info, tkhd_clip_result_t* result) {
	...
	if (atom_info->size < sizeof(*atom)) {
		vod_log_error(
			...
			context->request_context->log,
			0,
			"mp4_clipper_tkhd_clip_data: atom size %uL too small (1)",
			atom_info->size
		);
		return VOD_BAD_DATA;
	}
	...
	if (atom->version[0] == 1) {
		if (atom_info->size < sizeof(*atom64)) {
			vod_log_error(
				...
				context->request_context->log,
				0,
				"mp4_clipper_tkhd_clip_data: atom size %uL too small (2)",
				atom_info->size
			);
			return VOD_BAD_DATA;
		}
	...
	rc = mp4_clipper_clip_duration(
		context->request_context, &context->parse_params, &duration, context->mvhd_timescale
	);
	if (rc != VOD_OK) {
		return rc;
	}
...
mp4_clipper_mdhd_clip_data(
	parse_trak_atom_context_t* context, atom_info_t* atom_info, mdhd_clip_result_t* result, uint32_t* timescale
) {
	...
	if (atom_info->size < sizeof(*atom)) {
		vod_log_error(
			...
			context->request_context->log,
			0,
			"mp4_clipper_mdhd_clip_data: atom size %uL too small (1)",
			atom_info->size
		);
		return VOD_BAD_DATA;
	}
	...
	if (atom->version[0] == 1) {
		if (atom_info->size < sizeof(*atom64)) {
			vod_log_error(
				...
				context->request_context->log,
				0,
				"mp4_clipper_mdhd_clip_data: atom size %uL too small (2)",
				atom_info->size
			);
			return VOD_BAD_DATA;
		}
...
mp4_clipper_stts_clip_data(
	...
	if (entries <= 0) {
		vod_log_error(
			VOD_LOG_ERR, context->request_context->log, 0, "mp4_clipper_stts_clip_data: zero entries"
		);
		return VOD_BAD_DATA;
	}
	...
	mp4_parser_stts_iterator_init(
		&iterator, &context->parse_params, (stts_entry_t*)(atom_info->ptr + sizeof(stts_atom_t)), entries
	);
	...
	*first_frame = iterator.frame_index;
	...
	if (context->parse_params.clip_to != UINT_MAX) {
		...
		clip_to = iterator.accum_duration
		        + (((uint64_t)(context->parse_params.clip_to - context->parse_params.clip_from)
		            * context->timescale)
		           / 1000);
	} else {
		clip_to = ULLONG_MAX;
...
mp4_clipper_stts_write_atom(u_char* p, void* write_context, stts_clip_result_t* stts, bool_t copy_data) {
	...
	write_be32(p, stts->entries); // entry_count
	...
	if (copy_data) {
		...
		mp4_clipper_write_tail(
			write_context, MP4_CLIPPER_TRAK_INDEX_STTS_DATA, first_entry, stts->data_size
		);
	}
...
mp4_clipper_stss_clip_data(
	parse_trak_atom_context_t* context, atom_info_t* atom_info, stss_clip_result_t* result
) {
	uint32_t* start_pos;
	uint32_t entries;
	vod_status_t rc;
	...
	if (context->last_frame != UINT_MAX) {
		result->last_entry =
			start_pos + mp4_parser_find_stss_entry(context->last_frame, start_pos, entries);
	} else {
...
mp4_clipper_stss_write_atom(u_char* p, void* write_context, stss_clip_result_t* stss, bool_t copy_data) {
	...
	if (copy_data) {
		...
		if (stss->data_size > 0) {
			mp4_clipper_write_tail(
				write_context, MP4_CLIPPER_TRAK_INDEX_STSS_DATA, stss->first_entry, stss->data_size
			);
		}
	}
...
mp4_clipper_ctts_clip_data(
	parse_trak_atom_context_t* context, atom_info_t* atom_info, ctts_clip_result_t* result
) {
	...
	mp4_parser_ctts_iterator_init(
		&iterator, (ctts_entry_t*)(atom_info->ptr + sizeof(ctts_atom_t)), entries
	);

	if (context->first_frame > 0) {
		if (!mp4_parser_ctts_iterator(&iterator, context->first_frame)) {
			vod_log_error(
				VOD_LOG_ERR, context->request_context->log, 0, "mp4_clipper_ctts_clip_data: failed to find first frame"
			);
			return VOD_BAD_DATA;
		}
	}
...
mp4_clipper_ctts_write_atom(u_char* p, void* write_context, ctts_clip_result_t* ctts, bool_t copy_data) {
	...
	write_fullbox_header(p, 0, 0);
	write_be32(p, ctts->entries); // entry_count
	...
	if (copy_data) {
		...
		mp4_clipper_write_tail(
			write_context, MP4_CLIPPER_TRAK_INDEX_CTTS_DATA, ctts->first_entry, ctts->data_size
		);
	}
...
mp4_clipper_stsc_clip_data(
	...
	rc = mp4_parser_stsc_iterator(
		&iterator, context->first_frame, &target_chunk, &sample_count, &next_chunk, &prev_samples
	);
	if (rc != VOD_OK) {
		return rc;
	}
	...
	rc = mp4_parser_stsc_iterator(
		&iterator, context->last_frame, &target_chunk, &sample_count, &next_chunk, &prev_samples
	);
	if (rc != VOD_OK) {
		return rc;
	}
	...
	*last_chunk_frame_index = context->last_frame - last_entry_samples_per_chunk;
	...
	// calculate atom size
	result->atom_size =
		ATOM_HEADER_SIZE + sizeof(stsc_atom_t) + result->entries * sizeof(stsc_entry_t);
...
mp4_clipper_stsc_write_atom(u_char* p, void* write_context, stsc_clip_result_t* stsc, bool_t copy_data) {
	...
	write_fullbox_header(p, 0, 0);
	write_be32(p, stsc->entries); // entry_count
	...
	if (!copy_data) {
		mp4_clipper_write_tail(
			...
			MP4_CLIPPER_TRAK_INDEX_STSC_DATA,
			first_entry,
			(u_char*)last_entry - (u_char*)first_entry
		);
		start = p;
	}
	...
	*last_frame_chunk_offset = 0;
	...
	default:
		vod_log_error(
			...
			context->request_context->log,
			0,
			"mp4_clipper_stsz_clip_data: unsupported field size %uD",
			field_size
		);
		return VOD_BAD_DATA;
	}
...
mp4_clipper_stsz_write_atom(u_char* p, void* write_context, stsz_clip_result_t* stsz, bool_t copy_data) {
	...
	if (stsz->field_size == 32 || stsz->uniform_size != 0) {
		...
		write_fullbox_header(p, 0, 0);
		write_be32(p, stsz->uniform_size); // sample_size
	} else {
		write_atom_name(p, 's', 't', 'z', '2');
		write_fullbox_header(p, 0, 0);
		write_be32(p, stsz->field_size); // reserved(24), field_size(8)
	}
	write_be32(p, stsz->entries); // sample_count
	...
	if (copy_data) {
		...
		if (stsz->data_size > 0) {
			mp4_clipper_write_tail(
				write_context, MP4_CLIPPER_TRAK_INDEX_STSZ_DATA, stsz->first_entry, stsz->data_size
			);
		}
	}
...
static vod_status_t
mp4_clipper_stco_init_chunk_count(parse_trak_atom_context_t* context, atom_info_t* atom_info) {
	...
	if (atom_info->size < sizeof(*stco_atom)) {
		vod_log_error(
			...
			context->request_context->log,
			0,
			"mp4_clipper_stco_init_chunk_count: atom size %uL too small",
			atom_info->size
		);
		return VOD_BAD_DATA;
	}
...
mp4_clipper_stco_clip_data(
	...
	rc = mp4_parser_validate_stco_data(
		context->request_context, atom_info, context->last_chunk_index, &entries, &entry_size
	);
	if (rc != VOD_OK) {
		return rc;
	}
	...
	result->first_entry =
		(u_char*)atom_info->ptr + sizeof(stco_atom_t) + context->first_chunk_index * entry_size;
	result->last_entry =
		(u_char*)atom_info->ptr + sizeof(stco_atom_t) + context->last_chunk_index * entry_size;
...
static u_char*
mp4_clipper_stco_write_atom(
	u_char* p, void* write_context, stco_clip_result_t* stco, bool_t copy_data, int64_t chunk_pos_diff
) {
	...
	if (stco->entry_size == sizeof(uint32_t)) {
		...
		write_fullbox_header(p, 0, 0);
		write_be32(p, stco->entries); // entry_count
		...
		if (copy_data) {
			...
			mp4_clipper_write_tail(
				write_context, MP4_CLIPPER_TRAK_INDEX_STCO_DATA, stco->first_entry, stco->data_size
			);
		}
	} else {
...
mp4_clipper_get_media_type(
	request_context_t* request_context, atom_info_t* hdlr_atom_info, uint32_t* media_type
) {
	...
	if (hdlr_atom_info->size < sizeof(*atom)) {
		vod_log_error(
			...
			request_context->log,
			0,
			"mp4_clipper_get_media_type: hdlr atom size %uL too small",
			hdlr_atom_info->size
		);
		return VOD_BAD_DATA;
	}
...
mp4_clipper_process_moov_atom_callback(void* ctx, atom_info_t* atom_info) {
	...
	case ATOM_NAME_MVHD:
		rc = mp4_clipper_mvhd_clip_data(
			context, atom_info, &context->result.mvhd, &context->mvhd_timescale
		);
		if (rc != VOD_OK) {
			return rc;
		}
	...
	if (parsed_trak == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, context->request_context->log, 0, "mp4_clipper_process_moov_atom_callback: vod_alloc failed"
		);
	...
	rc = mp4_clipper_get_media_type(
		context->request_context, &parsed_trak->atoms[TRAK_ATOM_HDLR], &media_type
	);
	if (rc != VOD_OK) {
		return rc;
	}
	...
	parse_context.stbl_atom_size =
		ATOM_HEADER_SIZE + full_atom_size(parsed_trak->atoms[TRAK_ATOM_STSD]);
	...
	rc = mp4_clipper_mdhd_clip_data(
		&parse_context, &parsed_trak->atoms[TRAK_ATOM_MDHD], &parsed_trak->mdhd, &parse_context.timescale
	);
	if (rc != VOD_OK) {
		return rc;
	}
	...
	rc = mp4_clipper_stss_clip_data(
		&parse_context, &parsed_trak->atoms[TRAK_ATOM_STSS], &parsed_trak->stss
	);
	if (rc != VOD_OK) {
		return rc;
	}

	rc = mp4_clipper_ctts_clip_data(
		&parse_context, &parsed_trak->atoms[TRAK_ATOM_CTTS], &parsed_trak->ctts
	);
	if (rc != VOD_OK) {
		return rc;
	}
	...
	rc = mp4_clipper_stco_clip_data(
		&parse_context, &parsed_trak->atoms[TRAK_ATOM_STCO], &parsed_trak->stco, &first_offset, &last_offset
	);
	if (rc != VOD_OK) {
		return rc;
	}
...
static u_char*
mp4_clipper_trak_write_atom(
	u_char* p, void* write_context, parsed_trak_t* parsed_trak, bool_t copy_data, int64_t chunk_pos_diff
) {
	if (copy_data) {
		...
		write_atom_header(p, parsed_trak->trak_atom_size, 't', 'r', 'a', 'k');
		mp4_clipper_write_tail(
			write_context, MP4_CLIPPER_TRAK_INDEX_HEADER, p - ATOM_HEADER_SIZE, ATOM_HEADER_SIZE
		);

		// trak.tkhd
		mp4_clipper_tkhd_update_atom((u_char*)parsed_trak->atoms[TRAK_ATOM_TKHD].ptr, &parsed_trak->tkhd);
		write_full_atom(
			write_context, MP4_CLIPPER_TRAK_INDEX_TKHD_ATOM, parsed_trak->atoms[TRAK_ATOM_TKHD]
		);

		// trak.mdia
		write_atom_header(p, parsed_trak->mdia_atom_size, 'm', 'd', 'i', 'a');
		mp4_clipper_write_tail(
			write_context, MP4_CLIPPER_TRAK_INDEX_MDIA_HEADER, p - ATOM_HEADER_SIZE, ATOM_HEADER_SIZE
		);

		// trak.mdia.*
		mp4_clipper_mdhd_update_atom((u_char*)parsed_trak->atoms[TRAK_ATOM_MDHD].ptr, &parsed_trak->mdhd);
		write_full_atom(
			write_context, MP4_CLIPPER_TRAK_INDEX_MDHD_ATOM, parsed_trak->atoms[TRAK_ATOM_MDHD]
		);
		write_full_atom(
			write_context, MP4_CLIPPER_TRAK_INDEX_HDLR_ATOM, parsed_trak->atoms[TRAK_ATOM_HDLR]
		);

		// trak.mdia.minf
		write_atom_header(p, parsed_trak->minf_atom_size, 'm', 'i', 'n', 'f');
		mp4_clipper_write_tail(
			write_context, MP4_CLIPPER_TRAK_INDEX_MINF_HEADER, p - ATOM_HEADER_SIZE, ATOM_HEADER_SIZE
		);

		// trak.mdia.minf.*
		write_full_atom(
			write_context, MP4_CLIPPER_TRAK_INDEX_VMHD_ATOM, parsed_trak->atoms[TRAK_ATOM_VMHD]
		);
		write_full_atom(
			write_context, MP4_CLIPPER_TRAK_INDEX_SMHD_ATOM, parsed_trak->atoms[TRAK_ATOM_SMHD]
		);
		write_full_atom(
			write_context, MP4_CLIPPER_TRAK_INDEX_DINF_ATOM, parsed_trak->atoms[TRAK_ATOM_DINF]
		);

		// trak.mdia.minf.stbl
		write_atom_header(p, parsed_trak->stbl_atom_size, 's', 't', 'b', 'l');
		mp4_clipper_write_tail(
			write_context, MP4_CLIPPER_TRAK_INDEX_STBL_HEADER, p - ATOM_HEADER_SIZE, ATOM_HEADER_SIZE
		);

		// trak.mdia.minf.stbl.stsd
		write_full_atom(
			write_context, MP4_CLIPPER_TRAK_INDEX_STSD_ATOM, parsed_trak->atoms[TRAK_ATOM_STSD]
		);
	}
...
mp4_clipper_parse_moov(
	...
	if (vod_array_init(
			&process_moov_context.result.parsed_traks, request_context->pool, 2, sizeof(parsed_trak_t*)
		)
	    != VOD_OK) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "mp4_clipper_parse_moov: vod_array_init failed"
		);
		return VOD_ALLOC_FAILED;
	}
	...
	if (parse_result == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "mp4_clipper_parse_moov: vod_alloc failed"
		);
		return VOD_ALLOC_FAILED;
	}
...
mp4_clipper_build_header(
	...
	mp4_clipper_parse_result_t* mp4_parse_result =
		vod_container_of(parse_result, mp4_clipper_parse_result_t, base);
	...
	if (mp4_parse_result->copy_data) {
		...
		buffer_count = MP4_CLIPPER_INDEX_COUNT
		             + MP4_CLIPPER_TRAK_INDEX_COUNT * mp4_parse_result->parsed_traks.nelts;
	}
	alloc_size = buffer_count * sizeof(write_elts[0]);
	write_elts = vod_alloc(request_context->pool, alloc_size);
	if (write_elts == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "mp4_clipper_build_header: vod_alloc failed (1)"
		);
		return VOD_ALLOC_FAILED;
	}
	...
	chunk_pos_diff = parse_result->first_offset
	               - (ftyp_atom_size + mp4_parse_result->moov_atom_size + mdat_header_size);
	...
	if (output == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "mp4_clipper_build_header: vod_alloc failed (2)"
		);
		return VOD_ALLOC_FAILED;
	}
	...
	if (ftyp_size > 0) {
		...
		if (mp4_parse_result->copy_data) {
			...
			mp4_clipper_write_tail(
				&write_context, MP4_CLIPPER_INDEX_FTYP_HEADER, p - ATOM_HEADER_SIZE, ATOM_HEADER_SIZE
			);
			mp4_clipper_write_tail(
				&write_context, MP4_CLIPPER_INDEX_FTYP_DATA, metadata_parts[MP4_METADATA_PART_FTYP].data, ftyp_size
			);
		}
	}
	...
	if (mp4_parse_result->copy_data) {
		...
		mp4_clipper_write_tail(
			&write_context, MP4_CLIPPER_INDEX_MOOV_HEADER, p - ATOM_HEADER_SIZE, ATOM_HEADER_SIZE
		);
	...
	for (; cur_trak < last_trak; cur_trak++) {
		p = mp4_clipper_trak_write_atom(
			p, &write_context, *cur_trak, mp4_parse_result->copy_data, chunk_pos_diff
		);
	...
	if (!mp4_parse_result->copy_data) {
		mp4_clipper_write_tail(
			&write_context, MP4_CLIPPER_INDEX_MDAT_HEADER, p - mdat_header_size, mdat_header_size
		);
	}
	...
	if (p - output != (ssize_t)mp4_parse_result->alloc_size) {
		vod_log_error(
			...
			request_context->log,
			0,
			"mp4_clipper_build_header: alloc size %uz different than used size %O",
			mp4_parse_result->alloc_size,
			(off_t)(p - output)
		);
