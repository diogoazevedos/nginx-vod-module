...
mp4_aes_ctr_init(mp4_aes_ctr_state_t* state, request_context_t* request_context, u_char* key) {
	...
	if (cln == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "mp4_aes_ctr_init: vod_pool_cleanup_add failed"
		);
	...
	if (state->cipher == NULL) {
		vod_log_error(VOD_LOG_ERR, request_context->log, 0, "mp4_aes_ctr_init: EVP_CIPHER_CTX_new failed");
		return VOD_ALLOC_FAILED;
	}
	...
	if (1 != EVP_EncryptInit_ex(state->cipher, EVP_aes_128_ecb(), NULL, key, NULL)) {
		vod_log_error(VOD_LOG_ERR, request_context->log, 0, "mp4_aes_ctr_init: EVP_EncryptInit_ex failed");
		return VOD_ALLOC_FAILED;
	}
...
void
mp4_aes_ctr_set_iv(mp4_aes_ctr_state_t* state, u_char* iv) {
...
mp4_aes_ctr_process(mp4_aes_ctr_state_t* state, u_char* dest, const u_char* src, uint32_t size) {
	...
	while (src < src_end) {
		if (state->encrypted_pos >= state->encrypted_end) {
			...
			if (1
			        != EVP_EncryptUpdate(
						state->cipher, state->encrypted_counter, &out_size, state->counter, encrypted_size
					)
			    || out_size != (int)encrypted_size) {
				vod_log_error(
					VOD_LOG_ERR, state->request_context->log, 0, "mp4_aes_ctr_process: EVP_EncryptUpdate failed"
				);
