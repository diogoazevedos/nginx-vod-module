...
static const relevant_atom_t relevant_atoms_stbl[] = {
	...
	// senc should be under trak, maintained for backward compatibility
...
mp4_parser_parse_hdlr_atom(atom_info_t* atom_info, metadata_parse_context_t* context) {
	...
	if (atom_info->size < sizeof(*atom)) {
		vod_log_error(
			...
			0,
			"mp4_parser_parse_hdlr_atom: atom size %uL too small",
			atom_info->size
		);
		return VOD_BAD_DATA;
	}
	...
	if (tags->label.data == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, context->request_context->log, 0, "mp4_parser_parse_hdlr_atom: vod_alloc failed"
		);
...
mp4_parser_parse_tkhd_atom(atom_info_t* atom_info, metadata_parse_context_t* context) {
	...
	if (atom_info->size < sizeof(*atom)) {
		vod_log_error(
			...
			context->request_context->log,
			0,
			"mp4_parser_parse_tkhd_atom: atom size %uL too small (1)",
			atom_info->size
		);
		return VOD_BAD_DATA;
	}
	...
	if (atom->version[0] == 1) {
		if (atom_info->size < sizeof(*atom64)) {
			vod_log_error(
				...
				context->request_context->log,
				0,
				"mp4_parser_parse_tkhd_atom: atom size %uL too small (2)",
				atom_info->size
			);
			return VOD_BAD_DATA;
		}
	...
	if (context->media_info.track_id == 0) {
		vod_log_error(
			VOD_LOG_ERR, context->request_context->log, 0, "mp4_parser_parse_tkhd_atom: invalid track id"
		);
		return VOD_BAD_DATA;
	}
...
mp4_parser_parse_elst_atom(atom_info_t* atom_info, metadata_parse_context_t* context) {
	...
	if (atom_info->size < sizeof(*atom)) {
		vod_log_error(
			...
			context->request_context->log,
			0,
			"mp4_parser_parse_elst_atom: atom size %uL too small",
			atom_info->size
		);
		return VOD_BAD_DATA;
	}
	...
	if (entries <= 0) {
		vod_log_error(
			VOD_LOG_ERR, context->request_context->log, 0, "mp4_parser_parse_elst_atom: zero entries"
		);
		return VOD_BAD_DATA;
	}
	...
	if (entries >= (INT_MAX - sizeof(*atom)) / entry_size) { // integer overflow protection
		vod_log_error(
			...
			context->request_context->log,
			0,
			"mp4_parser_parse_elst_atom: number of entries %uD too big",
			entries
		);
		return VOD_BAD_DATA;
	}
	...
	if (atom_info->size < sizeof(*atom) + entries * entry_size) {
		vod_log_error(
			...
			context->request_context->log,
			0,
			"mp4_parser_parse_elst_atom: atom size %uL too small to hold %uD entries",
			atom_info->size,
			entries
		);
		return VOD_BAD_DATA;
	...
	if (time == -1) {
		...
		vod_log_error(
			...
			context->request_context->log,
			0,
			"mp4_parser_parse_elst_atom: unsupported edit - time is %L",
			time
		);
	}
...
mp4_parser_parse_mdhd_atom(atom_info_t* atom_info, metadata_parse_context_t* context) {
	...
	if (atom_info->size < sizeof(*atom)) {
		vod_log_error(
			...
			context->request_context->log,
			0,
			"mp4_parser_parse_mdhd_atom: atom size %uL too small (1)",
			atom_info->size
		);
		return VOD_BAD_DATA;
	}
	...
	if (atom->version[0] == 1) {
		if (atom_info->size < sizeof(*atom64)) {
			vod_log_error(
				...
				context->request_context->log,
				0,
				"mp4_parser_parse_mdhd_atom: atom size %uL too small (2)",
				atom_info->size
			);
			return VOD_BAD_DATA;
		}
	...
	if (timescale == 0) {
		vod_log_error(
			VOD_LOG_ERR, context->request_context->log, 0, "mp4_parser_parse_mdhd_atom: timescale is zero"
		);
		return VOD_BAD_DATA;
	}
	...
	if (duration > (uint64_t)MAX_DURATION_SEC * timescale) {
		vod_log_error(
			...
			context->request_context->log,
			0,
			"mp4_parser_parse_mdhd_atom: media duration %uL too big",
			duration
		);
		return VOD_BAD_DATA;
	}
...
mp4_parser_parse_udta_name_atom(atom_info_t* atom_info, metadata_parse_context_t* context) {
	...
	if (tags->label.data == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, context->request_context->log, 0, "mp4_parser_parse_udta_name_atom: vod_alloc failed"
		);
		return VOD_ALLOC_FAILED;
	}
...
mp4_parser_parse_stts_atom_total_duration_only(atom_info_t* atom_info, metadata_parse_context_t* context) {
	...
	if (duration > (uint64_t)MAX_DURATION_SEC * timescale) {
		vod_log_error(
			...
			context->request_context->log,
			0,
			"mp4_parser_parse_stts_atom_total_duration_only: media duration %uL too big",
			duration
		);
		return VOD_BAD_DATA;
	}
...
mp4_parser_parse_stts_atom_frame_duration_only(atom_info_t* atom_info, frames_parse_context_t* context) {
	...
	for (; cur_entry < last_entry; cur_entry++) {
		...
		if (cur_duration != 0
		    && (media_info->min_frame_duration == 0 || cur_duration < media_info->min_frame_duration)) {
...
mp4_parser_parse_stts_atom(atom_info_t* atom_info, frames_parse_context_t* context) {
	...
	empty_duration =
		rescale_time_neg(context->media_info->empty_duration, context->mvhd_timescale, timescale);
	...
	if (context->parse_params.clip_from > 0) {
		...
		if (context->stss_entries != 0) {
			...
			key_frame_stss_index =
				mp4_parser_find_stss_entry(frame_index, context->stss_start_pos, context->stss_entries);
		...
		if (cur_entry >= last_entry) {
			if (context->stss_entries != 0) {
				if (context->media_info->duration > clip_from_accum_duration) {
					context->first_frame_time_offset =
						context->media_info->duration - clip_from_accum_duration;
				}
				range->start = 0;
				range->end = 0;
			}
	...
	if (range->end == ULLONG_MAX) {
		...
		if (entries == 1) {
			...
			if (sample_duration == 0) {
				vod_log_error(
					VOD_LOG_ERR, context->request_context->log, 0, "mp4_parser_parse_stts_atom: sample duration is zero (1)"
				);
				return VOD_BAD_DATA;
			}
	...
	if (initial_alloc_size > context->parse_params.max_frame_count) {
		vod_log_error(
			...
			context->request_context->log,
			0,
			"mp4_parser_parse_stts_atom: initial alloc size %uD exceeds the max frame count %uD",
			initial_alloc_size,
			context->parse_params.max_frame_count
		);
		return VOD_BAD_DATA;
	}

	if (vod_array_init(
			&frames_array, context->request_context->pool, initial_alloc_size, sizeof(input_frame_t)
		)
	    != VOD_OK) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, context->request_context->log, 0, "mp4_parser_parse_stts_atom: vod_array_init failed"
		);
		return VOD_ALLOC_FAILED;
	}
		...
		for (;;) {
			...
			if (cur_count > context->parse_params.max_frame_count - frames_array.nelts) {
				vod_log_error(
					...
					context->request_context->log,
					0,
					"mp4_parser_parse_stts_atom: frame count exceeds the limit %uD",
					context->parse_params.max_frame_count
				);
				return VOD_BAD_DATA;
			}
	...
	if (context->last_frame < context->first_frame) {
		vod_log_error(
			...
			context->request_context->log,
			0,
			"mp4_parser_parse_stts_atom: last frame %uD smaller than first frame %uD",
			context->last_frame,
			context->first_frame
		);
		return VOD_BAD_DATA;
	...
	if (clip_to != ULLONG_MAX
	    && (cur_entry >= last_entry
	        || (accum_duration - clip_from_accum_duration) > clip_to - clip_from)) {
...
mp4_parser_parse_stco_atom(atom_info_t* atom_info, frames_parse_context_t* context) {
	...
	if (context->chunk_equals_sample) {
		if (entries < context->last_frame) {
			vod_log_error(
				...
				context->request_context->log,
				0,
				"mp4_parser_parse_stco_atom: number of entries %uD smaller than last frame %uD",
				entries,
				context->last_frame
			);
			return VOD_BAD_DATA;
	...
	if (last_frame[-1].key_frame >= entries) {
		vod_log_error(
			...
			context->request_context->log,
			0,
			"mp4_parser_parse_stco_atom: number of entries %uD smaller than last chunk %uD",
			entries,
			last_frame[-1].key_frame
		);
		return VOD_BAD_DATA;
	}

	// NOTE: we use key_frame to store the chunk index since it's temporary
	...
	if (atom_info->name == ATOM_NAME_CO64) {
		...
		for (; cur_frame < last_frame; cur_frame++) {
			if (cur_frame->key_frame != cur_chunk_index) {
				// NOTE: assuming chunk indexes always grow by 1, the way stsc is encoded ensures
				// that's always true
				cur_chunk_index = cur_frame->key_frame;
				read_be64(cur_pos, cur_file_offset);
			}
...
mp4_parser_parse_stsc_atom(atom_info_t* atom_info, frames_parse_context_t* context) {
	...
	if (entries == 1
	    && vod_memcmp(
			   atom_info->ptr + sizeof(stsc_atom_t), chunk_equals_sample_entry, sizeof(chunk_equals_sample_entry)
		   ) == 0) {
	...
	if (next_chunk != 1) {
		vod_log_error(
			VOD_LOG_ERR, context->request_context->log, 0, "mp4_parser_parse_stsc_atom: first chunk index is not 1"
		);
		return VOD_BAD_DATA;
	}
		...
		for (; cur_entry + 1 < last_entry; cur_entry++, frame_index += cur_entry_samples) {
			...
			if (next_chunk <= cur_chunk) {
				vod_log_error(
					...
					context->request_context->log,
					0,
					"mp4_parser_parse_stsc_atom: chunk index %uD is smaller than the previous index %uD (1)",
					next_chunk,
					cur_chunk
				);
				return VOD_BAD_DATA;
			...
			if (samples_per_chunk == 0) {
				vod_log_error(
					...
					context->request_context->log,
					0,
					"mp4_parser_parse_stsc_atom: invalid samples per chunk %uD",
					samples_per_chunk
				);
				return VOD_BAD_DATA;
			}
			...
			if (cur_entry_samples > UINT_MAX - frame_index) { // integer overflow protection
				...
				vod_log_error(
					...
					context->request_context->log,
					0,
					"mp4_parser_parse_stsc_atom: number of samples per entry %uL is too big",
					cur_entry_samples
				);
				return VOD_BAD_DATA;
			}
			...
			if (frame_index + cur_entry_samples > context->first_frame) {
				// the first frame is within the current entry, revert the value of next chunk
	...
	if (samples_per_chunk == 0) {
		vod_log_error(
			VOD_LOG_ERR, context->request_context->log, 0, "mp4_parser_parse_stsc_atom: samples per chunk is zero"
		);
		return VOD_BAD_DATA;
	}
	cur_chunk = next_chunk;
	if (cur_entry + 1 < last_entry) {
		...
		if (next_chunk <= cur_chunk) {
			vod_log_error(
				...
				context->request_context->log,
				0,
				"mp4_parser_parse_stsc_atom: chunk index %uD is smaller than the previous index %uD (2)",
				next_chunk,
				cur_chunk
			);
			return VOD_BAD_DATA;
	...
	if (skip_chunks >= next_chunk - cur_chunk) {
		vod_log_error(
			VOD_LOG_ERR, context->request_context->log, 0, "mp4_parser_parse_stsc_atom: failed to find first frame chunk"
		);
...
mp4_parser_estimate_key_frame_bitrate(frames_parse_context_t* context, atom_info_t* stsz, atom_info_t* stss) {
	...
	rc = mp4_parser_validate_stsz_atom(
		context->request_context, stsz, 0, &uniform_size, &field_size, &stsz_entries
	);
	if (rc != VOD_OK) {
		return rc;
	}
	...
	sample_count = (stss_entries > MAX_KEY_FRAME_BITRATE_TEST_SAMPLES)
	                 ? MAX_KEY_FRAME_BITRATE_TEST_SAMPLES
	                 : stss_entries;
...
static vod_status_t
mp4_parser_compute_bitrate(
	frames_parse_context_t* context, atom_info_t* stts, atom_info_t* stsz, atom_info_t* stss, uint32_t segment_duration_millis
) {
	const stts_entry_t* stts_cur;
	const stts_entry_t* stts_last;
	const uint32_t* stss_cur = NULL;
	const uint32_t* stss_end = NULL;
	const uint8_t* stsz_data;
	uint32_t uniform_size;
	uint32_t field_size;
	...
	uint32_t next_key_frame = 0;
	uint32_t frame_index;
	uint32_t frame_duration;
	uint32_t frame_size;
	uint32_t timescale = context->media_info->timescale;
	uint64_t target_duration = ((uint64_t)segment_duration_millis * timescale) / 1000;
	uint64_t max_bitrate = 0;
	uint64_t cur_bitrate;
	...
	rc = mp4_parser_validate_stts_data(context->request_context, stts, &stts_entries);
	if (rc != VOD_OK) {
		return rc;
	}
	...
	// absent stss means all frames are sync samples
	if (stss->size != 0) {
		rc = mp4_parser_validate_stss_atom(context->request_context, stss, &stss_entries);
		if (rc != VOD_OK) {
			return rc;
		...
		stss_cur = (const uint32_t*)(stss->ptr + sizeof(stss_atom_t));
		stss_end = stss_cur + stss_entries;
		next_key_frame = parse_be32(stss_cur) - 1;
		stss_cur++;
	}

	// init stts iteration
	stts_cur = (const stts_entry_t*)(stts->ptr + sizeof(stts_atom_t));
	stts_last = stts_cur + stts_entries;
	stts_remaining = parse_be32(stts_cur->count);
	frame_duration = parse_be32(stts_cur->duration);

	// init stsz data pointer
	stsz_data = stsz->ptr + sizeof(stsz_atom_t);

	for (frame_index = 0; frame_index < stsz_entries; frame_index++) {
		// determine whether this frame is a key frame
		is_key_frame = FALSE;
		if (stss_cur == NULL) {
			is_key_frame = TRUE;
		} else if (frame_index == next_key_frame) {
			is_key_frame = TRUE;
			next_key_frame = UINT32_MAX;
			if (stss_cur < stss_end) {
				next_key_frame = parse_be32(stss_cur) - 1;
				stss_cur++;
			}
		}

		// at a key frame boundary, close the previous segment if long enough
		if (is_key_frame && frame_index > 0 && seg_duration >= target_duration) {
			cur_bitrate = (seg_size * 8 * timescale) / seg_duration;
			if (cur_bitrate > max_bitrate) {
		...
		// get frame size
		frame_size = uniform_size;
		if (uniform_size == 0) {
			switch (field_size) {
			case 32:
				frame_size = parse_be32((const uint32_t*)stsz_data + frame_index);
				break;

			case 16:
				frame_size = parse_be16((const uint16_t*)stsz_data + frame_index);
				break;

			case 8:
				frame_size = stsz_data[frame_index];
				break;
			...
			default:
				vod_log_debug1(
					...
					context->request_context->log,
					0,
					"mp4_parser_compute_bitrate: unsupported field_size %uD",
					field_size
				);
				return VOD_BAD_DATA;
			}
		...
		if (stts_remaining == 0) {
			stts_cur++;
			stts_remaining = UINT32_MAX;
			if (stts_cur < stts_last) {
				stts_remaining = parse_be32(stts_cur->count);
				frame_duration = parse_be32(stts_cur->duration);
			}
		}
	...
	context->media_info->bitrate = (uint32_t)max_bitrate;
	return VOD_OK;
}
...
mp4_parser_parse_stsz_atom_total_size_estimate_only(atom_info_t* atom_info, frames_parse_context_t* context) {
	...
	rc = mp4_parser_validate_stsz_atom(
		context->request_context, atom_info, context->last_frame, &uniform_size, &field_size, &entries
	);
	if (rc != VOD_OK) {
		return rc;
	}
	...
	default:
		vod_log_error(
			...
			context->request_context->log,
			0,
			"mp4_parser_parse_stsz_atom_total_size_estimate_only: unsupported field size %ud",
			field_size
		);
		return VOD_BAD_DATA;
	}
...
mp4_parser_parse_stsz_atom(atom_info_t* atom_info, frames_parse_context_t* context) {
	...
	case 32:
		cur_pos =
			atom_info->ptr + sizeof(stsz_atom_t) + context->first_chunk_frame_index * sizeof(uint32_t);
		...
		for (; cur_frame < last_frame; cur_frame++) {
			...
			if (cur_size > MAX_FRAME_SIZE) {
				vod_log_error(
					...
					context->request_context->log,
					0,
					"mp4_parser_parse_stsz_atom: frame size %uD too big",
					cur_size
				);
				return VOD_BAD_DATA;
			}
	...
	case 16:
		cur_pos =
			atom_info->ptr + sizeof(stsz_atom_t) + context->first_chunk_frame_index * sizeof(uint16_t);
	...
	default:
		vod_log_error(
			...
			context->request_context->log,
			0,
			"mp4_parser_parse_stsz_atom: unsupported field size %ud",
			field_size
		);
		return VOD_BAD_DATA;
	}
...
mp4_parser_parse_stss_atom(atom_info_t* atom_info, frames_parse_context_t* context) {
	...
	if (context->stss_start_index == 0 && context->first_frame > 0) {
		context->stss_start_index =
			mp4_parser_find_stss_entry(context->first_frame, start_pos, entries);
	}
...
mp4_parser_parse_sinf_atoms(void* ctx, atom_info_t* atom_info) {
	...
	if (atom_info->size < sizeof(uint32_t)) {
		vod_log_error(
			...
			context->request_context->log,
			0,
			"mp4_parser_parse_sinf_atoms: frma atom size %uL too small",
			atom_info->size
		);
		return VOD_BAD_DATA;
	}
...
mp4_parser_parse_sinf_atom(atom_info_t* atom_info, metadata_parse_context_t* context) {
	...
	return mp4_parser_parse_atoms(
		context->request_context, atom_info->ptr, atom_info->size, TRUE, mp4_parser_parse_sinf_atoms, context
	);
}
...
// NOTE: no validations in the functions below - the stream functions will make sure we don't
...
mp4_parser_read_config_descriptor(metadata_parse_context_t* context, simple_read_stream_t* stream) { // ff_mp4_read_dec_config_descr
	...
	read_stream_skip(
		stream,
		member_size(config_descr_t, stream_type)
			+ member_size(config_descr_t, buffer_size)
			+ member_size(config_descr_t, max_bitrate)
	);
	...
	if (tag == MP4DecSpecificDescrTag) {
		if (len > (unsigned)(stream->end_pos - stream->cur_pos)) {
			vod_log_error(
				...
				context->request_context->log,
				0,
				"mp4_parser_read_config_descriptor: tag length %ud too big",
				len
			);
			return VOD_BAD_DATA;
		}
		...
		rc = codec_config_mp4a_config_parse(
			context->request_context, &context->media_info.extra_data, &context->media_info
		);
		if (rc != VOD_OK) {
			return rc;
		}
...
mp4_parser_parse_dops_atom(metadata_parse_context_t* context, atom_info_t* atom_info) {
	...
	if (atom_info->size < sizeof(*atom)) {
		vod_log_error(
			...
			context->request_context->log,
			0,
			"mp4_parser_parse_dops_atom: atom size %uL too small",
			atom_info->size
		);
		return VOD_BAD_DATA;
	}
	...
	if (p == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, context->request_context->log, 0, "mp4_parser_parse_dops_atom: vod_alloc failed"
		);
		return VOD_ALLOC_FAILED;
	}
...
mp4_parser_parse_audio_atoms(void* ctx, atom_info_t* atom_info) {
	...
	case ATOM_NAME_DEC3:
		...
		// set the extra data to the contents of this atom, since it's used as the setup info in
		// hls/sample-aes
		context->media_info.extra_data.len = atom_info->size;
		context->media_info.extra_data.data = (u_char*)atom_info->ptr;
		return VOD_OK;
...
mp4_parser_parse_audio_extra_data_atom(void* ctx, atom_info_t* atom_info) {
	...
	if (atom_info->name == ATOM_NAME_WAVE && atom_info->size > 8) {
		rc = mp4_parser_parse_atoms(
			context->request_context, atom_info->ptr, atom_info->size, TRUE, mp4_parser_parse_audio_atoms, context
		);
	} else {
...
static const u_char*
mp4_parser_skip_stsd_atom_video(
	const u_char* cur_pos, const u_char* end_pos, metadata_parse_context_t* context
) {
	stsd_video_t* video;
...
static const u_char*
mp4_parser_skip_stsd_atom_audio(
	const u_char* cur_pos, const u_char* end_pos, metadata_parse_context_t* context
) {
	stsd_audio_t* audio;
	uint16_t version;
...
mp4_parser_parse_stsd_atom(atom_info_t* atom_info, metadata_parse_context_t* context) {
	...
	if (atom_info->size < sizeof(*atom)) {
		vod_log_error(
			...
			context->request_context->log,
			0,
			"mp4_parser_parse_stsd_atom: atom size %uL too small",
			atom_info->size
		);
		return VOD_BAD_DATA;
	}
	...
	for (entries = parse_be32(atom->entries); entries; entries--) {
		if (cur_pos + sizeof(stsd_entry_header_t) > end_pos) {
			vod_log_error(
				VOD_LOG_ERR, context->request_context->log, 0, "mp4_parser_parse_stsd_atom: not enough room for stsd entry"
			);
			return VOD_BAD_DATA;
		}
		...
		if (size > (uint32_t)(end_pos - cur_pos)) {
			vod_log_error(
				VOD_LOG_ERR, context->request_context->log, 0, "mp4_parser_parse_stsd_atom: stsd entry overflow the stream"
			);
			return VOD_BAD_DATA;
		}
		...
		if (cur_pos == NULL) {
			vod_log_error(
				VOD_LOG_ERR, context->request_context->log, 0, "mp4_parser_parse_stsd_atom: failed to skip audio/video data"
			);
			return VOD_BAD_DATA;
		}
		...
		if (cur_pos + 8 < entry_end_pos) {
			vod_log_buffer(
				...
				context->request_context->log,
				0,
				"mp4_parser_parse_stsd_atom: extra data ",
				cur_pos,
				entry_end_pos - cur_pos
			);

			rc = mp4_parser_parse_atoms(
				context->request_context, cur_pos, entry_end_pos - cur_pos, TRUE, parse_function, context
			);
			if (rc != VOD_OK) {
				return rc;
			}
...
mp4_parser_parse_saiz_atom(atom_info_t* atom_info, frames_parse_context_t* context) {
	...
	if (atom_info->size < sizeof(*atom)) {
		vod_log_error(
			...
			context->request_context->log,
			0,
			"mp4_parser_parse_saiz_atom: atom size %uL too small (1)",
			atom_info->size
		);
		return VOD_BAD_DATA;
	}
	...
	if ((atom->flags[2] & 0x01) != 0) {
		if (atom_info->size < sizeof(*atom_with_type)) {
			vod_log_error(
				...
				context->request_context->log,
				0,
				"mp4_parser_parse_saiz_atom: atom size %uL too small (2)",
				atom_info->size
			);
			return VOD_BAD_DATA;
		}
	...
	if (first_entry + context->last_frame > atom_info->ptr + atom_info->size) {
		vod_log_error(
			...
			context->request_context->log,
			0,
			"mp4_parser_parse_saiz_atom: atom too small to hold %uD entries",
			context->last_frame
		);
		return VOD_BAD_DATA;
	}
	...
	context->encryption_info.auxiliary_sample_sizes =
		vod_alloc(context->request_context->pool, context->frame_count);
	if (context->encryption_info.auxiliary_sample_sizes == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, context->request_context->log, 0, "mp4_parser_parse_saiz_atom: vod_alloc failed"
		);
		return VOD_ALLOC_FAILED;
	}
	vod_memcpy(
		context->encryption_info.auxiliary_sample_sizes, first_entry + context->first_frame, context->frame_count
	);
...
mp4_parser_parse_senc_atom(atom_info_t* atom_info, frames_parse_context_t* context) {
	...
	if (atom_info->size == 0
	    || context->auxiliary_info_start_offset >= context->auxiliary_info_end_offset) { // optional atom
		return VOD_OK;
	}
	...
	if (atom_info->size < sizeof(*atom) + context->auxiliary_info_end_offset) {
		vod_log_error(
			...
			context->request_context->log,
			0,
			"mp4_parser_parse_senc_atom: atom smaller than end offset %uD",
			context->auxiliary_info_end_offset
		);
		return VOD_BAD_DATA;
	}
	...
	context->encryption_info.auxiliary_info =
		vod_alloc(context->request_context->pool, auxiliary_info_size);
	if (context->encryption_info.auxiliary_info == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, context->request_context->log, 0, "mp4_parser_parse_senc_atom: vod_alloc failed"
		);
		return VOD_ALLOC_FAILED;
	}
	...
	context->encryption_info.auxiliary_info_end =
		context->encryption_info.auxiliary_info + auxiliary_info_size;
...
mp4_parser_parse_mvhd_atom(process_moov_context_t* context, atom_info_t* atom_info) {
	...
	if (atom_info->size < sizeof(*atom)) {
		vod_log_error(
			...
			context->request_context->log,
			0,
			"mp4_parser_parse_mvhd_atom: atom size %uL too small",
			atom_info->size
		);
		return VOD_BAD_DATA;
	}
	...
	if (atom->version[0] == 1) {
		if (atom_info->size < sizeof(*atom64)) {
			vod_log_error(
				...
				context->request_context->log,
				0,
				"mp4_parser_parse_mvhd_atom: atom size %uL too small (2)",
				atom_info->size
			);
			return VOD_BAD_DATA;
		}
...
mp4_parser_process_moov_atom_callback(void* ctx, atom_info_t* atom_info) {
	...
	if (metadata_parse_context.media_info.codec_id == VOD_CODEC_ID_INVALID) {
		vod_log_debug3(
			...
			metadata_parse_context.media_info.media_type,
			metadata_parse_context.media_info.format,
			(uint32_t)metadata_parse_context.media_info.u.audio.object_type_id
		);
		return VOD_OK;
	}

	if (!vod_codec_in_mask(
		...
		vod_log_debug2(
			...
			metadata_parse_context.media_info.codec_id,
			metadata_parse_context.parse_params.codecs_mask
		);
		return VOD_OK;
	}
	...
	if (sequence->tags.label.len != 0) {
		...
		// NOTE: it is not possible for the sequence to have a language without a label, since a
		// default label will be assigned according to the language
	...
	if (!vod_is_bit_set(
			context->parse_params.required_tracks_mask[metadata_parse_context.media_info.media_type], track_index
		)) {
		return VOD_OK;
	}
	...
	if (context->parse_params.langs_mask != NULL
	    ...
	    && !vod_is_bit_set(
			context->parse_params.langs_mask, metadata_parse_context.media_info.tags.language
		)) {
		return VOD_OK;
	}
	...
	duration_millis =
		vod_min(metadata_parse_context.media_info.duration_millis, context->parse_params.clip_to)
		- context->parse_params.clip_from;
	if (duration_millis != metadata_parse_context.media_info.duration_millis) {
		...
		metadata_parse_context.media_info.duration =
			rescale_time(duration_millis, 1000, metadata_parse_context.media_info.timescale);
	} else {
		metadata_parse_context.media_info.duration = metadata_parse_context.media_info.full_duration;
	}

	rc = media_format_finalize_track(
		context->request_context, parse_type, &metadata_parse_context.media_info
	);
	if (rc != VOD_OK) {
		return rc;
	}
	...
	if (result->base.tracks.nelts > MAX_TRACK_COUNT) {
		vod_log_error(
			...
			context->request_context->log,
			0,
			"mp4_parser_process_moov_atom_callback: track count exceeded the limit of %i",
			(ngx_int_t)MAX_TRACK_COUNT
		);
	...
	metadata_parse_context.media_info.bitrate =
		sequence->bitrate[metadata_parse_context.media_info.media_type];
	metadata_parse_context.media_info.avg_bitrate =
		sequence->avg_bitrate[metadata_parse_context.media_info.media_type];
	...
	if (result->base.duration == 0
	    || result->base.duration * metadata_parse_context.media_info.timescale
	           < metadata_parse_context.media_info.duration * result->base.timescale) {
...
vod_status_t
mp4_parser_get_ftyp_atom_into(
	request_context_t* request_context, const u_char* buffer, size_t buffer_size, const u_char** ptr, size_t* size
) {
	...
	mp4_parser_parse_atoms(
		request_context, buffer, buffer_size, FALSE, &mp4_parser_find_atom_callback, &find_context
	);
	if (find_context.ptr == NULL) {
		vod_log_error(
			VOD_LOG_ERR, request_context->log, 0, "mp4_parser_get_ftyp_atom_into: failed to parse any atoms"
		);
		return VOD_BAD_DATA;
	}
...
vod_status_t
mp4_parser_get_moov_atom_info(
	request_context_t* request_context, const u_char* buffer, size_t buffer_size, off_t* offset, size_t* size
) {
	atom_info_t find_moov_context = {NULL, 0, ATOM_NAME_MOOV, 0};

	// not checking the result of mp4_parser_parse_atoms since we always stop the enumeration here
	mp4_parser_parse_atoms(
		request_context, buffer, buffer_size, FALSE, &mp4_parser_find_atom_callback, &find_moov_context
	);
	if (find_moov_context.ptr == NULL) {
		vod_log_error(
			VOD_LOG_ERR, request_context->log, 0, "mp4_parser_get_moov_atom_info: failed to parse any atoms"
		);
		return VOD_BAD_DATA;
	}
...
static const trak_atom_parser_t trak_atom_parsers[] = {
	...
	{mp4_parser_parse_stts_atom_frame_duration_only,
     offsetof(trak_atom_infos_t, stts),
     PARSE_FLAG_DURATION_LIMITS},
	{mp4_parser_parse_ctts_atom, offsetof(trak_atom_infos_t, ctts), PARSE_FLAG_FRAMES_PTS_DELAY},
	{mp4_parser_parse_ctts_atom_initial_pts_delay,
     offsetof(trak_atom_infos_t, ctts),
     PARSE_FLAG_INITIAL_PTS_DELAY},
	...
	{mp4_parser_parse_stsz_atom_total_size_estimate_only,
     offsetof(trak_atom_infos_t, stsz),
...
mp4_parser_parse_basic_metadata(
	...
	if (metadata == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "mp4_parser_parse_basic_metadata: vod_alloc failed"
		);
		return VOD_ALLOC_FAILED;
	}

	vod_memzero(metadata, sizeof(*metadata));
	if (vod_array_init(&metadata->base.tracks, request_context->pool, 2, sizeof(mp4_track_base_metadata_t))
	    != VOD_OK) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "mp4_parser_parse_basic_metadata: vod_array_init failed"
		);
		return VOD_ALLOC_FAILED;
	}
	...
	if (metadata->mvhd_timescale == 0) {
		vod_log_error(
			VOD_LOG_ERR, request_context->log, 0, "mp4_parser_parse_basic_metadata: mvhd timescale was not set"
		);
		return VOD_BAD_DATA;
	}
...
mp4_parser_copy_raw_atoms(request_context_t* request_context, raw_atom_t* dest, trak_atom_infos_t* source) {
	const raw_atom_mapping_t* end =
		raw_atom_mapping + sizeof(raw_atom_mapping) / sizeof(raw_atom_mapping[0]);
	...
	if (p == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "mp4_parser_copy_raw_atoms: vod_alloc failed"
		);
		return VOD_ALLOC_FAILED;
	}
...
static void
mp4_parser_strip_stsd_encryption(raw_atom_t* dest_stsd_atom, mp4_track_base_metadata_t* track) {
	...
	sinf_offset = (src_sinf_atom->ptr - src_sinf_atom->header_size)
	            - (src_stsd_atom->ptr - src_stsd_atom->header_size);
...
mp4_parser_parse_frames(
	...
	if (vod_array_init(&tracks, request_context->pool, 2, sizeof(media_track_t)) != VOD_OK) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "mp4_parser_parse_frames: vod_array_init failed"
		);
		return VOD_ALLOC_FAILED;
	}
	...
	for (cur_track = first_track; cur_track < last_track; cur_track++) {
		...
		vod_memzero(
			(u_char*)&context + offsetof(frames_parse_context_t, stss_start_pos),
		...
		if (cur_track == first_track
			...
			rc = mp4_parser_validate_stss_atom(
				context.request_context, &cur_track->trak_atom_infos.stss, &context.stss_entries
			);
			if (rc != VOD_OK) {
				return rc;
			}

			context.stss_start_pos =
				(const uint32_t*)(cur_track->trak_atom_infos.stss.ptr + sizeof(stss_atom_t));
		}
		...
		for (cur_parser = trak_atom_parsers; cur_parser->parse; cur_parser++) {
			...
			vod_log_debug1(
				...
				request_context->log,
				0,
				"mp4_parser_parse_frames: running parser 0x%xD",
				cur_parser->flag
			);
			rc = cur_parser->parse(
				(atom_info_t*)((u_char*)&cur_track->trak_atom_infos + cur_parser->offset), &context
			);
			if (rc != VOD_OK) {
				return rc;
			}
		...
		if (cur_track->media_info.full_duration > 0) {
			// estimate from frame size when not resolved
			if (cur_track->media_info.avg_bitrate == 0) {
				cur_track->media_info.avg_bitrate =
					(uint32_t)((context.total_frames_size * cur_track->media_info.timescale * 8)
				               / cur_track->media_info.full_duration);
			}

			// compute from stream data when not resolved
			if (cur_track->media_info.bitrate == 0) {
				if (media_type == MEDIA_TYPE_VIDEO
				    && (parse_params->parse_type & PARSE_FLAG_PEAK_BITRATE) != 0) {
					rc = mp4_parser_compute_bitrate(
						&context,
						&cur_track->trak_atom_infos.stts,
				...
				// fallback to avg_bitrate if unresolved
				if (cur_track->media_info.bitrate == 0) {
					cur_track->media_info.bitrate = cur_track->media_info.avg_bitrate;
				}
			}
		...
		if (result_track == NULL) {
			vod_log_debug0(
				VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "mp4_parser_parse_frames: vod_array_push failed"
			);
			return VOD_ALLOC_FAILED;
		}
		...
		rc = frames_source_cache_init(
			request_context, read_cache_state, source, media_type, &frames_source_context
		);
		if (rc != VOD_OK) {
			return rc;
		}
		...
		if (context.encryption_info.auxiliary_info < context.encryption_info.auxiliary_info_end) {
			...
			if (source->encryption.key.len != MP4_AES_CTR_KEY_SIZE) {
				vod_log_error(
					...
					request_context->log,
					0,
					"mp4_parser_parse_frames: invalid encryption key len %uz",
			...
			if (source->encryption.scheme != MCS_ENC_CENC) {
				vod_log_error(
					...
					request_context->log,
					0,
					"mp4_parser_parse_frames: invalid encryption scheme type %d",
		...
		if (context.frame_count > 0
		    && vod_all_flags_set(
				parse_params->parse_type, PARSE_FLAG_FRAMES_SIZE | PARSE_FLAG_FRAMES_OFFSET
			)) {
		...
		if ((parse_params->parse_type & PARSE_FLAG_SAVE_RAW_ATOMS) != 0) {
			rc = mp4_parser_copy_raw_atoms(
				request_context, result_track->raw_atoms, &cur_track->trak_atom_infos
			);
			if (rc != VOD_OK) {
				return rc;
			}
...
mp4_parser_uncompress_moov(
	...
	rc = mp4_parser_parse_atoms(
		request_context, buffer, size, TRUE, &mp4_parser_save_relevant_atoms_callback, &save_atoms_context
	);
	if (rc != VOD_OK) {
		return rc;
	}
	...
	if (moov_atom_infos.dcom.ptr == NULL || moov_atom_infos.cmvd.ptr == NULL) {
		*out_buffer = NULL;
		return VOD_OK; // uncompressed or corrupt, if corrupt, will fail in trak parsing
	}
	...
	if (moov_atom_infos.dcom.size < sizeof(*dcom)) {
		vod_log_error(
			...
			request_context->log,
			0,
			"mp4_parser_uncompress_moov: dcom atom size %uL too small",
			moov_atom_infos.dcom.size
		);
		return VOD_BAD_DATA;
	}
	...
	if (parse_le32(dcom->type) != DCOM_TYPE_ZLIB) {
		vod_log_error(
			...
			request_context->log,
			0,
			"mp4_parser_uncompress_moov: dcom type %*s is not zlib",
			(size_t)sizeof(dcom->type),
			dcom->type
		);
		return VOD_BAD_DATA;
	}
	...
	if (moov_atom_infos.cmvd.size < sizeof(*cmvd)) {
		vod_log_error(
			...
			request_context->log,
			0,
			"mp4_parser_uncompress_moov: cmvd atom size %uL too small",
			moov_atom_infos.cmvd.size
		);
		return VOD_BAD_DATA;
	}
	...
	if (alloc_size > max_moov_size) {
		vod_log_error(
			...
			request_context->log,
			0,
			"mp4_parser_uncompress_moov: moov size %uz exceeds the max %uz",
			alloc_size,
			max_moov_size
		);
		return VOD_BAD_DATA;
	...
	if (uncomp_buffer == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "mp4_parser_uncompress_moov: vod_alloc failed"
		);
		return VOD_ALLOC_FAILED;
	}
	...
	if (zrc != Z_OK) {
		vod_log_error(
			VOD_LOG_ERR, request_context->log, 0, "mp4_parser_uncompress_moov: uncompress failed %d", zrc
		);
		return VOD_BAD_DATA;
	}
	...
	mp4_parser_parse_atoms(
		request_context, uncomp_buffer, uncomp_size, TRUE, &mp4_parser_find_atom_callback, &find_context
	);
	if (find_context.ptr == NULL) {
		vod_log_error(
			VOD_LOG_ERR, request_context->log, 0, "mp4_parser_uncompress_moov: failed to find moov atom"
		);
		return VOD_BAD_DATA;
	}
