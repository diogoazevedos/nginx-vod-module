...
// maximum number of attempts to find the moov atom start for non-fast-start files
#define MAX_MOOV_START_READS (4)
...
static vod_status_t
mp4_reader_identify_callback(void* context, atom_info_t* atom_info) {
...
mp4_metadata_reader_init(
	...
	mp4_parser_parse_atoms(
		request_context, buffer->data, buffer->len, FALSE, mp4_reader_identify_callback, &atom_found
	);
	if (!atom_found) {
	...
	if (state == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "mp4_metadata_reader_init: vod_alloc failed"
		);
...
mp4_metadata_reader_read(
	...
	if (state->parts[MP4_METADATA_PART_FTYP].len == 0) {
		...
		rc = mp4_parser_get_ftyp_atom_into(
			state->request_context, buffer->data, buffer->len, &ftyp_ptr, &ftyp_size
		);
		if (rc != VOD_OK) {
			vod_log_debug1(
				VOD_LOG_DEBUG_LEVEL,
				state->request_context->log,
				0,
				"mp4_metadata_reader_read: mp4_parser_get_ftyp_atom_into failed %i",
				rc
			);
			return rc;
		}

		if (ftyp_size > 0 && ftyp_ptr + ftyp_size <= buffer->len + buffer->data) {
			// got a full ftyp atom
			state->parts[MP4_METADATA_PART_FTYP].data =
				vod_alloc(state->request_context->pool, ftyp_size);
			if (state->parts[MP4_METADATA_PART_FTYP].data == NULL) {
				vod_log_debug0(
					VOD_LOG_DEBUG_LEVEL, state->request_context->log, 0, "mp4_metadata_reader_read: vod_alloc failed"
				);
				return VOD_ALLOC_FAILED;
			}
			...
			vod_log_debug0(
				VOD_LOG_DEBUG_LEVEL, state->request_context->log, 0, "mp4_metadata_reader_read: ftyp atom not found"
			);
		}
	}
	...
	rc = mp4_parser_get_moov_atom_info(
		state->request_context, buffer->data, buffer->len, &moov_offset, &moov_size
	);
	if (rc != VOD_OK) {
		vod_log_debug1(
			VOD_LOG_DEBUG_LEVEL,
			state->request_context->log,
			0,
			"mp4_metadata_reader_read: mp4_parser_get_moov_atom_info failed %i",
			rc
		);
		return rc;
	}
	...
	if (moov_size <= 0) {
		...
		if (state->moov_start_reads <= 0) {
			vod_log_error(
				VOD_LOG_ERR, state->request_context->log, 0, "mp4_metadata_reader_read: exhausted all moov read attempts"
			);
			return VOD_BAD_DATA;
		}
	...
	if (moov_offset + moov_size <= buffer->len) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, state->request_context->log, 0, "mp4_metadata_reader_read: already read the full moov atom"
		);
		goto done;
	}
	...
	if (rc != VOD_OK) {
		vod_log_debug1(
			...
			state->request_context->log,
			0,
			"mp4_metadata_reader_read: mp4_parser_uncompress_moov failed %i",
			rc
		);
		return rc;
	}
