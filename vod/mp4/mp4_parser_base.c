...
mp4_parser_parse_atoms(
	...
	while (cur_pos + ATOM_HEADER_SIZE <= end_pos) {
		...
		vod_log_debug3(
			...
			request_context->log,
			0,
			"mp4_parser_parse_atoms: atom name=%*s, size=%uL",
			(size_t)sizeof(atom_info.name),
			(char*)&atom_info.name,
			atom_size
		);
		...
		if (atom_size < atom_info.header_size) {
			if (validate_full_atom) {
				vod_log_error(
					...
					request_context->log,
					0,
					"mp4_parser_parse_atoms: atom size %uL is less than the atom header size %ud",
					atom_size,
					atom_info.header_size
				);
			}
			return VOD_BAD_DATA;
		}
		...
		if (atom_size > (uint64_t)(end_pos - cur_pos)) {
			if (validate_full_atom) {
				vod_log_error(
					...
					request_context->log,
					0,
					"mp4_parser_parse_atoms: atom size %uL overflows the input stream size %uL",
					atom_size,
					(uint64_t)(end_pos - cur_pos)
				);
				return VOD_BAD_DATA;
			}
...
mp4_parser_validate_stts_data(request_context_t* request_context, atom_info_t* atom_info, uint32_t* entries) {
	...
	if (atom_info->size < sizeof(*atom)) {
		vod_log_error(
			...
			request_context->log,
			0,
			"mp4_parser_validate_stts_data: atom size %uL too small",
			atom_info->size
		);
		return VOD_BAD_DATA;
	}
	...
	*entries = parse_be32(atom->entries);
	if (*entries >= (INT_MAX - sizeof(*atom)) / sizeof(stts_entry_t)) { // integer overflow protection
		vod_log_error(
			...
			request_context->log,
			0,
			"mp4_parser_validate_stts_data: number of entries %uD too big",
			*entries
		);
		return VOD_BAD_DATA;
	}
	...
	if (atom_info->size < sizeof(*atom) + *entries * sizeof(stts_entry_t)) {
		vod_log_error(
			...
			request_context->log,
			0,
			"mp4_parser_validate_stts_data: atom size %uL too small to hold %uD entries",
			atom_info->size,
			*entries
		);
		return VOD_BAD_DATA;
...
mp4_parser_validate_stss_atom(request_context_t* request_context, atom_info_t* atom_info, uint32_t* entries) {
	...
	if (atom_info->size < sizeof(*atom)) {
		vod_log_error(
			...
			request_context->log,
			0,
			"mp4_parser_validate_stss_atom: atom size %uL too small",
			atom_info->size
		);
		return VOD_BAD_DATA;
	}
	...
	*entries = parse_be32(atom->entries);
	if (*entries >= (INT_MAX - sizeof(*atom)) / sizeof(uint32_t)) { // integer overflow protection
		vod_log_error(
			...
			request_context->log,
			0,
			"mp4_parser_validate_stss_atom: number of entries %uD too big",
			*entries
		);
		return VOD_BAD_DATA;
	}
	...
	if (atom_info->size < sizeof(*atom) + *entries * sizeof(uint32_t)) {
		vod_log_error(
			...
			request_context->log,
			0,
			"mp4_parser_validate_stss_atom: atom size %uL too small to hold %uD entries",
			atom_info->size,
			*entries
		);
		return VOD_BAD_DATA;
...
mp4_parser_validate_ctts_atom(request_context_t* request_context, atom_info_t* atom_info, uint32_t* entries) {
	...
	if (atom_info->size < sizeof(*atom)) {
		vod_log_error(
			...
			request_context->log,
			0,
			"mp4_parser_validate_ctts_atom: atom size %uL too small",
			atom_info->size
		);
		return VOD_BAD_DATA;
	}
	...
	*entries = parse_be32(atom->entries);
	if (*entries <= 0) {
		vod_log_error(VOD_LOG_ERR, request_context->log, 0, "mp4_parser_validate_ctts_atom: zero entries");
		return VOD_BAD_DATA;
	}
	...
	if (*entries >= (INT_MAX - sizeof(*atom)) / sizeof(ctts_entry_t)) { // integer overflow protection
		vod_log_error(
			...
			request_context->log,
			0,
			"mp4_parser_validate_ctts_atom: number of entries %uD too big",
			*entries
		);
		return VOD_BAD_DATA;
	}
	...
	if (atom_info->size < sizeof(*atom) + *entries * sizeof(ctts_entry_t)) {
		vod_log_error(
			...
			request_context->log,
			0,
			"mp4_parser_validate_ctts_atom: atom size %uL too small to hold %uD entries",
			atom_info->size,
			*entries
		);
		return VOD_BAD_DATA;
...
mp4_parser_validate_stsc_atom(request_context_t* request_context, atom_info_t* atom_info, uint32_t* entries) {
	...
	if (atom_info->size < sizeof(*atom)) {
		vod_log_error(
			...
			request_context->log,
			0,
			"mp4_parser_validate_stsc_atom: atom size %uL too small",
			atom_info->size
		);
		return VOD_BAD_DATA;
	}
	...
	*entries = parse_be32(atom->entries);
	if (*entries <= 0) {
		vod_log_error(VOD_LOG_ERR, request_context->log, 0, "mp4_parser_validate_stsc_atom: zero entries");
		return VOD_BAD_DATA;
	}
	...
	if (*entries >= (INT_MAX - sizeof(*atom)) / sizeof(stsc_entry_t)) { // integer overflow protection
		vod_log_error(
			...
			request_context->log,
			0,
			"mp4_parser_validate_stsc_atom: number of entries %uD too big",
			*entries
		);
		return VOD_BAD_DATA;
	}
	...
	if (atom_info->size < sizeof(*atom) + *entries * sizeof(stsc_entry_t)) {
		vod_log_error(
			...
			request_context->log,
			0,
			"mp4_parser_validate_stsc_atom: atom size %uL too small to hold %uD entries",
			atom_info->size,
			*entries
		);
		return VOD_BAD_DATA;
...
mp4_parser_validate_stsz_atom(
	...
	if (atom_info->size < sizeof(*atom)) {
		vod_log_error(
			...
			request_context->log,
			0,
			"mp4_parser_validate_stsz_atom: atom size %uL too small",
			atom_info->size
		);
		return VOD_BAD_DATA;
	}
	...
	if (atom_info->name == ATOM_NAME_STZ2) {
		*field_size = atom2->field_size[0];
		if (*field_size == 0) { // protect against division by zero
			vod_log_error(
				VOD_LOG_ERR, request_context->log, 0, "mp4_parser_validate_stsz_atom: field size is zero"
			);
			return VOD_BAD_DATA;
		}
		*uniform_size = 0;
		...
		if (*uniform_size != 0) {
			if (*uniform_size > MAX_FRAME_SIZE) {
				vod_log_error(
					...
					request_context->log,
					0,
					"mp4_parser_validate_stsz_atom: uniform size %uD is too big",
					*uniform_size
				);
				return VOD_BAD_DATA;
			}
	...
	*entries = parse_be32(atom->entries);
	if (*entries < last_frame) {
		vod_log_error(
			...
			request_context->log,
			0,
			"mp4_parser_validate_stsz_atom: number of entries %uD smaller than last frame %uD",
			*entries,
			last_frame
		);
		return VOD_BAD_DATA;
	...
	if (*entries >= INT_MAX / *field_size) { // integer overflow protection
		vod_log_error(
			...
			request_context->log,
			0,
			"mp4_parser_validate_stsz_atom: number of entries %uD too big for size %ud bits",
			*entries,
			*field_size
		);
		return VOD_BAD_DATA;
	...
	if (atom_info->size < sizeof(*atom) + vod_div_ceil(*entries * *field_size, 8)) {
		vod_log_error(
			...
			request_context->log,
			0,
			"mp4_parser_validate_stsz_atom: atom size %uL too small to hold %uD entries of %ud bits",
			atom_info->size,
			*entries,
			*field_size
		);
...
mp4_parser_validate_stco_data(
	...
	if (atom_info->size < sizeof(*atom)) {
		vod_log_error(
			...
			request_context->log,
			0,
			"mp4_parser_validate_stco_data: atom size %uL too small",
			atom_info->size
		);
		return VOD_BAD_DATA;
	}
	...
	*entries = parse_be32(atom->entries);
	if (*entries < last_chunk_index) {
		vod_log_error(
			...
			request_context->log,
			0,
			"mp4_parser_validate_stco_data: number of entries %uD smaller than last chunk %uD",
			*entries,
			last_chunk_index
		);
		return VOD_BAD_DATA;
	...
	if (*entries >= (INT_MAX - sizeof(*atom)) / (*entry_size)) { // integer overflow protection
		vod_log_error(
			...
			request_context->log,
			0,
			"mp4_parser_validate_stco_data: number of entries %uD too big",
			*entries
		);
		return VOD_BAD_DATA;
	}
	...
	if (atom_info->size < sizeof(*atom) + (*entries) * (*entry_size)) {
		vod_log_error(
			...
			request_context->log,
			0,
			"mp4_parser_validate_stco_data: atom size %uL too small to hold %uD entries",
			atom_info->size,
			*entries
		);
		return VOD_BAD_DATA;
...
bool_t
mp4_parser_stts_iterator(stts_iterator_state_t* iterator, uint64_t offset) {
	...
	for (;;) {
		...
		// to use div_ceil
		if (offset != ULLONG_MAX && sample_duration > 0 && offset < next_accum_duration) {
			break;
		}
...
bool_t
mp4_parser_ctts_iterator(ctts_iterator_state_t* iterator, uint32_t required_index) {
...
mp4_parser_stsc_iterator_init(
	...
	if (iterator->cur_chunk < 1) {
		vod_log_error(
			VOD_LOG_ERR, iterator->request_context->log, 0, "mp4_parser_stsc_iterator_init: chunk index is zero"
		);
		return VOD_BAD_DATA;
	}
	...
	if (iterator->samples_per_chunk == 0) {
		vod_log_error(
			VOD_LOG_ERR, iterator->request_context->log, 0, "mp4_parser_stsc_iterator_init: samples per chunk is zero"
		);
		return VOD_BAD_DATA;
	}
	...
	*prev_samples = 0;
	...
	for (; cur_entry + 1 < last_entry; cur_entry++, frame_index += cur_entry_samples) {
		...
		if (next_chunk <= cur_chunk) {
			vod_log_error(
				VOD_LOG_ERR,
				iterator->request_context->log,
				0,
				"mp4_parser_stsc_iterator: chunk index %uD is smaller than the previous index %uD (1)",
				next_chunk,
				cur_chunk
			);
			return VOD_BAD_DATA;
		...
		if (next_chunk - cur_chunk
		    > (UINT_MAX - frame_index) / samples_per_chunk) { // integer overflow protection
			vod_log_error(
				VOD_LOG_ERR,
				iterator->request_context->log,
				0,
				"mp4_parser_stsc_iterator: chunk index %uD is too big for previous index %uD and samples per chunk %uD",
				next_chunk,
				cur_chunk,
				samples_per_chunk
			);
		...
		*prev_samples = samples_per_chunk;
		...
		if (samples_per_chunk == 0) {
			vod_log_error(
				VOD_LOG_ERR, iterator->request_context->log, 0, "mp4_parser_stsc_iterator: samples per chunk is zero"
			);
			return VOD_BAD_DATA;
		}
	...
	if (frame_index + cur_entry_samples < required_index) {
		vod_log_error(
			...
			iterator->request_context->log,
			0,
			"mp4_parser_stsc_iterator: required index %uD exceeds stsc indexes %uD",
			required_index,
			frame_index + cur_entry_samples
		);
		return VOD_BAD_DATA;
	}
