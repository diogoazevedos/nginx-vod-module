...
static const u_char ftyp_atom_v2[] = {
	...
	0x00, 0x00, 0x00, 0x01, // minor version = 1
	0x69, 0x73, 0x6F, 0x35, // compatible brand = iso5
...
static const u_char smpte_tt_stsd_atom[] = {
	...
	0x6D, 0x70, 0x74, 0x65, //     http://www.w3.org/ns/ttml#styling http://www.w3.org/ns/ttml
	0x2D, 0x72, 0x61, 0x2E, //     http://www.w3.org/ns/ttml#metadata urn:ebu:tt:metadata
	0x6F, 0x72, 0x67, 0x2F, //     urn:ebu:tt:style\0
	0x73, 0x63, 0x68, 0x65,
	0x6D, 0x61, 0x73, 0x2F,
	0x32, 0x30, 0x35, 0x32,
	0x2D, 0x31, 0x2F, 0x32,
...
static u_char*
mp4_init_segment_write_matrix(u_char* p, int16_t a, int16_t b, int16_t c, int16_t d, int16_t tx, int16_t ty) {
...
mp4_init_segment_write_tkhd_atom(
	u_char* p, uint32_t track_id, uint32_t duration, uint32_t media_type, uint16_t width, uint16_t height
) {
...
mp4_init_segment_write_tkhd64_atom(
	u_char* p, uint32_t track_id, uint64_t duration, uint32_t media_type, uint16_t width, uint16_t height
) {
...
mp4_init_segment_write_esds_atom(u_char* p, media_info_t* media_info) {
	...
	*p++ = sizeof(config_descr_t) + sizeof(descr_header_t) + extra_data_len; // len
...
mp4_init_segment_calc_size(
	...
	result->total_size = (media_set->version >= 2 ? sizeof(ftyp_atom_v2) : sizeof(ftyp_atom))
...
mp4_init_segment_build_stsd_atom(request_context_t* request_context, media_track_t* track) {
	...
	if (p == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "mp4_init_segment_build_stsd_atom: vod_alloc failed (1)"
		);
		return VOD_ALLOC_FAILED;
	}

	track->raw_atoms[RTA_STSD].ptr = p;
	track->raw_atoms[RTA_STSD].size = mp4_init_segment_write_stsd_atom(p, atom_size, track) - p;
...
mp4_init_segment_build(
	...
	sizes = vod_alloc(
		...
		sizeof(*sizes) + sizeof(sizes->track_sizes[0]) * (media_set->total_track_count - 1)
	);
	if (sizes == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "mp4_init_segment_build: vod_alloc failed (1)"
		);
		return VOD_ALLOC_FAILED;
	}
	...
	if (result->data == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "mp4_init_segment_build: vod_alloc failed (2)"
		);
		return VOD_ALLOC_FAILED;
	}
...
mp4_init_segment_init_encrypted_stsd_writer(
	request_context_t* request_context, media_track_t* track, stsd_writer_context_t* result
) {
	...
	if (original_stsd->size
	    < original_stsd->header_size + sizeof(stsd_atom_t) + sizeof(stsd_entry_header_t)) {
		vod_log_error(
			VOD_LOG_ERR,
			request_context->log,
			0,
			"mp4_init_segment_init_encrypted_stsd_writer: invalid stsd size %uL",
			original_stsd->size
		);
		return VOD_BAD_DATA;
	}
	...
	result->original_stsd_entry =
		(stsd_entry_header_t*)(original_stsd->ptr + original_stsd->header_size + sizeof(stsd_atom_t));
	...
	if (result->original_stsd_entry_size < sizeof(stsd_entry_header_t)
	    || result->original_stsd_entry_size
	           > original_stsd->size - original_stsd->header_size - sizeof(stsd_atom_t)) {
		vod_log_error(
			VOD_LOG_ERR,
			request_context->log,
			0,
			"mp4_init_segment_init_encrypted_stsd_writer: invalid stsd entry size %uD",
			result->original_stsd_entry_size
		);
		return VOD_BAD_DATA;
	}
	...
	result->sinf_atom_size =
		ATOM_HEADER_SIZE + result->frma_atom_size + result->schm_atom_size + result->schi_atom_size;
...
mp4_init_segment_write_encrypted_stsd(void* ctx, u_char* p) {
	...
	p = vod_copy(
		p, context->original_stsd_entry + 1, context->original_stsd_entry_size - sizeof(stsd_entry_header_t)
	);
...
mp4_init_segment_get_encrypted_stsd_writers(
	...
	stsd_atom_writer = vod_alloc(
		request_context->pool,
		(sizeof(*stsd_atom_writer) + sizeof(*stsd_writer_context)) * media_set->total_track_count
	);
	if (stsd_atom_writer == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "mp4_init_segment_get_encrypted_stsd_writers: vod_alloc failed"
		);
		return VOD_ALLOC_FAILED;
	}
	...
	*result = stsd_atom_writer;
	...
	for (; cur_track < last_track; cur_track++, stsd_atom_writer++) {
		...
		rc = mp4_init_segment_init_encrypted_stsd_writer(request_context, cur_track, stsd_writer_context);
		if (rc != VOD_OK) {
			vod_log_debug1(
				VOD_LOG_DEBUG_LEVEL,
				request_context->log,
				0,
				"mp4_init_segment_get_encrypted_stsd_writers: mp4_init_segment_init_encrypted_stsd_writer failed %i",
				rc
			);
			return rc;
		}
