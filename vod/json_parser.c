...
#define ASSERT_CHAR(state, ch)                                                                             \
	if (*(state)->cur_pos != ch) {                                                                         \
		vod_snprintf(                                                                                      \
			state->error, state->error_size, "expected 0x%xd got 0x%xd%Z", (int)ch, (int)*(state)->cur_pos \
		);                                                                                                 \
...
// globals
static vod_json_type_t vod_json_string = {VOD_JSON_STRING, sizeof(vod_str_t), vod_json_parser_string};

static vod_json_type_t vod_json_array = {VOD_JSON_ARRAY, sizeof(vod_json_array_t), vod_json_parser_array};

static vod_json_type_t vod_json_object = {
	VOD_JSON_OBJECT, sizeof(vod_json_object_t), vod_json_parser_object
};

static vod_json_type_t vod_json_bool = {VOD_JSON_BOOL, sizeof(bool_t), vod_json_parser_bool};

static vod_json_type_t vod_json_frac = {VOD_JSON_FRAC, sizeof(vod_json_fraction_t), vod_json_parser_frac};

static vod_json_type_t vod_json_int = {VOD_JSON_INT, sizeof(int64_t), vod_json_parser_int};
...
vod_json_parse_fraction(vod_json_parser_state_t* state, vod_json_fraction_t* result) {
	...
	if (*state->cur_pos == '.') {
		...
		if (!isdigit(*state->cur_pos)) {
			vod_snprintf(
				state->error, state->error_size, "expected digit got 0x%xd%Z", (int)*state->cur_pos
			);
			return VOD_JSON_BAD_DATA;
		}
...
vod_status_t
vod_json_init_hash(
	vod_pool_t* pool, vod_pool_t* temp_pool, char* hash_name, void* elements, size_t element_size, vod_hash_t* result
) {
...
void
vod_json_get_object_values(vod_json_object_t* object, vod_hash_t* values_hash, vod_json_value_t** result) {
	...
	for (; cur_element < last_element; cur_element++) {
		key_def = vod_hash_find(
			values_hash, cur_element->key_hash, cur_element->key.data, cur_element->key.len
		);
		if (key_def == NULL) {
			continue;
		}
		...
		if (cur_element->value.type == key_def->type
		    // allow passing int for a fraction
		    || (cur_element->value.type == VOD_JSON_INT && key_def->type == VOD_JSON_FRAC)) {
...
vod_status_t
vod_json_parse_object_values(vod_json_object_t* object, vod_hash_t* values_hash, void* context, void* result) {
	vod_json_key_value_t* cur_element = object->elts;
	vod_json_key_value_t* last_element = cur_element + object->nelts;
...
vod_json_parse_union(
	...
	for (; cur < last; cur++) {
		...
		if (cur->value.type != VOD_JSON_STRING) {
			vod_log_error(
				...
				request_context->log,
				0,
				"vod_json_parse_union: \"%V\" field has an invalid type %d",
				type_field,
				cur->value.type
			);
	...
	if (type.len == 0) {
		vod_log_error(
			VOD_LOG_ERR, request_context->log, 0, "vod_json_parse_union: missing \"%V\" field", type_field
		);
		return VOD_BAD_REQUEST;
	}
	...
	type_def = vod_hash_find(union_hash, key, type.data, type.len);
	if (type_def == NULL) {
		vod_log_error(
			VOD_LOG_ERR, request_context->log, 0, "vod_json_parse_union: unknown object type \"%V\"", &type
		);
		return VOD_BAD_REQUEST;
	}
...
vod_json_replace_array(vod_json_array_t* array1, vod_json_array_t* array2) {
	...
	for (cur_object1 = part1->first, cur_object2 = part2->first;; cur_object1++, cur_object2++) {
