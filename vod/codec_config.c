...
codec_config_avcc_get_nal_units(
	...
	if (extra_data->len < sizeof(avcc_config_t)) {
		vod_log_error(
			...
			request_context->log,
			0,
			"codec_config_avcc_get_nal_units: extra data size %uz too small",
			extra_data->len
		);
		return VOD_BAD_DATA;
	}
	...
	*nal_packet_size_length = (((avcc_config_t*)extra_data->data)->nula_length_size & 0x3) + 1;
		...
		for (unit_count = (*cur_pos++ & 0x1f); unit_count; unit_count--) {
			...
			if (cur_pos + unit_size > extra_data_end) {
				vod_log_error(
					...
					request_context->log,
					0,
					"codec_config_avcc_get_nal_units: unit size %uD overflows the extra data buffer",
					(uint32_t)unit_size
				);
				return VOD_BAD_DATA;
			}
	...
	if (p == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "codec_config_avcc_get_nal_units: vod_alloc failed"
		);
	...
	vod_log_buffer(
		...
		request_context->log,
		0,
		"codec_config_avcc_get_nal_units: parsed extra data ",
		result->data,
		result->len
	);
	return VOD_OK;
...
codec_config_hevc_config_parse(
	...
	if (reader.stream.eof_reached) {
		vod_log_error(
			VOD_LOG_ERR, request_context->log, 0, "codec_config_hevc_config_parse: overflow while parsing hevc config"
		);
		return VOD_BAD_DATA;
	}
...
codec_config_hevc_get_nal_units(
	...
	*nal_packet_size_length = cfg.nal_unit_size;
	...
	if (p == NULL) {
		vod_log_debug0(
			VOD_LOG_DEBUG_LEVEL, request_context->log, 0, "codec_config_hevc_get_nal_units: vod_alloc failed"
		);
		return VOD_ALLOC_FAILED;
	}
	result->data = p;
	...
	vod_log_buffer(
		...
		request_context->log,
		0,
		"codec_config_hevc_get_nal_units: parsed extra data ",
		result->data,
		result->len
	);
	return VOD_OK;
...
codec_config_get_avc_codec_name(request_context_t* request_context, media_info_t* media_info) {
	...
	if (media_info->extra_data.len < sizeof(avcc_config_t)) {
		vod_log_error(
			VOD_LOG_ERR, request_context->log, 0, "codec_config_get_avc_codec_name: extra data too small"
		);
		return VOD_BAD_DATA;
	}
...
codec_config_get_av1_codec_name(request_context_t* request_context, media_info_t* media_info) {
	...
	if (marker != 1) {
		vod_log_error(
			VOD_LOG_ERR, request_context->log, 0, "codec_config_get_av1_codec_name: invalid marker %uD", marker
		);
		return VOD_BAD_DATA;
	}
	...
	if (version != 1) {
		vod_log_error(
			VOD_LOG_ERR, request_context->log, 0, "codec_config_get_av1_codec_name: invalid version %uD", version
		);
		return VOD_BAD_DATA;
	}
	...
	if (reader.stream.eof_reached) {
		vod_log_error(
			VOD_LOG_ERR, request_context->log, 0, "codec_config_get_av1_codec_name: overflow while parsing av1 config"
		);
		return VOD_BAD_DATA;
	}
...
codec_config_mp4a_config_parse(
	request_context_t* request_context, vod_str_t* extra_data, media_info_t* media_info
) {
	bit_reader_state_t reader;
	...
	vod_log_buffer(
		...
		request_context->log,
		0,
		"codec_config_mp4a_config_parse: extra data ",
		extra_data->data,
		extra_data->len
	);
