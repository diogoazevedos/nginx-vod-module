...
// globals
static ngx_str_t ngx_http_vod_last_modified_default_types[] = {ngx_null_string};
...
ngx_http_vod_init_parsers(ngx_conf_t* cf) {
	...
	if (rc != VOD_OK) {
		ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "failed to initialize media set parsers %i", rc);
		return NGX_ERROR;
	}

	rc = udrm_init_parser(cf->pool, cf->temp_pool);
	if (rc != VOD_OK) {
		ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "failed to initialize udrm parser %i", rc);
		return NGX_ERROR;
	}

	rc = ngx_child_request_init(cf);
	if (rc != VOD_OK) {
		ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "failed to initialize hide headers hash %i", rc);
		return NGX_ERROR;
	}
...
ngx_http_vod_merge_loc_conf(ngx_conf_t* cf, void* parent, void* child) {
	...
	ngx_conf_merge_ptr_value(
		conf->request_handler, prev->request_handler, ngx_http_vod_local_request_handler
	);
	...
	ngx_conf_merge_ptr_value(
		conf->segmenter.get_segment_count, prev->segmenter.get_segment_count, segmenter_get_segment_count_last_short
	);
	ngx_conf_merge_ptr_value(
		conf->segmenter.get_segment_durations,
		prev->segmenter.get_segment_durations,
		segmenter_get_segment_durations_estimate
	);
	ngx_conf_merge_uint_value(
		conf->segmenter.manifest_duration_policy, prev->segmenter.manifest_duration_policy, MDP_MAX
	);
	...
	ngx_conf_merge_str_value(
		conf->path_response_prefix,
		prev->path_response_prefix,
	...
	if (ngx_http_merge_types(
			...
			ngx_http_vod_last_modified_default_types
		)
	    != NGX_OK) {
		return NGX_CONF_ERROR;
	}
	...
	ngx_conf_merge_uint_value(
		conf->min_single_nalu_per_frame_segment, prev->min_single_nalu_per_frame_segment, 0
	);
	...
	if (conf->request_handler == ngx_http_vod_remote_request_handler) {
		if (conf->upstream_location.len == 0) {
			ngx_conf_log_error(
				NGX_LOG_EMERG, cf, 0, "\"vod_upstream_location\" is mandatory for remote mode"
			);
			return NGX_CONF_ERROR;
		}
	}
	...
	if (conf->segmenter.segment_duration <= 0) {
		ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "\"vod_segment_duration\" must be positive");
		return NGX_CONF_ERROR;
	}
	...
	if (rc != VOD_OK) {
		ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "failed to initialize the segmenter %i", rc);
		return NGX_CONF_ERROR;
	}

	if (conf->drm_enabled) {
		if (conf->drm_upstream_location.len == 0) {
			ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "\"vod_drm_upstream_location\" is mandatory for drm");
			return NGX_CONF_ERROR;
		}
	}
	...
	if (conf->clip_to_param_name.len > MAX_URI_PARAM_NAME_LEN) {
		ngx_conf_log_error(
			NGX_LOG_EMERG, cf, 0, "\"vod_clip_to_param_name\" should not be more than %d characters", MAX_URI_PARAM_NAME_LEN
		);
		return NGX_CONF_ERROR;
	}
	...
	if (conf->clip_from_param_name.len > MAX_URI_PARAM_NAME_LEN) {
		ngx_conf_log_error(
			NGX_LOG_EMERG, cf, 0, "\"vod_clip_from_param_name\" should not be more than %d characters", MAX_URI_PARAM_NAME_LEN
		);
		return NGX_CONF_ERROR;
	}
	...
	if (conf->tracks_param_name.len > MAX_URI_PARAM_NAME_LEN) {
		ngx_conf_log_error(
			NGX_LOG_EMERG, cf, 0, "\"vod_tracks_param_name\" should not be more than %d characters", MAX_URI_PARAM_NAME_LEN
		);
		return NGX_CONF_ERROR;
	}
	...
	if (conf->time_shift_param_name.len > MAX_URI_PARAM_NAME_LEN) {
		ngx_conf_log_error(
			NGX_LOG_EMERG, cf, 0, "\"vod_time_shift_param_name\" should not be more than %d characters", MAX_URI_PARAM_NAME_LEN
		);
		return NGX_CONF_ERROR;
	}
	...
	if (conf->speed_param_name.len > MAX_URI_PARAM_NAME_LEN) {
		ngx_conf_log_error(
			NGX_LOG_EMERG, cf, 0, "\"vod_speed_param_name\" should not be more than %d characters", MAX_URI_PARAM_NAME_LEN
		);
		return NGX_CONF_ERROR;
	}
	...
	if (conf->lang_param_name.len > MAX_URI_PARAM_NAME_LEN) {
		ngx_conf_log_error(
			NGX_LOG_EMERG, cf, 0, "\"vod_lang_param_name\" should not be more than %d characters", MAX_URI_PARAM_NAME_LEN
		);
		return NGX_CONF_ERROR;
	}
	...
	if (rc != NGX_OK) {
		ngx_log_debug0(
			NGX_LOG_DEBUG_HTTP, cf->log, 0, "ngx_http_vod_merge_loc_conf: ngx_http_vod_init_uri_params_hash failed"
		);
		return NGX_CONF_ERROR;
	}
	...
	for (cur_module = submodules; *cur_module != NULL; cur_module++) {
		...
		if (err != NGX_CONF_OK) {
			ngx_log_debug0(
				NGX_LOG_DEBUG_HTTP, cf->log, 0, "ngx_http_vod_merge_loc_conf: submodule merge_loc_conf failed"
			);
			return err;
		}
	}
...
ngx_http_vod_perf_counters_command(ngx_conf_t* cf, ngx_command_t* cmd, void* conf) {
	...
	*zone = ngx_perf_counters_create_zone(cf, &value[1], &ngx_http_vod_module);
	if (*zone == NULL) {
		ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "failed to create perf counters cache zone");
		return NGX_CONF_ERROR;
	}
...
ngx_command_t ngx_http_vod_commands[] = {
	...
	// basic parameters
	{ngx_string("vod"), NGX_HTTP_LOC_CONF | NGX_CONF_TAKE1, ngx_http_vod, NGX_HTTP_LOC_CONF_OFFSET, 0, NULL},
	...
	{ngx_string("vod_status"), NGX_HTTP_LOC_CONF | NGX_CONF_NOARGS, ngx_http_vod_status, 0, 0, NULL},
