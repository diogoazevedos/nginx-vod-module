...
for letter in range(26):
	curCodes = list(filter(lambda x: x.startswith(chr(ord('a') + letter)), codes))
	langIntCodes = list(map(lambda x: (((ord(x[0]) - 96) & 0x1f) << 10) | (((ord(x[1]) - 96) & 0x1f) << 5) | ((ord(x[2]) - 96) & 0x1f), curCodes))
