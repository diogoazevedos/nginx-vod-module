...
ngx_http_vod_thumb_get_url(
	...
	rc = ngx_http_vod_get_base_url(
		r, conf->segments_base_url != NULL ? conf->segments_base_url : conf->base_url, &r->uri, &base_url
	);
	if (rc != VOD_OK) {
		ngx_log_debug1(
			NGX_LOG_DEBUG_HTTP,
			...
			0,
			"ngx_http_vod_thumb_get_url: ngx_http_vod_get_base_url failed %i",
			rc
		);
		return rc;
	}
	...
	if (rc != VOD_OK) {
		ngx_log_debug1(
			...
			submodule_context->request_context.log,
			0,
			"ngx_http_vod_thumb_get_url: manifest_utils_build_request_params_string failed %i",
			rc
		);
	...
	result_size = base_url.len
	            + conf->thumb.file_name_prefix.len
	            + 1 // '-'
	            + VOD_INT64_LEN
	            + request_params_str.len
	            + (sizeof(jpg_file_ext) - 1);
...
ngx_http_vod_thumb_init_frame_processor(
	...
	if (rc != VOD_OK) {
		ngx_log_debug1(
			...
			submodule_context->request_context.log,
			0,
			"ngx_http_vod_thumb_init_frame_processor: thumb_grabber_init_state failed %i",
			rc
		);
...
static void
ngx_http_vod_thumb_create_loc_conf(ngx_conf_t* cf, ngx_http_vod_thumb_loc_conf_t* conf) {
...
ngx_http_vod_thumb_parse_dimensions(
	ngx_http_request_t* r, u_char* start_pos, u_char* end_pos, request_params_t* result
) {
...
ngx_http_vod_thumb_parse_uri_file_name(
	...
	if (ngx_http_vod_match_prefix_postfix(start_pos, end_pos, &conf->thumb.file_name_prefix, jpg_file_ext)) {
		...
		*request = &thumb_request;
		...
		ngx_log_error(
			NGX_LOG_ERR, r->connection->log, 0, "ngx_http_vod_thumb_parse_uri_file_name: unidentified request"
		);
	...
	if (start_pos >= end_pos || *start_pos < '0' || *start_pos > '9') {
		ngx_log_error(
			NGX_LOG_ERR, r->connection->log, 0, "ngx_http_vod_thumb_parse_uri_file_name: failed to parse thumbnail time"
		);
		return ngx_http_vod_status_to_ngx_error(r, VOD_BAD_REQUEST);
	}
	...
	if (start_pos == NULL) {
		ngx_log_error(
			NGX_LOG_ERR, r->connection->log, 0, "ngx_http_vod_thumb_parse_uri_file_name: failed to parse width/height"
		);
		return ngx_http_vod_status_to_ngx_error(r, VOD_BAD_REQUEST);
	}
#endif // NGX_HAVE_LIB_SW_SCALE
	...
	if (rc != NGX_OK) {
		ngx_log_debug1(
			...
			r->connection->log,
			0,
			"ngx_http_vod_thumb_parse_uri_file_name: ngx_http_vod_parse_uri_file_name failed %i",
			rc
		);
		return rc;
	}
