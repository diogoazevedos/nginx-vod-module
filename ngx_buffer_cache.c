 ...
 *
 * 1. fixed size headers - contains the ngx_slab_pool_t struct allocated by nginx, the log context
 *    string and ngx_buffer_cache_sh_t
 *
 * 2. entries - an array of ngx_buffer_cache_entry_t, each entry has a key and points to a buffer
 *    in the buffers section. the entries are connected with a red/black tree for fast lookup by
 *    key. the entries section grows as needed until it bumps into the buffers section. each entry
 *    is a member of one of 2 doubly linked lists - the free queue and the used queue. the entries
 *    move between these queues as they are allocated / deallocated
 *
 * 3. buffers - a cyclic queue of variable size buffers. the buffers section starts at the end of
 *    the shared memory and grows towards its beginning until it bumps into the entries section.
 *    The buffers section has 2 pointers:
...
ngx_buffer_cache_rbtree_insert_value(
	...
	for (;;) {
		...
		if (node->key != temp->key) {
			...
			p = (ngx_memcmp(n->key, t->key, BUFFER_CACHE_KEY_SIZE) < 0) ? &temp->left : &temp->right;
		}
...
// NOTE: must be called with the mutex locked
static u_char*
ngx_buffer_cache_get_free_buffer(ngx_buffer_cache_sh_t* cache, size_t size) {
	u_char* buffer_start;

	// check whether it's possible to allocate the requested size
	if ((u_char*)(cache->entries_end + ENTRIES_ALLOC_MARGIN) + size + BUFFER_ALIGNMENT
	    > cache->buffers_end) {
		return NULL;
	}
	...
	for (;;) {
		...
		if (cache->buffers_write < cache->buffers_read
			...
			cache->buffers_write = cache->buffers_end;
			buffer_start =
				(u_char*)((intptr_t)(cache->buffers_write - size) & (~(BUFFER_ALIGNMENT - 1)));
			continue;
		}
...
ngx_flag_t
ngx_buffer_cache_fetch(ngx_buffer_cache_t* cache, u_char* key, ngx_str_t* buffer, uint32_t* token) {
	ngx_buffer_cache_entry_t* entry;
	...
	if (!sh->reset) {
		...
		if (entry != NULL
		    ...
		    && (cache->expiration == 0
		        || ngx_time() < (time_t)(entry->write_time + cache->expiration))) {
			result = 1;
			...
			*token = entry->write_time;
			...
			// NOTE: setting the access time of the entry and cache to prevent it from being freed
...
void
ngx_buffer_cache_release(ngx_buffer_cache_t* cache, u_char* key, uint32_t token) {
	ngx_buffer_cache_entry_t* entry;
	ngx_buffer_cache_sh_t* sh = cache->sh;
	uint32_t hash;
...
ngx_flag_t
ngx_buffer_cache_store_gather(ngx_buffer_cache_t* cache, u_char* key, ngx_str_t* buffers, size_t buffer_count) {
	ngx_buffer_cache_entry_t* entry;
	ngx_buffer_cache_sh_t* sh = cache->sh;
	...
	if (sh->reset) {
		// a previous store operation was killed in progress, need to reset the cache since the data
		// structures may be corrupt. we can only reset the cache after the access time expires
		// since other processes may still be reading from / writing to the cache
	...
	// NOTE: the memcpy is performed after releasing the lock to avoid holding the lock for a long
	// time setting the access time of the entry and cache prevents it from being freed
	sh->access_time = entry->access_time = ngx_time();
...
ngx_flag_t
ngx_buffer_cache_store(ngx_buffer_cache_t* cache, u_char* key, u_char* source_buffer, size_t buffer_size) {
	ngx_str_t buffer;
...
void
ngx_buffer_cache_get_stats(ngx_buffer_cache_t* cache, ngx_buffer_cache_stats_t* stats) {
	ngx_buffer_cache_sh_t* sh = cache->sh;
