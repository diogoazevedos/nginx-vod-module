...
* On-the-fly repackaging of MP4 files to DASH, HLS
...
* Video codecs: H264, H265, AV1, VP8 (DASH), VP9 (DASH)

* Audio codecs: AAC, MP3 (HLS), AC-3, E-AC-3, VORBIS (DASH), OPUS (DASH), FLAC (HLS), DTS (HLS)
...
* trackparams - can be used to select specific audio/video tracks.
	For example, manifest-a1.mpd will return an MPD containing only the first audio stream of each sequence.
...
Nginx-vod-module has the ability to perform on-the-fly encryption for MPEG DASH (CENC) and FairPlay HLS.
...
5. When using DRM enabled DASH, if the video files have a single nalu per frame, set `vod_min_single_nalu_per_frame_segment` to non-zero.
...
3. `hls` - Apple HTTP Live Streaming packager
4. `thumb` - thumbnail capture
5. `volume_map` - audio volume map
...
This cache holds the following types of responses for live: DASH MPD, HLS index M3U8.
...
The setting currently affects only HLS and DASH.
...
Same as `vod_expires` (above) for live requests that are not time dependent and not segments (e.g. HLS - master.m3u8).
...
Same as `vod_expires` (above) for live requests that are time dependent (HLS - index.m3u8, DASH - manifest.mpd).
...
Sets the seed that is used to generate the TS encryption key and DASH encryption IVs.
...
This setting only affects DASH configurations that have DRM enabled.
