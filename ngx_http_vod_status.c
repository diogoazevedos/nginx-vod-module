...
#define PERF_COUNTER_FORMAT \
	"<sum>%uA</sum>\r\n<count>%uA</count>\r\n<max>%uA</max>\r\n<max_time>%uA</max_time>\r\n<max_pid>%uA</max_pid>\r\n"

#define PROM_STATUS_PREFIX "nginx_vod_build_info{version=\"" NGINX_VOD_VERSION "\"} 1\n\n"
...
ngx_http_vod_status_xml_handler(ngx_http_request_t* r) {
	...
	if (perf_counters != NULL) {
		...
		for (i = 0; i < PC_COUNT; i++) {
			result_size += perf_counters_open_tags[i].len
			             + sizeof(PERF_COUNTER_FORMAT)
			             + 5 * NGX_ATOMIC_T_LEN
			             + perf_counters_close_tags[i].len;
		}
	...
	if (response.data == NULL) {
		ngx_log_debug0(
			NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "ngx_http_vod_status_xml_handler: ngx_palloc failed"
		);
...
ngx_http_vod_status_prom_handler(ngx_http_request_t* r) {
	...
	for (i = 0; i < vod_array_entries(cache_infos); i++) {
		...
		result_size +=
			((sizeof(PROM_VOD_CACHE_METRIC_FORMAT) - 1) + cache_infos[i].open_tag.len + NGX_ATOMIC_T_LEN)
				* vod_array_entries(buffer_cache_stat_defs)
			+ names_len
			+ (sizeof("\n") - 1);
	}
	...
	if (perf_counters != NULL) {
		for (i = 0; i < PC_COUNT; i++) {
			result_size += (sizeof(PROM_PERF_COUNTER_METRICS) - 1)
			             + (perf_counters_open_tags[i].len + NGX_ATOMIC_T_LEN) * 5;
		}
	}
	...
	if (p == NULL) {
		ngx_log_debug0(
			NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "ngx_http_vod_status_prom_handler: ngx_palloc failed"
		);
		return NGX_HTTP_INTERNAL_SERVER_ERROR;
	}
	...
	for (i = 0; i < vod_array_entries(cache_infos); i++) {
		...
		for (cur_stat = buffer_cache_stat_defs; cur_stat->name.data != NULL; cur_stat++) {
			p = ngx_sprintf(
				p,
				PROM_VOD_CACHE_METRIC_FORMAT,
				&cur_stat->name,
				&cache_name,
				*(ngx_atomic_t*)((u_char*)&stats + cur_stat->offset)
			);
		}
		*p++ = '\n';
	}
