...
ngx_http_vod_hls_init_segment_encryption(
	...
	if (rc != VOD_OK) {
		ngx_log_debug1(
			...
			submodule_context->request_context.log,
			0,
			"ngx_http_vod_hls_init_segment_encryption: aes_cbc_encrypt_init failed %i",
			rc
		);
...
ngx_http_vod_hls_handle_encryption_key(
	...
	ngx_memcpy(
		encryption_key, submodule_context->media_set.sequences[0].encryption_key, BUFFER_CACHE_KEY_SIZE
	);
...
ngx_http_vod_hls_init_ts_frame_processor(
	...
	rc = ngx_http_vod_hls_init_segment_encryption(
		submodule_context, segment_writer, HLS_CONTAINER_MPEGTS, &encryption_params
	);
	if (rc != NGX_OK) {
		return rc;
	}
	...
	if (encryption_params.type == HLS_ENC_AES_128) {
		...
		if (rc != VOD_OK) {
			ngx_log_debug1(
				...
				submodule_context->request_context.log,
				0,
				"ngx_http_vod_hls_handle_mp4_init_segment: aes_cbc_encrypt_init failed %i",
				rc
			);
			return ngx_http_vod_status_to_ngx_error(submodule_context->r, rc);
		}

		rc = aes_cbc_encrypt(encrypted_write_context, response, response, TRUE);
		if (rc != VOD_OK) {
			ngx_log_debug1(
				NGX_LOG_DEBUG_HTTP,
				submodule_context->request_context.log,
				0,
				"ngx_http_vod_hls_handle_mp4_init_segment: aes_cbc_encrypt failed %i",
				rc
			);
			return ngx_http_vod_status_to_ngx_error(submodule_context->r, rc);
		}
...
ngx_http_vod_hls_init_fmp4_frame_processor(
	...
	rc = ngx_http_vod_hls_init_segment_encryption(
		submodule_context, segment_writer, HLS_CONTAINER_FMP4, &encryption_params
	);
	if (rc != NGX_OK) {
		return rc;
	}
	...
	if (submodule_context->media_set.total_track_count > 1) {
		...
		if (rc != VOD_OK) {
			ngx_log_debug1(
				...
				submodule_context->request_context.log,
				0,
				"ngx_http_vod_hls_init_fmp4_frame_processor: mp4_muxer_init_fragment failed %i",
				rc
			);
			return ngx_http_vod_status_to_ngx_error(submodule_context->r, rc);
		}
...
ngx_http_vod_hls_merge_loc_conf(
	...
	ngx_conf_merge_str_value(
		conf->m3u8_config.encryption_key_file_name, prev->m3u8_config.encryption_key_file_name, "encryption"
	);
	ngx_conf_merge_str_value(
	...
	ngx_conf_merge_value(
		conf->m3u8_config.force_unmuxed_segments, prev->m3u8_config.force_unmuxed_segments, 0
	);
...
ngx_http_vod_hls_parse_uri_file_name(
	...
	// encryption key
		...
		ngx_http_vod_match_prefix_postfix(
			start_pos, end_pos, &conf->hls.m3u8_config.encryption_key_file_name, key_file_ext
		)
		&& !conf->drm_enabled
