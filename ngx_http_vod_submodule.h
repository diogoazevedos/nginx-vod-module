...
// macros
#define DEFINE_SUBMODULE(x)                                                 \
	const ngx_http_vod_submodule_t x = {                                    \
		(u_char*)#x,                                                        \
		sizeof(#x) - 1,                                                     \
...
// dash init segment, hls iframes manifest, hls master manifest, hls encryption key
#define REQUEST_CLASS_OTHER (0x08)
...
typedef void (*ngx_http_vod_create_loc_conf_t)(ngx_conf_t* cf, void* conf);

typedef char* (*ngx_http_vod_merge_loc_conf_t)(
	ngx_conf_t* cf, struct ngx_http_vod_loc_conf_s* base, void* conf, void* prev
);
