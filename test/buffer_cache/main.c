...
// debugging functions
void
print_queue(ngx_queue_t* queue, const char* name, u_char* relative_offset) {
	...
	for (cur = queue->next; cur != queue; cur = cur->next) {
		...
		printf(
			"\tSO=%lx BS=%zx ST=%lu\n",
			entry->start_offset - relative_offset,
			entry->buffer_size,
			entry->state
		);
	}
...
void
print_cache_status(ngx_buffer_cache_sh_t* cache) {
	printf(
		"ES=%lx EE=%lx BS=%lx BW=%lx BR=%lx BE=%lx\n",
...
// test functions
void
generate_random_buffer(unsigned int seed, u_char* buffer, size_t size) {
...
int
validate_random_buffer(unsigned int seed, const u_char* buffer, size_t size) {
	short filler = rand_r(&seed) & 0xFFFF;
	short* end_pos = (short*)(buffer + (size & ~1));
	short* cur_pos;
...
int
run_test_cycle(time_t seed, size_t cache_size, int iterations, int size_factor) {
	...
	printf(
		"starting test - seed %llu cache_size %zu iterations %d size factor %d\n",
	...
	for (i = 0; i < iterations; i++) {
		...
		max_size =
			(sh->buffers_end - (u_char*)(sh->entries_end + ENTRIES_ALLOC_MARGIN + 1) - BUFFER_ALIGNMENT)
			/ size_factor;
		...
		if (stats.store_ok - stats.evicted != i + 1 - min_existing_index) {
			printf(
				"Error: unexpected number of items in the cache, stats=%lu fetched=%d\n",
				stats.store_ok - stats.evicted,
				i + 1 - min_existing_index
			);
			return 0;
		}
