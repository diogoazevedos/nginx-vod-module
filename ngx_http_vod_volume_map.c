...
ngx_http_vod_volume_map_init_frame_processor(
	...
	if (rc != VOD_OK) {
		ngx_log_debug1(
			...
			0,
			"ngx_http_vod_volume_map_init_frame_processor: volume_map_writer_init failed %i",
			rc
		);
...
static void
ngx_http_vod_volume_map_create_loc_conf(ngx_conf_t* cf, ngx_http_vod_volume_map_loc_conf_t* conf) {
...
ngx_http_vod_volume_map_parse_uri_file_name(
	...
	if (ngx_http_vod_match_prefix_postfix(
			start_pos, end_pos, &conf->volume_map.file_name_prefix, csv_file_ext
		)) {
		...
		*request = &volume_map_request;
		...
		ngx_log_error(
			NGX_LOG_ERR, r->connection->log, 0, "ngx_http_vod_volume_map_parse_uri_file_name: unidentified request"
		);
	...
	if (rc != NGX_OK) {
		ngx_log_debug1(
			...
			r->connection->log,
			0,
			"ngx_http_vod_volume_map_parse_uri_file_name: ngx_http_vod_parse_uri_file_name failed %i",
			rc
		);
		return rc;
	}
